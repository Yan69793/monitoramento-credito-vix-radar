...
Última atualização: 2026-09-12 (noite). Medido ao vivo: Worker v4.9.246, produção saudável (`ok:true`, `painel_fresco:true`, `feed_fresco:true`) e a configuração de skills reproduzível no clone. O estado de sincronização do Git é sempre medido pelo gate antes do push, sem tratar um hash de `HEAD` como igualdade permanente. **OR402-DEGRADADA1, QUARENTENACOB1, FALLBACKTTL1, EMAILSILENT1 residual de lote e o código de SCANFALLBACK-MORTO1 estão FECHADOS no repositório e em produção na v4.9.245.** **SCANFALLBACK-MORTO1 segue aberto apenas pela prova runtime real do passo Scan de emergencia.** **Sessão de 12/09: `main` sincronizado depois de 8 commits de atraso, gate de ancestralidade em `deploy-worker.ps1` (`3209aac`) e reidratação de sessão no `AGENTS.md` (`1fd8eda`), sem deploy.** **Biblioteca de skills: `.claude/skills/` é o canonico versionado, `.agents/skills/` é local e não versionada, os 5 junctions globais VIX apontam diretamente para o canonico, e `kilo.json` versionado aponta para `./.claude/skills`.**

> [!success] 13/09 (tarde): **DRENOMUDO1 fechado no repositório (commit `c8ab9f9`), sem deploy.** O dreno da fila de verificação que a varredura chama no fim escrevia sempre `<POS-ROTINA>: dreno concluido (exit=N)`. Medido em 13/09: `POS-MATINAL: dreno concluido (exit=5)` com a fila NÃO drenada, porque a verificação assíncrona abortou sem credencial (cota de assinatura estourada às 16:46) e esse ramo do aborto era o único de auth que não levantava `ALERTA_AUTH`, então o vigia diário não tinha o que ler. O motor passa a avaliar o código de saída (`Get-VixDrenoTexto`, exit 0 mantém `concluido`, qualquer outro vira `FALHOU` com o código) e registra `ERRO DRENO` com o caminho do log do dreno. O ramo "nenhuma credencial" da verificação passa a levantar `ALERTA_AUTH` (e `DRYRUN_ALERTA_AUTH` em dry-run) e a notificar o admin, igual ao ramo irmão de escalada paga. **Provas:** `test-varredura-defeitos` 18/18 com o D5 de duas pontas (o texto do motor em `HEAD` com exit 5 carregava `concluido`, medido), `test-dryrun-metrics` 21/21 em PS 5.1 e pwsh 7 (mesmo regex reprova o fonte antigo e aprova o novo), suíte agregada `run-all-tests` 22 verdes com as 2 vermelhas do baseline de 12/09, `lint-encoding` 114 `.ps1` + 5 shell RISCO 0, `check-claude-free` exit 0, parse PS 5.1 sem erro nos 4 arquivos. **Sem deploy**, a mudança é local no motor chamado pelo Task Scheduler, Worker e Pages intocados. **Contexto do dia, medido:** a matinal rodou 16:10 em vez de 10:00 porque a máquina subiu às 16:05 (gatilho 10:06, `StartWhenAvailable=True`, `NumberOfMissedRuns=0`), o lote full-4 morreu no limite de sessão às 16:46 com `LastTaskResult=7` (código do aborto por auth, não benigno para o `monitor-tasks`), 11 emissores ficaram `DEFERIDO` por cap e 2 lotes não foram processados. **Condição de Obsolescência:** cai se o dreno voltar a registrar sucesso com exit diferente de zero, ou se o ramo sem credencial deixar de levantar `ALERTA_AUTH`.

> [!success] 12/09 (noite) — **ASSINATURA-PRO1: as duas rotinas de produto rodam na assinatura, com retry de verdade e o caminho paralelo do Claude Desktop aposentado.**
> **Status:** fechado no repositório e no scheduler local, 6 commits pushados (`0cbfd6a..218e575`), sem deploy de Worker ou Pages. **Data:** 2026-09-12. **Origem do Registro:** correção do provider depois de duas rotinas perdidas no mesmo dia.
> **Causa raiz medida.** O commit `0cbfd6a`, ao reativar `claude-subscription`, adicionou um ramo novo dentro de `Test-VixLlmPermiteClaude`. Essa função é usada em dois lugares com significados diferentes: em `Get-VixAnthropicApiKey` ela decidia se a **chave paga** podia ser lida, e passou a devolver `$true` para a assinatura também. Efeito: token de assinatura recusado às 05:01 e às 10:06, fallback mudo para chave paga, `Set-VixClaudeAuthEnv` injetou `ANTHROPIC_API_KEY` no processo, o próprio CLI desabilitou o login (`claude.ai connectors are disabled because ANTHROPIC_API_KEY ... takes precedence over your claude.ai login`), `AUTH_MODO: nenhum` e exit 5 sem um único lote. A assinatura ficou pior por causa de uma credencial que não pode ser usada.
> **Correção.** `Get-VixAnthropicApiKey` passa a exigir `claude-manual` + `-ForceClaude` e devolve `null` (não exit 86) quando não for o caso, então a assinatura segue normal e só a escalada paga fica desarmada. `Test-VixClaudeSessionLimit` separa cota de credencial (as duas dão exit 1) e `Test-VixClaudeSaldoPagoZerado` nomeia o 400 `Credit balance is too low`. O log final passou a ler a sonda do **token**, não a do credential store, e portanto parou de reportar limite de cota como credencial inválida.
> **Medição das três pontas (sonda real).** (A) só o token de assinatura devolve 429 `You've hit your session limit, resets 9:10pm`, logo o token **vale** e só falta cota. (B) só a chave paga devolve 400 `Credit balance is too low`, a chave é válida e sem saldo. (C) ambiente limpo devolve o **mesmo** 400, ou seja o credential store do CLI resolve para cobrança de API nesta máquina e não é caminho de assinatura.
> **Retry.** `retry-vixradar.ps1` relancava por `run_claude_routine.ps1`, que executa SKILL do Claude Desktop, caminho legado morto desde o MOTOR1 e que a própria SKILL proibia chamar o motor (`NAO chame run_vixradar_noturno_claude.ps1`). `Get-VixRetryRunner` mapeia por rotina para o wrapper do motor e não passa `-RoutineId`, que os wrappers não aceitam. Dimensionamento medido: a noturna leva `duracao_sec=5135.7` (85 min) nos 104 emissores, com `ExecutionTimeLimit=PT4H` a partir de 18:05, então morre 22:05 e um reset por volta das 21h **não cabe**. Os gatilhos do retry (21:30 e 23:20) caem depois do reset e ele sai no-op quando o ledger confirma entrega. Em `monitor-tasks` ele saiu de `GuardedDisabled` e entrou em `MustBeEnabled`: Enabled é o esperado, Disabled volta a ser erro. O retry da matinal segue desligado.
> **Scheduler e caminho paralelo.** `VIXRadar-Noturno` estava `Disabled` com motor `task-scheduler` ativo (9002 real) e o `$Desejado` do `cutover-motor.ps1` dizia `VIXRadar-Matinal = $false`, ao contrário do scheduler vivo, desde `0cbfd6a` e sem registro na mensagem do commit; alinhado para `$true` e `-Acao Ativar` rodado (snapshot `cutover-snapshot_20260912_200933.json`, reversível). Task avulsa `VIXRadar-Noturno-20260912-0801` removida, definição preservada em `logs/monitor-tasks/removida_VIXRadar-Noturno-20260912-0801.xml`. As duas SKILLs do Claude Desktop viraram lápide que recusa execução: o Passo 0 delas rodava `Disable-ScheduledTask` na task do motor, origem do "guard quebrado", e a implementação paralela já corrompeu o feed em 08/08 (segunda execução leu fonte mais velha e sobrescreveu 3 emissores: Rumo, Bradesco, Tupy). Originais preservados em `~\.claude\scheduled-tasks\backups\20260912-2032\`.
> **Provas.** `test-locks-overlap` 21/31 → **31/31** (as fixtures gravavam `pid` sem `inicio_utc` e o D1 de `0cbfd6a` passou a exigir PID vivo E mesmo StartTime, então o caso B acusava o motor de apagar lock alheio quando ele seguia a regra nova; mais 4 asserts com a redação antiga `tocado ha N min`, e o `| Out-Null` nas chamadas `Assert-*` engolia a própria mensagem, então 10 falhas imprimiam 3). `test-retry-janela` **24/24** com a Parte 3 nova, que extrai `Get-VixRetryRunner` por AST. `lint-encoding` 113 `.ps1` + 5 shell, RISCO 0. `check-claude-free` exit 0. `check-desktop-orquestrador-drift` de 4 divergentes para **OK** (2 eram desta sessão, 2 já existiam sem ninguém notar). `monitor-tasks -DryRun` sem `ERRO MOTOR` e sem 9002. Produção intocada: `HTTP:200`, `ok:true`, `versao:v4.9.246`, `painel_fresco:true`, `feed_fresco:true`, `feed_evento_mais_novo:2026-09-11`, `feed_idade_du:0`, `kv:true`, `telemetria:true`, `sentry_ok:true`, `verificador_ok:true`.
> **OpenRouter descartado por decisão do operador.** Não haverá crédito: a conta está zerada (medido `total_credits=299`, `total_usage=299.1988`, restante −0,20 USD) e era essa a razão real da reativação da assinatura. O `PROVIDER-SPLIT1` (commit `5ca6f3e`) foi revertido em `ab0e6cd`: a noturna **não** sai por OpenRouter, e a assinatura é o único caminho.
> **Resíduo conhecido.** O motor que aborta entre a criação do lock e o `finally` deixa lock de probe para trás (visto em 12/09: o dry-run das 20:21 saiu no exit 5 de auth e deixou `vixradar-noturno_20260912_dryrun_24772.lock`). Não bloqueia nada, porque `Get-VixLockState` classifica PID morto como `LOCK_ORFAO`, mas faz a verificação de higiene da suíte de locks acusar. Limpeza manual ou `-Force`. As entradas `vixradar-noturno` e `vixradar-matinal` seguem no catálogo do `run_claude_routine.ps1` apontando para as SKILLs aposentadas, sem chamador.
> **Condição de Obsolescência:** cai se a escalada paga voltar a ser liberada sem `claude-manual` + `-ForceClaude`, se o retry voltar a relancar o runner legado, ou se a matinal passar a ter relançamento automático.

> [!success] 11/09 a 12/09. **A biblioteca de skills foi corrigida, validada em Windows PowerShell 5.1 e pwsh 7, e a configuração `kilo.json` passou a ser versionada.**
...
> **Status:** RESOLVIDO no repo (`origin/main=112ffbe`), sem deploy de Worker/Pages. **Data:** 2026-09-10. **Origem:** `/vix-radar-next-steps` → execução dos quick wins + destravamento do CI.
> **Actions:** `checkout@v4`/`setup-node@v4`/`upload-artifact@v4` (Node 20 deprecado, prazo 23/09/2026) → `@v6` (Node 24) nos 7 workflows. **Case F:** `scripts/test-locks-overlap.ps1:379` — `Start-Process -ArgumentList` sem aspas no caminho do runner cortava o caminho (filho morria com `-196608` sem executar); aspas embutidas, mesmo padrão de G/H. Commit `8b26d74` (8 arquivos, 16+/16−).
> **Drift Pampa Sul:** `check-emissores-cnpj.mjs` reprovava `nao_declarado` + `primario_worker_diverge` + `predictiva_diverge` — o 104º emissor tinha CNPJ `04.739.720/0001-24` no `worker.js` e no predictive, mas faltava no canônico. Adicionado `EMISSOR_CNPJ` + `SNAPSHOT_CVM`. Commit `112ffbe` (1 arquivo, 3+).
> **Provas:** test-locks-overlap 31/31 OK (sem `-ComTokens`), YAML 7/7, guards CNPJ/alias/metricas/familia exit 0, check-claude-free exit 0. CI push-triggered **4/4 verdes** (claude-free, worker-tests, frontend-qa, emissores-cadastro). `HEAD == origin/main`, ahead/behind 0 0.

> [!info] 10/09 (tarde) — **FULL model resolvido: `deepseek/deepseek-v4-pro-0813`. Sem alternativa melhor de custo/benefício.**
> **Status:** CONFIGURADO (env User `VIXRADAR_OPENROUTER_MODEL_FULL`), sem deploy. **Data:** 2026-09-10. **Origem:** decisão do operador ("full model é o deepseek v4 pro, ou o melhor custo/benefício no OpenRouter").
> **Config:** `VIXRADAR_OPENROUTER_MODEL_FULL = deepseek/deepseek-v4-pro-0813` (User). Antes, FULL caía no Flash. Verificado via adapter: `FULL=pro-0813`, `LIGHT=flash-0731`, `fallback=flash`.
> **Custo (por 1M tokens, OpenRouter):** Flash 0731 = $0,065 input / $0,18 output · Pro 0813 = $0,58 input / $1,74 output (**~9×**). Cache read quase igual ($0,016 vs $0,018).
> **Sem alternativa melhor:** V4.1 Flash ($0,15/$0,60) é tier flash (raso, ~8B ativos — insuficiente pra verificação adversarial). V4.1 Pro tem preço "roughly flat" (sem ganho de custo). Concorrentes mais caras: Qwen3.8 Max $2/$6, Claude Fable 5.1 $10/$50. Pro 0813 é o deep model mais barato que faz o serviço.
> **Efeito:** só a fila aprofundada (FULL) paga o ~9×; LIGHT (maioria, 104 emissores) segue no Flash. Disjuntor conta tokens, não dólar.

> [!success] 10/09 — **Cobertura de 09/09 fechada em 104/104, medida no ledger. Nenhuma rotina executada, sem deploy, sem mudança de scheduler.**
> **Status:** CONCLUÍDO (verificação, sem mudança de código). **Data:** 2026-09-10. **Origem do Registro:** pedido do operador para recuperar o Radar com cobertura de 09/09 e do dia corrente provada ou explicada.
> **Medição.** `logs/routines/vixradar-noturno_20260909.log` tem 104 linhas `OK|`, 104 emissores distintos, 65 `ANALISADO` e 39 `SKIP`, zero repetição. Os 39 SKIP são crédito de análise de 08/09 dentro da janela de 14h, não buraco de cobertura. O cadastro inteiro ficou com análise recente no dia.
> **A dúvida do dia, resolvida.** A noite de 09/09 tem duas parciais. A das 00:24 foi reexecução extra fora do slot canônico, entregou o lote `light-1` (15 analisados) e morreu sem linha `FIM`, deixando 54 linhas no ledger (39 SKIP + 15 ANALISADO). A das 18:05 fechou com `Fila LIGHT: 50`, `submit_ok=50`, `deferidos=0`. O plano das 18:05 tinha 96 não-SKIP (8 SKIP, 45 LIGHT, 51 FULL) e a fila ficou em 50 porque 46 desses já tinham linha `OK|` do run da madrugada. A regra é `Fila = 104 − tier SKIP − ledger do dia` (`scripts/run_vixradar_varredura.ps1:925-926`), sem corte por cap: nenhuma linha `CAP pre-lote` no log e `cap_efetivo=700000` intacto. `listar_plano_rotina` sempre devolve os 104; o que varia entre execuções é quanto o ledger de idempotência remove.
> **Duplicatas.** Nenhuma no ledger, medido. No feed, as duas reposições de 09/09 (20:07 e 20:11) reenviaram os mesmos dois payloads byte a byte, mesma `data_evento` e mesma `fonte_primaria`, então a segunda caiu na mesma chave de dedup (`data|empresa|host+path`). Prova por construção, não por leitura direta do feed, que exige `routine_key` e não é passada por linha de comando.
> **Dia corrente (10/09).** Janela ainda não abriu no momento da medição (01:51 BRT). `VIXRadar-Matinal` 10:06, `VIXRadar-Verificacao-Async` 11:03, `VIXRadar-Noturno` 18:05, e a fila de verificação estava vazia nos três drenos de 09/09. Nada pendente para fechar.
> **Scheduler intocado.** 8 tasks `Ready`, `VIXRadar-Health-Watch` `Disabled` por decisão de 21/08. O matinal de 09/09 voltou `Result=267014` (terminado à força) no lote 4 de 23 e não houve retry, as duas `Szuchmacher-RetryVixMatinal/Noturno` seguem `Disabled`. O noturno absorveu os 15 restantes em profundidade LIGHT, então caiu profundidade, não cobertura. Registrado para medir se repetir, sem mexer no agendamento agora.
> **Produção (medida em 10/09):** `HTTP:200 TEMPO:3.003772s`, `ok:true`, `versao:v4.9.244`, `kv:true`, `rate_limiter:true`, `telemetria:true`, `sentry_ok:true`, `verificador_ok:true`, `feed_fresco:true`, `feed_idade_du:2`, `feed_evento_mais_novo:2026-09-08`, `painel_fresco:true`. `logs/routines/cobertura_*.json` = 0 arquivos, sem lock de noturno ativo.
> **Sem deploy, sem código, sem scheduler.** Além de leitura, nada foi executado nesta verificação.
> **Condição de Obsolescência:** cai se aparecer linha `OK|` faltante ou duplicada no log de 09/09, ou se uma execução nova reabrir aquela data.

> [!success] 10/09 — **PREDICTIVE-PRIVACY1 preparado e validado, sem deploy.** Os 47 `data/historico/*/predictive.json` deixam o próximo HEAD, mas continuam fisicamente locais para backtests e ignorados pelo Git. `.gitignore`, `scripts/hooks/pre-commit` e `scripts/run_vixradar_export_historico.ps1` preservam o dump local e bloqueiam entrada por staging explícito. O gate rejeita o caso ruim e aceita um commit sem o payload. `prob_30d` e `prob_90d` continuam somente no laboratório administrativo protegido, com `user_facing:false`; o frontend publicado não contém esses campos nem `predictive_v1`. A calibração estatística por emissor continua pendente.

> [!success] 10/09 — BUSCADEGRADADA1 FECHADO/PASS em `main`. Fix `990dbb0886e97872ac1557a8d8cb4669f79d7f35` (`fix: persist degraded-search recheck until resolution`, 2 arquivos, +193/−47), depois do histórico legítimo `3e51ef8564e4bde3bacbaf0e936799b18c8477bf` (`chore(data): historico 2026-09-09`, só `data/historico/2026-09-09/`, +43). Push fast-forward `edbfe69..990dbb0`; `HEAD == origin/main == 990dbb0`, ahead/behind `0 0`.
> **O que mudou:** `RECHECK_PENDENTE` de cobertura passou a valer ATÉ SER RESOLVIDO, sem a janela de 2 dias. A priorização lê TODOS os `logs/routines/cobertura_*.json`, do mais novo para o mais antigo, e resolve cada emissor pelo registro mais recente dele. Sem `RESOLVIDO` explícito a pendência continua na frente da fila, e fim de semana ou dia sem execução não resolve nada. Resolução só é gravada com submit aceito e cobertura fechada válida (`$subOk -and -not recheck`), então busca degradada não vira `sem_eventos` nem sai da fila.
> **Causa raiz do defeito que isto fecha:** a leitura antiga usava `.Values` sobre o `PSCustomObject` devolvido por `ConvertFrom-Json`, e `.Values` não existe nesse tipo, então contrato gravado nunca priorizava ninguém. A leitura agora é por `PSObject.Properties`.
> **Prova:** `test-cobertura-contrato` 29/29 (PS 5.1 e pwsh 7), com pendência de 5 dias ainda priorizada (D1), prova reversa do `.Values` (D1b), `RESOLVIDO` mais novo limpa (D2), `RESOLVIDO` anterior não limpa (D3), buraco de fim de semana não resolve (D4), leitura idempotente (D5), contrato corrompido ignorado (D6), wiring no motor (D7/D7b). Mais `test-busca-degradada` 31/31, `test-locks-overlap` 13/13, `test-dryrun-metrics` 18/18, `check-claude-free` EXIT=0, `lint-encoding` 110 `.ps1` + 5 shell, RISCO 0.
> **Produção (medida em 10/09):** `HTTP:200 TEMPO:3.010440s`, `ok:true`, `versao:v4.9.244`, `kv:true`, `telemetria:true`, `sentry_ok:true`, `feed_fresco:true`, `feed_idade_du:2`, `painel_fresco:true`, `feed_evento_mais_novo:2026-09-08`. Frontend `v202.43` em repo e em produção, sem drift.
> **Backfill 09/09 = NENHUM.** Três rodadas dirigidas sobre os 16 emissores que fecharam INCONCLUSIVO por busca degradada em 09/09 não acharam fato de crédito datado 09/09 ausente do feed. O que apareceu foi o comunicado da Vale de 09/09 negando decisão sobre emissão de títulos de dívida (negativa, sem impacto, fontes fora da lista confiável do Worker) e a 22ª emissão da Hypera, cujo fato é de 08/09 e já está na fronteira do feed. Não criar evento artificial.
> **`cobertura_20260909.json` ausente é legado operacional.** A noite de 09/09 registrou os 16 como INCONCLUSIVO no log e não persistiu contrato, então a priorização não tem o que ler para aquela data e eles voltam pela rotação normal. Não reconstruir o arquivo à mão, seria inventar estado a partir de log. A persistência passa a valer a partir da primeira execução que gravar contrato.
> **Sem deploy.** A mudança é do motor local chamado pelo Task Scheduler, Worker e Pages intocados.
> **Data:** 2026-09-10. **Origem do Registro:** fechamento do item aberto em `Obsidian VIX Radar/03 - Estado Atual.md`. **Condição de Obsolescência:** cai se a priorização voltar a ter janela de data ou se `RESOLVIDO` passar a ser gravado sem submit aceito.

> [!success] 08/09 (madrugada) — OPENROUTER-429-SENTINELA: causa raiz provada, fix validado em execução real (sem deploy).
> **Status:** RESOLVIDO no repositório local (`HEAD=e155d57`, branch `pub/delta-2026-09`, pendente push/deploy). **Data:** 2026-09-08. **Causa raiz do 429 de 07/09 (17/17 lotes falharam):** modelo operacional `~deepseek/deepseek-v4-flash-latest` (alias) caía em upstream saturado — OpenRouter devolve `429 "Provider returned error"` com `provider_name:null` — agravado por `allow_fallbacks=false` no roteamento do adapter (um upstream ruim derrubava o lote inteiro) e retry `0/5/20` (≈27s) insuficiente para episódio de horas. Não era calendário (07/09 = feriado B3, matinal/noturno SKIP legítimo), nem scheduler (todas as tasks dispararam), nem crédito (mesma chave 200 em teste controlado), nem model ID inválido (`claude-sonnet-4-6` → 400, nunca enviado). Labels `claude-sonnet/haiku` eram cosméticas — o adapter sempre enviou DeepSeek.
>
> **Correção (commit `e155d57`, 12 arquivos, 449+/148−):** modelo fixo `deepseek/deepseek-v4-flash-0731` (sem alias operacional); fallback explícito `deepseek/deepseek-v4-flash` mesmo contrato de tools; `Retry-After` do 429 respeitado (cap 120s); 400/401/403 sem retry/fallback (determinísticos); exit **9** quando 0 lotes por falha de provider (antes: exit 0 falso); logs registram modelo real/intentos/fallback/status/Retry-After; KV de export/reconciliação com retry bounded + classificação de falha (transiente ≠ "sem série"); timeout 12 min + teto de parede por lote (`TotalTimeoutSec`).
>
> **Validação real (08/09, 01:22–01:36 BRT):** sentinela controlada teto 8 → **resultado=OK, 8/8 emissores processados** (Raízen 7, Eneva 2, CEMIG 14, Eletrobras 5, CCR 3, Natura&Co 8, Cyrela 1, Vibra 5 = **45 docs CVM marcados**), `submit_fail=0`, `lotes_ok=2, lotes_falha_provider=0`, **0 timeouts, 0 429**, modelo efetivo `deepseek/deepseek-v4-flash-0731` nos 2 lotes, backlog preservado (`backlog=True`, `excedente_worker=41`), exit 0. Testes: adapter 17/17, dispatch 20/20, contratos 24/24, payload 10/10, wrangler-falha 10/10 (novo), smoke real 200, PROD-LIKE 200 nos dois modelos, parse PS 5.1 12/12, `git diff --check` limpo, sem secrets. **Produção (Worker/Pages/KV) intocada** — pendência: push + deploy quando autorizado. Fix adicional no mesmo commit: `Test-VixClaudeAuthFailure` só avaliado no ramo claude (no openrouter a função não existia e o erro de runtime mascarava o FIM com exit 0 falso).
>
> **Fechamento PRs (08/09):** PR #23 (INCIDENTE-FRESHNESS2) — a correção do registrador AgendaSemanal (`7879d50d1`) **já está em origin/main** via `c6411d5`, mesmo conteúdo funcional; resto da branch era drift (freshness/429/worker) já coberto por outros commits em main ou substituído; PR será fechada sem merge. PR #24 — bloco morto de 12 linhas (`RoutineId vixradar-agenda-semanal` em `run_claude_routine.ps1`, inalcançável: `ContainsKey` → exit 2 no pre-flight; catálogo atual só tem `atualizar-agenda-macro-szuchmacher`/`vixradar-noturno`/`vixradar-matinal`; wrapper dedicado `run_vixradar_agenda_semanal.ps1` trata a própria `ROUTINE_API_KEY`) **removido em commit separado**, provado sem referência remanescente.


> [!success] 06/09 (noite) — Frontend-QA (baseline) e D2 Fases 1–2 fechados (sem deploy).
> **Status:** CONCLUÍDO no repositório (`origin/main=10e8e18`); pendência humana = aprovação estética das capturas BEFORE×AFTER. **Frontend-QA:** suíte `app/tests` (Playwright+axe+Lighthouse) e baseline aprovado versionado em `836e095` (`axe-baseline.json` + `landing-desktop/mobile.png`; CI normal verde `34046032345`); workflow `.github/workflows/frontend-qa.yml` gera baseline Linux via `workflow_dispatch generate-baseline` e nunca commita (`permissions: contents: read`). **D2 Fase 1** (`f38e6f5`, 11+/7-): corpo 13px, `.ph-lead` 16px/400, card demo mais legível, `:focus-visible` dourado global. **D2 Fase 2** (`d44f37b` 2+/2- e `10e8e18` 3+/3-): stat-cards radius 10px/grid 14px; EWS rows radius 8px, hover dourado `rgba(183,152,93,.06)` (substitui azul solto), rank 11px. Baseline atualizado em commit separado (`01992d6`). CI final verde em cada etapa (`34047047264`, `34047304063`, `34047661718`); axe sem regressão nova (desktop 0 / mobile `link-in-text-block` 1, igual ao baseline); Lighthouse 56/98/96/100 (não pior); `git diff --check` OK; custo R$ 0; sem deploy/backend/API. **Riscos registrados:** (1) flap leve do PNG desktop entre regenerações sem mudança visual — se reaparecer, congelar relógio/animações/font loading antes de atualizar baseline; (2) Actions `checkout/setup-node/upload-artifact@v4` seguem em runtime Node 20 (aviso em todo run) — migrar antes de 23/09/2026. **Reversão D2:** `git revert 10e8e18 d44f37b 01992d6 f38e6f5` (mantém o scaffolding do Frontend-QA).

> [!success] 06/09 (tarde) — SWEEP-ORFAOS1/LIVENESS1 fechado e deployado (v4.9.243).
> **Status:** RESOLVIDO, em produção. **Data:** 2026-09-06. **Causa:** falso-verde estrutural — `sweepFilaVerificacaoOrfaos` removia itens >48h da fila sem registro durável, e o health media a FILA (verde) com `_pendente_verificacao` ainda em aberto. **Três provas tentadas:** releitura de KV (refutada — consistência eventual devolve snapshot velho e recria falso-verde); remoção da fila como prova (refutada por LIVENESS1 — órfão só existe porque o sweep já esvaziou a fila, então exigir remoção nova prende o órfão para sempre); **solução adotada:** prova = retorno DIRETO das escritas terminais do EVENTO (`mesclarEventoVerificado() !== false` aprovado / `retratarEventoRejeitado() === true` reprovado sem fonte), nunca releitura de KV; marcador de conclusão `radar:verif:concluido:{id}` (sem TTL) gravado ANTES do delete do órfão; reconcile no sweep só resolve com marcador posterior ao `expirado_em` (anterior/stale não). `removerDaFilaVerificacaoInterno` passou a devolver `{removido:true}` só após put/delete sem erro (falso em leitura_falhou/sem_escrita_id_ausente/escrita_falhou) e apaga a chave do dia quando a fila esvazia (delete-when-empty). **Commits:** `811c7c8` (SWEEP-ORFAOS1: órfão registrado antes da remoção e só resolve por conclusão terminal), `a6791bc` (LIVENESS1 fecha convergência + write-proof da remoção + delete-when-empty), `df95d05` (chore deploy). **Provas:** suíte 323/323 em 31 arquivos (sweep-orfaos com 25 testes de duas pontas: merge recusado com fila vazia não resolve, merge confirmado resolve mesmo com fila vazia, `fila_remover_put` injetado não prova, id ausente sem escrita não vale, delete-when-empty); portão `HTTP:200`, `versao v4.9.243`, `ok:true`, `kv:true`, `telemetria:true`, `sentry_ok:true`, `verificador_ok:true`, `admin_email_ok:true`, `verif_orfaos_ativos:0` nos dois endpoints. **Rollback nativo:** version `5745bfbe-b54e-4515-9d9a-10d6ab0aada6` (v4.9.242).

> [!success] 05/09 (noite) — MERGEDUP1 corrigido e deployado (v4.9.241).
> **Status:** RESOLVIDO, em produção. **Data:** 2026-09-05. **Causa:** `mesclarEventoVerificadoInterno` casava o evento verificado por chave dedup recalculada (que inclui host+path de `fonte_primaria`), então um veredicto CORRIGIR que reescreve a fonte mudava a chave e empurrava duplicata em vez de atualizar. **Fix:** `it.id` viaja como `chaveOriginal`; casou 1, atualiza no lugar; casou 0 ou mais de 1, aborta sem gravar (`mesclas_recusadas`); reenvio idempotente vira no-op. **Provas:** `mergedup-verificador.test.mjs` (7, prova de 2 pontas) + `verif-cache-roundtrip.test.mjs`; suíte 27 arquivos / 261 testes (260 verdes + 1 `login-timing` flaky sob paralelo, isolado 3/3). **Portão:** `HTTP:200`, `versao v4.9.241`, `ok:true`, `kv:true`, `telemetria:true`, `sentry_ok:true`, `verificador_ok:true`. Pós-deploy somente leitura: 104 emissores, Pampa Sul com exatamente 2 eventos (ITR fonte CVM RAD 160616, ANEEL sem fonte primária), fila com 2 itens intacta. **Task `VIXRadar-Verificacao-Async` reabilitada em 06/09** (decisão do operador; State Ready, triggers intactos, ver bloco 06/09 acima e `PENDENCIAS.md`).

> [!success] 05/09 (madrugada) — CLAUDE-FREE-MIGRATION Fase B D1: OpenRouter virou o transporte do runtime automático.
> **Status:** CONCLUÍDA no código, com prova offline e canários reais mínimos. **Data:** 2026-09-05. **Transporte novo:** adapter HTTP próprio `scripts/lib/vixradar-openrouter.ps1` (POST `openrouter.ai/api/v1/chat/completions`, modelo `deepseek/deepseek-v4-flash`, web_search/web_fetch por server tools, retry bounded `@(0,5,20)`, envelope `.result`/`.usage`/`.is_error` no mesmo formato do CLI). **Provider efetivo persistido:** `VIXRADAR_LLM_PROVIDER=openrouter` em escopo User.
>
> **O que mudou no código:** o motor `run_vixradar_varredura.ps1` (cobre matinal e noturno, 103 emissores) já despachava pelo adapter desde o canário real PASS; nesta leva `verificacao_async`, sentinela e `agenda_semanal` ganharam o mesmo branch openrouter no boot e no dispatch. **5/5 rotinas migradas** (matinal, noturno, verificacao_async, sentinela, agenda_semanal). Transport-first: prompt, parser e contrato de saída intactos (motor/sentinela = `RESULTADO|` textual; verificacao_async/agenda = JSON puro). O ramo `claude`/Anthropic (CLI local, `Initialize-VixClaudeAuth`, escalação) virou `else` só alcançável com `claude-manual` + `-ForceClaude` manual do operador, fora do scheduler. Exit 86 / `BLOQUEADO_SEM_PROVIDER` / fail-closed / Gate 8 / `check-claude-free.ps1` / guarda incidente-73 preservados.
>
> **Provas:** smoke real (B1) HTTP 2xx + resposta parseável + exit 0; canário real do motor (B2) com `AUTH_MODO: openrouter` no log, parser `RESULTADO` intacto, zero `claude.exe`/auth Anthropic/escalação; `test-openrouter-adapter.ps1` 45/45; `test-openrouter-contratos.ps1` 24/24 (parsers reais por AST, fail-closed por contrato); `test-openrouter-dispatch.ps1` 20/20 em pwsh 7 **e** PS 5.1 (dispatch real chama adapter, ramo CLI legado não tocado, tokens via `.usage`); parse das 3 rotinas wired + motor em PS 5.1 exit 0; `check-claude-free.ps1` exit 0; lint-encoding limpo. **Scheduler real medido e preservado como estava** (não é efeito colateral desta leva): Matinal/Noturno/Verificacao-Async `Disabled`, Sentinela/AgendaSemanal `Ready`, 2 retries `Ready`; divergência vs declarativo `cutover-motor` (5 Enabled) é domínio do operador, documentada em `PENDENCIAS.md`.
>
> **Produção (Worker/Pages/Cloudflare) INTOCADA.** Pendências que ficam: os portões G1-G5/G7 da Fase A (agora o operador decide se seta provider `openrouter` de forma definitiva e reata o scheduler), migração KV→DO, rotação da `routine_key` (recusada em 03/09, não reabrir). Detalhe e inventário em `PENDENCIAS.md` (bloco 05/09) e no commit de fechamento da Fase B D1.
...
- **11/09 (triagem readonly) — contradições documentais medidas, nenhuma fechada só por documento:** o índice ficou em 2 arquivos (`api/src/worker.js`, `api/test/email-registro-rastro.test.mjs`), sem deploy, sem `git add`/`reset`/`checkout`/stash — **a afirmação de 'índice congelado/sem commit' caducou em 11/09: os 2 arquivos foram commitados em `dd37c8e`, CI `Worker Tests` run `34646171658` PASS**. Cada linha abaixo tem comando e saída; nenhuma pendência foi encerrada por "código aparentemente correto".
  - **VERIFHORARIO1 (P3) — premissa do item caducou para o `CLAUDE.md`:** medido em 11/09, `CLAUDE.md:272` diz `| VIXRadar-Verificacao-Async | ... | Diário **11h03 e 19h15** BRT |`, que é o que produção usa, enquanto `routines/README.md` mantém `11h00 e 18h45`. O item não fecha: falta decidir a régua e sincronizar o README. Não foi possível remedir o scheduler nesta sessão, `Get-ScheduledTask` devolveu `Acesso negado`.
  - **BRASKEMDETECT1/Braskem — entrada antiga desatualizada, história preservada:** o bloco de 24/08 mais abaixo ainda diz "DEPLOY PENDENTE ... v4.9.227", mas o deploy aconteceu: `git log --all -S'v4.9.227' -- api/wrangler.toml` devolve `af2abb19 chore(worker): deploy v4.9.227 em producao`, e a produção declarada no topo deste arquivo é v4.9.244. Leitura correta hoje: resolvido e deployado. O bloco antigo fica como registro daquela data.
  - **TOKENCHAT1 (P1, ação do operador) — enunciado corrigido:** o item afirma que o substituto "não foi instalado na variável". Medido em 11/09 sem imprimir valor nenhum: `[Environment]::GetEnvironmentVariable('CLOUDFLARE_API_TOKEN','User')` existe; no processo desta sessão não está presente. Isso não prova qual token está lá, e o único teste real é rodar `deploy-pages.ps1` (Pages:Edit). Item segue aberto com o operador.
  - **Recuperação dos 101 (CURADORIA1 Marco 2, P2) — dois números diferentes no mesmo estado:** `ESTADO.md` registra 400→396 cards e `PENDENCIAS.md` registra 400→397. Nenhum dos dois foi remedido nesta sessão. Não fechar sem rodar `scripts/check-metricas-curadas.mjs` e colar a saída.
  - **CLAUDE-FREE Fase A, G1/G3 com evidência, G2 ainda sem medição:** `logs/monitor-tasks/motor.json` = `{"motor":"task-scheduler","desde":"2026-09-05T04:31:08Z","snapshot":"cutover-snapshot_20260905_013058.json"}` e `VIXRADAR_LLM_PROVIDER` (User) = `openrouter`, ou seja G3 executado e G1 superado pela Fase B D1 (não é `none`). G2 (desligar tasks CCD/Remote) não foi confirmado porque o scheduler está inacessível nesta sessão.
  - **FALLBACKTTL1 (P2) — "fechado no repositório" está errado:** `git status --short -- api/test/fallback-ttl.test.mjs` devolve ` A` (intent-to-add) e `git log --oneline -- api/test/fallback-ttl.test.mjs` devolve 0 linhas. O teste existe e passa (`Tests 2 passed`), mas está fechado no working tree, não no repositório.
  - **Tarefas VIX `Disabled` × `Enabled`:** o bloco de 05/09 em `PENDENCIAS.md` ("continuam `Disabled`") é registro daquela data; `CLAUDE.md` e este arquivo descrevem o estado vivo (5 tasks `Enabled` no Task Scheduler nativo, `Verificacao-Async` reabilitada em 06/09). Ler o vivo pelo `CLAUDE.md`, o histórico pelo bloco datado.
  - **Case F — código já corrigido, falta só a execução:** `scripts/test-locks-overlap.ps1:379` usa `('-File', ('"' + $Runner + '"'), ...)`, o caso está atrás de `if ($ComTokens)` (linha 372) e a linha 415 imprime `CASO F - pulado (rode com -ComTokens; custa um lote LIGHT de 1 emissor)`. Fecha rodando com `-ComTokens`, não por leitura.
  - **Frente vermelha medida e não apurada — `frescor-check.yml` reprovou 3 dos últimos 8 runs:** `34194909759` (08/09), `34319869392` (09/09) e `34570605397` (11/09 06:35 UTC). O mais recente: `Feed sem evento novo ha 3 dias uteis (limite 2), mais novo em 2026-09-08, 90 eventos no estado. avanco_feed=saudavel_sem_fato_novo`; o de 09/09 trazia `avanco_feed=pipeline_nao_persistiu`. O topo deste arquivo declara `feed_fresco:true` medido em 11/09, em outro horário e por outro campo. Não é contradição provada, é pendência sem dono: ninguém apurou se o feed parou de receber evento datado ou se a régua de dias úteis do workflow está errada. Nada corrigido nesta sessão.

- **11/09, FALLBACKTTL1 e EMAILSILENT1 residual — lacunas de teste FECHADAS no repositório, sem deploy; SCANFALLBACK-MORTO1 segue NÃO encerrado:**
  - **FALLBACKTTL1 (P2):** o fix do cache de último recurso (TTL 72h na escrita, corte 48h na leitura, v4.9.225) não tinha teste. Fechado por `api/test/fallback-ttl.test.mjs`, prova de duas pontas pelo caminho real do Worker (`consulta_empresa` → `buscarCacheUltimoResorte`): cache de 36h é servido (`200`, `_de_cache:true`, idade 35–37, score >= 0.3) e cache de 60h **não** é servido (`503`, com prova prévia de que a chave existe no KV). Medido: `Test Files 1 passed (1)`, `Tests 2 passed (2)`.
  - **EMAILSILENT1 residual:** os dois envios de e-mail ao admin dentro de `handleRegistrar` (`api/src/worker.js`) foram migrados de `enviarResend` cru para `enviarEmailRastreado`: reenvio pendente (+24h) com dedup `cadastro:notif_reenvio:*` gravado **apenas** quando `ok === true` (mantendo nova tentativa aberta em falha ou ausência de chave); cadastro novo tratando explicitamente `ok === false` (telemetria e log) sem depender de `catch`; ambos usando `env.ADMIN_EMAIL || ADMIN_EMAIL` e preservando a resposta HTTP 200 de anti-enumeração. Fechado por `api/test/email-registro-rastro.test.mjs`: 6/6 testes passando (ponta boa, ponta ruim de provedor 422, RESEND_API_KEY ausente, lote misto, reenvio com sucesso gravando dedup, e reenvio com falha mantendo dedup limpo). **FECHADO no repositório em 11/09: commit `dd37c8e` (2 arquivos, +284/−16), CI `Worker Tests` run `34646171658` completed/success, push `112ffbe..dd37c8e`, `HEAD == origin/main`. Sem deploy.**
  - **SCANFALLBACK-MORTO1 continua aberto por falta de execução real:** o pré-check do workflow e o secret existem (`ab2622f` é ancestral de `HEAD`; `ANTHROPIC_API_KEY` no repo desde 29/08), mas a condição de obsolescência declarada não é "nunca rodou" e sim "nunca completou". **Correção medida em 11/09 (`gh run list`/`gh run view`, só leitura, sem disparar nada):** o workflow roda por `schedule` todo dia por volta de 01:20 UTC, e o gate já armou. Run `34176257297` (08/09 01:19 UTC): o passo `Scan de emergencia` **executou** e falhou, `X Fallback incompleto: 19 emissor(es) falharam.` com `! [falha] undefined: Worker dados_para_analise: HTTP 400: {"ok":false,"erro":"empresa e setor obrigatorios."}`. Nos runs seguintes o passo aparece pulado (`- Scan de emergencia`), ou seja gate em `prosseguir=false` por estado fresco: `34424970595` (10/09) e `34550366477` (11/09). A falha anterior, `33233411974` (29/08 04:16 UTC), é de outra causa e anterior ao secret: `ANTHROPIC_API_KEY ausente nos secrets do repo` (secret criado depois, 29/08 18:09 UTC). **Causa raiz do 08/09, por leitura de código:** `scripts/scan-emergencia.mjs:207-208` lê `emissor.nome` e `emissor.setor`, mas `listar_emissores_prioritarios` (`api/src/worker.js:21041-21045`) devolve os itens de `selecionarEmissoresPrioritarios` (`api/src/worker.js:11372`) no formato `{empresa, ews_score, materialidade_max, eco_count, staleness_boost, score_combinado}`, sem `nome` e sem `setor`, e `dados_para_analise` (`api/src/worker.js:21059-21062`) exige os dois. O paraquedas está **armado e quebrado**, não "não exercitado". Segue aberto e nada neste relatório o encerra: o critério de saída é uma execução real com `prosseguir=true` que chegue a `Processados: 19/19` sem `::error::Fallback incompleto`. Correção exige mexer no script ou no contrato do endpoint mais uma nova execução; não feita nesta sessão. **Comando do operador para obter a prova real:** `gh workflow run scan-emergencia.yml --ref main` e depois `gh run watch`, ou esperar o `schedule` e conferir `gh run view <id>`; na tela do run, o passo `Scan de emergencia` tem que aparecer executado, não pulado. **Observação para decisão do operador, não corrigida aqui:** o script chama `api.anthropic.com` com `ANTHROPIC_API_KEY` (`scripts/scan-emergencia.mjs:31-40`, `135-150`), e o `CLAUDE.md` registra `ANTHROPIC_API_PAYG = NÃO AUTORIZADO`. O fallback de emergência é, hoje, o último caminho que ainda gastaria Anthropic pago.
  - **Ressalva de escopo da validação — os testes de 11/09 não rodaram contra árvore isolada:** a suíte citada acima (`Test Files 34 passed (34)`, `Tests 339 passed (339)`) exercitou o **working tree completo**, não o índice. O `api/src/worker.js` que rodou ainda carregava a mudança de QUARENTENACOB1 (não staged), e a suíte também pegou arquivos intent-to-add/untracked (`api/test/fallback-ttl.test.mjs`, `api/test/reprovado-failclosed-quarentena.test.mjs`, `scripts/test-monitor-degradado402.ps1`). O teste focado do EMAILSILENT1 residual (6/6) rodou pelo mesmo caminho, contra o mesmo working tree. Não existe, nesta data, medição de que a entrega isolada passa sozinha: `git diff --cached` prova o que o commit levaria, não o que foi testado. Isolar exigiria worktree separado ou stash/checkout, que não foram executados. Portanto: **entrega não validada isoladamente**, só validada no conjunto do working tree.
  - **Índice de 11/09, commitado em `dd37c8e`, sem deploy:** staged exatamente `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs` (`git diff --cached --name-status`, 2 linhas), `git diff --cached --check` sem saída e exit 0, `HEAD = 382e8c1`. QUARENTENACOB1, FALLBACKTTL1, o teste do monitor 402 e os outros 23 arquivos unstaged/112 untracked ficam fora da entrega; os 2 arquivos do índice foram commitados depois em `dd37c8e`, CI `Worker Tests` run `34646171658` PASS.
- **11/09 (continuação, correção SCANFALLBACK-MORTO1 + EMAILSILENT1 residual; diagnóstico 396×397 e frescor-check):** working tree apenas, sem deploy nesta rodada, sem `git add`/`reset`/`checkout`/stash. O índice seguia com `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs` (2 linhas) e as alterações desta rodada ficaram unstaged; esses 2 arquivos foram commitados depois em `dd37c8e`.
  - **SCANFALLBACK-MORTO1 (segue ABERTO):** o contrato foi corrigido no working tree: `listar_emissores_prioritarios` agora aceita somente `setor` presente em `SETOR_DE_EMPRESA` (sem `|| "Outros"`); item sem setor canônico não é enviado e sai em `sem_setor`, e `scripts/scan-emergencia.mjs` passou a consumir `emissor.empresa` + `emissor.setor`, contando ausências como falha visível. Não encerrar: o critério de saída continua sendo uma execução real com `prosseguir=true` chegando a `Processados: 19/19` sem `::error::Fallback incompleto`.
  - **Prova determinística:** `node scripts/test-scanfallback-email.mjs` roda o handler real (`worker.default.fetch`) com bindings em memória e reproduz o 400 antigo nos dois payloads quebrados (`empresa: emissor.nome` e `setor: ""`), provando o payload válido (`empresa` + `setor` canônicos) com HTTP 200. Também foi adicionada a versão de suíte `api/test/scan-emergencia-contrato.test.mjs`.
  - **EMAILSILENT1 residual:** `enviarResend` agora devolve `total`, `enviados`, `falhas`, `ids` e `erros`; o endpoint admin `email_enviar` expõe lote misto com HTTP 207 e não descarta os sucessos. `api/test/email-lote-misto.test.mjs` cobre sucesso, falha e lote misto; no teste direto do handler real os 6 casos passaram, incluindo ausência de setor canônico e falha total de lote.
  - **Ressalva de runner:** a suíte Vitest não pôde rodar neste ambiente porque esbuild/workerd falham com `spawn EPERM`; a prova local é o teste direto acima. Nenhum teste foi declarado verde pelo runner bloqueado.
  - **Diagnóstico 396 × 397 (sem fix):** `node scripts/check-metricas-curadas.mjs` mediu `Carteira (EMISSORES_LISTA): 104 | Com card (METRICAS_CURADAS): 104 | No menu (EMISSORES): 104`, `Pendencia declarada: 396 card(s) sem as_of/source_date/metric_type`, `OK` e zero falhas. Logo 396/397 não são cobertura faltante: o número atual da guarda é 396 cards sem trio; 397 não tem lastro na medição de 11/09.
  - **Diagnóstico frescor-check (sem fix):** a leitura do workflow `.github/workflows/frescor-check.yml:71-105` confirma que o gate que reprova é `ev_idade > ev_limite` (evento mais novo em dias úteis); `avanco_feed` é diagnóstico e não decide. O run `34570605397` publicado em `ESTADO.md` bate com `3 DU` contra `limite 2`; o de `34319869392` traz `pipeline_nao_persistiu` no diagnóstico. Não houve consulta live (`api.vixradar.com` falhou SSL/offline neste ambiente) e nenhum workflow foi executado.

- **11/09 (segunda continuação, revisão objetiva): frescor-check apurado, 396×397 fechado, FALLBACKTTL1 e EMAILSILENT1 residual fechados por teste direto, demais pendências revisadas:** sem deploy/`git add` nesta rodada; o índice seguia com `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs`, commitados depois em `dd37c8e`.
  - **frescor-check — causa do vermelho mais recente (`34570605397`, 11/09 06:35 UTC):** job `103171682021`; `companies=104`, `updated_at=2026-09-10T23:42:40.811Z`; `evento_mais_novo.data=2026-09-08`, `idade_du=3`, `limite_du=2`, `total_eventos=90`, `avanco_feed=saudavel_sem_fato_novo`. O run de 10/09 (`34445952403`, job `102770668111`) passou com o mesmo evento e `idade_du=2`, `feed_max=2026-09-08`, `teto_elegivel=2026-09-05`, `max_data_entrega=2026-09-05`, `dentro_da_cadencia=true`, `escreveu_apos_lote=true`, `proxima_prevista=2026-09-13`. Nos runs de 08/09 (`34194909759`) e 09/09 (`34319869392`) o diagnóstico era `pipeline_nao_persistiu`; o feed voltou a 09-08 até 10/09. Conclusão: não é dado inconsistente nem escritor parado; o run mais recente é a régua de 2 DU disparando durante ausência real de evento novo, com a fonte dentro da cadência. Corrigir a régua mudaria a semântica que o AVANCOFEED1 protege; sem decisão de produto e teste determinístico, fica ABERTO.
  - **396 × 397 — divergência fechada por medição atual:** `node scripts/check-metricas-curadas.mjs --json` devolve `carteira=104`, `com_card=104`, `no_menu=104`, `cards_sem_datacao=396`, `falhas=[]`. Portanto 396 é a contagem corrente de cards sem trio; 397 não se reproduz em 11/09. Nenhum dado foi alterado para coincidir.
  - **FALLBACKTTL1 — fechado por teste direto atual:** probe no handler real, com KV em memória e cascade forçado a falhar: cache de 36h → HTTP 200, `_de_cache:true`, `_cache_idade_horas=36`, `_score_confianca=0.3`; cache de 60h presente no KV → HTTP 503 `Servico de analise temporariamente indisponivel`. A suíte Vitest continua bloqueada por `spawn EPERM`, mas a prova direta cobre as duas pontas.
  - **EMAILSILENT1 residual — fechado por teste direto atual:** `node scripts/test-scanfallback-email.mjs` passou 6/6, incluindo sucesso total, falha unidade, lote misto 207, falha total 500 sem 207 parcial e rejeição de emissor sem setor canônico. A suíte Vitest não rodou neste ambiente; não foi declarada verde.
  - **Pendências abertas confirmadas:** `SCANFALLBACK-MORTO1` aberto até execução real autorizada (`prosseguir=true`, `Processados: 19/19`); `QUARENTENACOB1` segue código não validado por runner; `VERIFHORARIO1` com `Get-ScheduledTask` retornando `Acesso negado`; `Case F` exige `-ComTokens`; `DRIVERMORTO1`, `CURADORIA1`, `PISODIFF1`, `ROUTINEKEY-PLAIN1`, `TOKENCHAT1` e `CCDOFFLINE1` permanecem sem fechamento nesta rodada.
  - **Gates desta rodada:** Vitest focado falhou com `spawn EPERM` (não executado); `node --check` de `worker.js`, `scan-emergencia.mjs`, `test-scanfallback-email.mjs` e testes Vitest = 0; `git diff --check` e `git diff --cached --check` = 0; `scripts/lint-encoding.ps1` = 112 .ps1 + 5 shell OK, RISCO 0; `scripts/check-claude-free.ps1` exit 0.

- **06/09 (noite), P4 ABERTO (OPERADOR) — Frontend-QA/D2 fechados; falta aprovação estética:** capturas BEFORE×AFTER (públicas e internas, desktop+mobile) em `app/tests/.local/compare/` (não versionadas). Até aprovar, não evoluir a camada visual nem tocar nos baselines (`axe-baseline.json` + PNGs de `__screenshots__`). Reversão disponível: `git revert 10e8e18 d44f37b 01992d6 f38e6f5`.
- **05\09 (noite), RESOLVIDO 06/09 (MERGEDUP1 + task reabilitada):** no dia 05/09 a `VIXRadar-Verificacao-Async` foi parada (Disabled 18:47:19) porque os 2 eventos da Usina Pampa Sul estavam com `_pendente_verificacao=true`, armando os dois desfechos destrutivos do dreno: no ramo REPROVADO, `retratarEventoRejeitadoInterno` (`worker.js:10010-10013`) **apagaria** o evento por causa da flag; no ramo CORRIGIR, `aplicarCorrecaoVerificador` (`worker.js:12827`) reescreve `fonte_primaria`, a chave dedup muda, e `mesclarEventoVerificadoInterno` (`worker.js:9963-9970`) casava por chave recalculada em vez do `id` da fila e **empurraria duplicata**. Resolvido em produção desde v4.9.241 (MERGEDUP1, casar por `chaveOriginal`/`it.id` e abortar sem gravar quando não casar), com REPROVADO-FAILCLOSED1 (v4.9.242) e LIVENESS1/SWEEP-ORFAOS1 (v4.9.243) cobrindo retry e órfãos. A task foi **reabilitada em 06/09** por decisão do operador (`Enable-ScheduledTask`, State Ready, triggers intactos, próximo disparo 19:15). Detalhe em `PENDENCIAS.md`
- **05\09 (noite), RESOLVIDO nos dados (SOURCEFIX-PAMPASUL1, sem deploy):** os 2 eventos da Usina Pampa Sul em `radar:estado:2026-W36` tinham `fonte_primaria` errada (ITR apontava para o PDF do 1T26 sendo evento do 2T26; ANEEL apontava para imprensa). Corrigido por escrita direta no KV, porque nenhum endpoint administrativo faz isso sem duplicar (`admin_upsert_analise` funde por chave dedup que inclui a própria fonte; `admin_kv_put` tem allowlist que exclui `radar:estado:`). ITR passou a apontar para o documento oficial **160616** do RAD CVM, com os números conferidos contra o dataset aberto da CVM; ANEEL ficou com `fonte_primaria=null`, MegaWhat rebaixada a `fonte_secundaria` e `_fonte_oficial_confirmada=false`, por não existir fonte oficial localizável. Verificado por releitura independente: 104 emissores, 2 eventos (sem duplicata), fila consistente, **0 empresas alteradas fora da Pampa Sul**. Backup em `%TEMP%\pampasul-fix-20260905-183644\backup`. Detalhe em `PENDENCIAS.md`
- **05\09 (noite), P3 ABERTO (VERIFHORARIO1):** o Task Scheduler tem `VIXRadar-Verificacao-Async` em **11:03 e 19:15** (medido em `StartBoundary`), enquanto `CLAUDE.md` e `routines/README.md` documentam 11h00 e 18h45. Produção vence documentação, então o horário real é 11:03/19:15. Não corrigido nesta sessão; falta decisão do operador sobre qual dos dois é o desejado. Detalhe em `PENDENCIAS.md`
- **05\09 (noite), ACHADO OPERACIONAL (WRANGLERMOJIBAKE1):** `npx wrangler kv key get ... > arquivo` corrompe todo caractere acentuado quando o console não está em UTF-8. 15 dos 104 emissores tiveram o nome destruído na captura (`Assaí` virou `Assa├¡`), inflando o arquivo em ~22 KB sem que nada tivesse mudado no KV. Isso fez uma guarda de hash abortar duas vezes por falso positivo e simulou um escritor concorrente que não existia (`updated_at` era idêntico ao milissegundo nas duas leituras). É o 5º caminho de mojibake em PowerShell registrado no projeto. Mitigação: forçar `[Console]::OutputEncoding` e `$OutputEncoding` antes de chamada nativa, e comparar hash canônico (leitura `-Encoding utf8` + `TrimEnd()`) em vez de bytes crus. Detalhe em `PENDENCIAS.md`
...
- ABERTO 24/08 (EMAILSILENT1 resíduo, P3): os dois envios de notificação ao admin em `handleRegistrar` (cadastro novo e reenvio com dedup de 24h) seguem fora do helper. Não são silenciosos, têm `console.log` mais evento de Analytics Engine, mas ficam sem rastro por destinatário e sem alerta na Sentry. Segundo item, `enviarResend` devolve o objeto único quando sobra 1 resultado, então lote de 2 com 1 falha retorna forma indistinguível de envio único bem-sucedido, o que engana quem for instrumentar os call sites de lote. **Adendo 11/09, sem apagar o acima:** a primeira metade está **fechada no repositório** em `dd37c8e` (CI `Worker Tests` run `34646171658` PASS, `HEAD == origin/main`), sem deploy, ver bloco 11/09 em `## Itens abertos`. A segunda metade continua aberta e medida no código: `api/src/worker.js:5989-5997` engole a falha por destinatário quando `destArray.length > 1` (só `console.error`), e o retorno `{ batch: true, enviados: N, ids: [...] }` conta só os sucessos, então lote de 2 com 1 falha sai com `enviados: 1` sem campo que denuncie o destinatário perdido. `enviarEmailRastreado` não é afetado porque sempre envia para um destinatário só (`[dest]`); o risco é dos call sites de lote.
...
- RESOLVIDO 31/08 (MANIFESTOFRAGIL1, P3): o `status/allclear-manifesto.json` indexava cada frase de ausência junto com o HTML e o estilo inline, então trocar a cor de um texto fazia a mesma frase reprovar como NOVA. `check-frontend-allclear.mjs` ganhou `textoVisivel()` (remove tag antes de normalizar a chave), manifesto migrado sem perda (57→57, zero colisão, 1 frase nova classificada `escopo_registro`). Prova: `CHECK_ALLCLEAR_OK`. Achado no caminho: a correção já tinha sido feita e commitada numa passada anterior (PR #20, aberto 22/08), mas o commit sumiu da branch `claude/vixradar-monitoring-audit-k7i0bv` (não resolve mais como objeto git), provável fast-forward/reset de outra sessão diária reaproveitando o mesmo nome de branch — refeita do zero sobre o `main` atual. Detalhe em `PENDENCIAS.md`
- RESOLVIDO 31/08 (FRESCORHARNESS-DRIFT2, P3): `scripts/test-frescor-cvm.mjs` quebrado de novo (`ReferenceError: _cvmDiasCorridosApos is not defined`), segunda vez (primeira foi CVMCADENCIA1 em 20/08). O CVMDURA1 (24/08) acrescentou `falha_dura`/`degrada_servico` a `avaliarFrescorCVM` e o harness não tinha acompanhado; o conserto do SACFALSA-RESIDUO em 24/08 só tocou o comentário de cabeçalho, não a extração. Corrigido: 2 funções e 2 constantes novas na extração/prelúdio, 3 casos novos. Prova: `TUDO VERDE em 43 casos.` Detalhe em `PENDENCIAS.md`
