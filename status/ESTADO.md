...
Última atualização: 2026-09-11. Medido ao vivo: Worker v4.9.244, frontend v202.43, repo e produção na mesma versão (o `app/index.html` do repo bate por hash com o do `deploy_zip`), `ok:true`, `feed_fresco:true`, `painel_fresco:true`. **EMAILSILENT1 residual FECHADO no repositório: commit `dd37c8e` (2 arquivos, +284/−16), CI `Worker Tests` run `34646171658` completed/success, push `112ffbe..dd37c8e`, `HEAD == origin/main`, sem deploy.** **FALLBACKTTL1 segue com o teste só no working tree (não commitado).** **BUSCADEGRADADA1 fechado/PASS em `main` (`990dbb0`), sem deploy e sem backfill de 09/09.** **`VIXRadar-Verificacao-Async` reabilitada** em 06/09 por decisão do operador, com MERGEDUP1 v4.9.241, REPROVADO-FAILCLOSED1 v4.9.242 e LIVENESS1/SWEEP-ORFAOS1 v4.9.243 em produção. **D2 Fases 1–2 estão em produção desde o v202.43**, com o bump de `CACHE_VERSION` que o achado P2 da auditoria de 08/09 apontava como faltante. **Cobertura de 09/09 fechada em 104/104** (65 `ANALISADO` + 39 `SKIP`, zero duplicata), provada por leitura de ledger, sem rotina executada, sem deploy e sem mudança de scheduler. **Sessão de 10/09 (tarde): Actions @v4→@v6 (7 workflows), Case F e drift Pampa Sul resolvidos — commits `8b26d74` + `112ffbe` pushados, CI 100% verde (4/4 push-triggered), sem deploy.** Blocos abaixo.
