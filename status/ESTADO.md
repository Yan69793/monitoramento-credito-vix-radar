...
- ABERTO 25/08 (SUBSTRINGDONO1, P1, corrigido no repo e **não deployado**): documento da CVM ia para o emissor errado por casamento de substring sem fronteira de palavra e sem desempate. Medido em produção: dos 28 documentos que o plano entregava para a Oi, 4 eram dela, o resto era Três Tentos, Saneamento de Goiás, Sequoia, Ecoponte e Equatorial Goiás, casando dentro de SEQU(OI)A, AGR(OI)NDUSTRIAL, G(OI)AS e NITER(ÓI). A CSN recebia os 6 documentos da CSN Mineração, que é outro emissor. Pior que isso, CSN e Copasa nunca tiveram alias e a CVM as registra como "CIA SIDERURGICA NACIONAL" e "COMPANHIA DE SANEAMENTO DE MINAS GERAIS", então documento nenhum delas entrava no KV: a CSN foi rebaixada pela Fitch em 31/07, o relatório entrou na CVM em 05/08, e a rotina caiu para imprensa com a fonte primária disponível. No caminho apareceram mais 5 emissores cegos (Dasa, Natura, Vivo, TIM, Taesa) por divergência entre a tabela de ingestão e a de atribuição. Correção: árbitro único `_donoDocumentoCVM`, âncora no início de palavra, termo mais longo vence, 4 call sites unificados, `SYNC_ALIAS_NOMES_CVM` aposentada. Guardas com prova de 2 pontas: `api/test/cvm-atribuicao.test.mjs` (17 testes, 7 falham contra o código antigo) e checagem nova de razão social própria em `scripts/check-emissores-cadastro.mjs`. Suíte 93 testes, 13 arquivos. **Falta deployar** (`deploy-worker.ps1 -Version v4.9.215`). Detalhe em `Obsidian VIX Radar/PENDENCIAS.md`
