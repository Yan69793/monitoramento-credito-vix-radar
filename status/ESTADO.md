...
Última atualização: 2026-09-06 (Worker v4.9.243, frontend v202.42. **SWEEP-ORFAOS1/LIVENESS1 fechado em produção**: convergência de órfãos da fila de verificação por prova de escrita terminal, deploy `df95d05`; MERGEDUP1 segue em produção desde v4.9.241. Blocos abaixo)
...
- **05\09 (noite), P1 ABERTO + AÇÃO DO OPERADOR (MERGEDUP1 + task parada):** a `VIXRadar-Verificacao-Async` está **`Disabled` desde 18:47:19**, parada preventivamente 28 min antes do disparo das 19:15, com autorização explícita do operador. Motivo medido: os 2 eventos da Usina Pampa Sul estão com `_pendente_verificacao=true`, e isso arma os dois desfechos destrutivos do dreno. No ramo REPROVADO, `retratarEventoRejeitadoInterno` (`worker.js:10010-10013`) **apaga** o evento justamente por causa dessa flag. No ramo CORRIGIR, `aplicarCorrecaoVerificador` (`worker.js:12827`) reescreve `fonte_primaria`, a chave dedup muda, e `mesclarEventoVerificadoInterno` (`worker.js:9963-9970`) casa por chave recalculada em vez do `id` da fila, não acha nada e **empurra duplicata**. O call site (`worker.js:20037-20052`) tem o `it.id` e não passa adiante. Bug pré-existente, não introduzido pela correção de hoje. Patch mínimo fail-closed preparado e **não aplicado**, aguardando autorização. Reabilitar a task só depois do fix: `Enable-ScheduledTask -TaskName "VIXRadar-Verificacao-Async"`. Detalhe em `PENDENCIAS.md`
