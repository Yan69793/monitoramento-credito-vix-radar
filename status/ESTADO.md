...
Última atualização: 2026-08-25 (agente: Claude Opus 5)
...
- **ABERTO 25/08 (INVERSAO-CD1, P1, uma ação do operador, no app):** as três sessões agendadas do Claude Desktop ainda estão nos horários antigos. Precisam ir para `vixradar-noturno` (varredura completa dos 103) às **10h00** diário, `vixradar-matinal` (top 15) às **18h00** Seg-Sex, e `vixradar-verificacao-async` em **duas** sessões, 11h00 e 18h45. Apagar o agendamento velho antes de criar o novo, senão a rotina dispara duas vezes. Não há arquivo de agendamento no repo nem em `%APPDATA%\Claude`, é ação de GUI. **A parte automatizável já foi feita:** os dois vigias de retry trocaram de horário (`Szuchmacher-RetryVixNoturno` diário 13h30, `Szuchmacher-RetryVixMatinal` Seg-Sex 21h30). Não há janela de risco entre uma coisa e outra, porque `scripts/retry-vixradar.ps1:36-39` sai sem relançar quando o log do dia não existe. Enquanto os horários novos não entrarem, a varredura completa fica sem vigia, o que é perda de cobertura e não relançamento à toa. Motivo, evidência e tabelas em `routines/README.md`
- **ABERTO 25/08 (SENTINELA-SYNC1, P2, decisão do operador):** a rotina Sentinela promete latência de até uma hora entre o **Worker ingerir** um documento da CVM e a análise sair. Ela **não** cobre o trecho entre a CVM **publicar** e o Worker ingerir, que segue preso aos crons das 12h30 e 18h30 BRT. Medido em 25/08: a CVM republicou o arquivo às **07h58** e o Worker só ingeriu às **12h30**, 4h32 de atraso estrutural com todo semáforo verde. Fechar essa ponta exige uma ação de sincronização autenticada por `ROUTINE_API_KEY`, que hoje não existe: `admin_sync_cvm_auto` e `sync_cvm` pedem `admin_senha`, e dar a senha de admin a uma rotina contraria o CHAVEESCOPO1. Não improvisei arquitetura para isso. A Sentinela faz `HEAD` no zip a cada execução e registra no log quando o arquivo está à frente do que o Worker ingeriu, para a decisão nascer de dado medido
- **ABERTO 25/08 (DEFERGRUDA2, P1, bloqueia a Sentinela):** a correção do DEFERGRUDA1 fecha para uns emissores e não para outros, e por isso **`VIXRadar-Sentinela` está `Disabled`**. Medido em quatro execuções contra produção já em v4.9.217: Copel, Neoenergia, CPFL e Comerc limparam; Vibra, Compass, Iguá e PRIO limparam; VLI, Embraer, Nexa Resources e Even Construtora **não limpam nem depois de duas análises com `submit_ok`**, seguem `deferido=True` com `horas_stale=0,1`. Candidatos saíram de 34 para 33 e travaram. Não é propagação, quatro minutos e meio sem escrita nova e o quadro não mudou. Hipótese não confirmada: `montarPlanoRotina` lê com `carregarEstadoMultiSemana(env, 3)`, três semanas mescladas, e a escrita vai só para a semana corrente, então a bandeira pode estar viva num registro de semana anterior (família STATELEAK1). Para confirmar, ler o registro cru de VLI nas 3 chaves de semana. Com 4 das 8 vagas presas e backlog ligado, a rotina entraria em toda tentativa, 16 vezes por dia a ~70k tokens, mais de 1 milhão de tokens diários em trabalho repetido. Reabilitar só depois de fechar: `Enable-ScheduledTask -TaskName "VIXRadar-Sentinela"`
...
- **ABERTO 25/08 (SENTINELA-HANG1, P2):** lote da Sentinela pode ficar pendurado esperando rede. Duas execuções ficaram 20+ min no segundo lote com o processo vivo e **1,5s de CPU acumulado**, provável limite de taxa da assinatura (a noturna rodou às 19h30 e dois runs saíram em 15 min). Mitigação já no ar: teto de relógio de 22 min por execução, conferido antes de cada lote, mais o `ExecutionTimeLimit` de 40 min da task como fundo de poço. Falta timeout no próprio `claude -p`, que exigiria trocar o pipeline por `System.Diagnostics.Process` com `WaitForExit`. A noturna tem o mesmo desenho e nunca precisou, mas roda 1 vez por dia contra 16 da Sentinela
