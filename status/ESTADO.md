...
Última atualização: 2026-09-03 (INCIDENTE-FRESHNESS2 corrigido no código e testado; deploy do v4.9.236 ainda PENDENTE de confirmação em produção; recuperação operacional das rotinas já confirmada por medição. Ver bloco abaixo)

> [!warning] 03/09 — INCIDENTE-FRESHNESS2 (v4.9.236): painel parado 12h41 em 11:09 BRT com `ok:true`. Causa nas ROTINAS, não no dado.
> **Status:** código e testes fechados (241 vitest + 95 PowerShell); recuperação operacional das rotinas CONFIRMADA por medição; deploy do Worker v4.9.236 ainda NÃO confirmado em produção (portão de verificação às 13:57Z de 03/09 respondeu `versao:"v4.9.235"`, sem os campos `painel_*` — ver linha "Deploy" logo abaixo). 3 resíduos declarados · **Data da Versão:** 2026-09-03 · **Origem do Registro:** evidência visual do operador (02/09 ~23:50 BRT, aba anônima) + medição ao vivo (DOM, `op=state`, headers, Cloudflare Observability, transcripts das sessões agendadas, `%TEMP%\wsprobe_err_*`) · **Condição de Obsolescência:** cai quando o Worker passar do v4.9.236 ou quando a agenda das rotinas mudar (o SLA de `painel_fresco` deriva dela).
>
> **Deploy.** Múltiplas tentativas de `deploy-worker.ps1` nesta sessão travaram no passo `npm ci` (hang local, não erro de código; workaround = matar o processo e repetir `npm ci` isolado). Bundle `api/v4.9.236.js` já foi gerado localmente (untracked) por uma tentativa anterior, mas produção não avançou de versão. Não declarar o incidente fechado até o portão de verificação responder `versao:"v4.9.236"` com `ok:true`.
>
...
>
> **Provas de teste.** 241 testes vitest em 26 arquivos (+8 de `painel_fresco`, incluindo o instante exato do incidente e os dois cenários de feriado B3 do FERIADOB3-PAINEL1) e 95 asserts em 4 suítes PowerShell, todos verdes na última rodada desta sessão. Duas pontas com o CLI REAL: linha nova passa a sonda headless; linha antiga reprova por falta de `PowerShell/Bash, Read, Write, Edit, Agent(Task)`, mesmo defeito de 02/09. Prova de recuperação operacional: ver bloco "Recuperação operacional" acima. Prova de produção (portão de verificação com `versao:"v4.9.236"`): ainda pendente, ver bloco "Deploy" acima.
>
> **Resíduos declarados** (detalhe em `PENDENCIAS.md`): o laço de retry da invocação principal ainda não é 429-aware (só o pré-flight é); a virada de meia-noite ainda parte o ledger entre dois arquivos; `monitor-tasks.ps1` ainda não usa a função de janela compartilhada.
