...
empurrar os achados verificáveis ao Worker.
...
1. `POST {action:"listar_emissores_prioritarios", top_n:15, routine_key}` → lista de
   `{nome, setor, ...}`. Esse é o universo de hoje.
2. Para CADA emissor (pode paralelizar em grupos de ~3):
   ...
   c. `POST {action:"receber_analise", empresa, setor, resultado, _matinal:true, routine_key}`.
      `_matinal:true` marca o provedor como `claude-opus-routine`. Confirme a
      resposta `{ok:true, n_eventos, sem_eventos}`.
3. Ao final, reporte um resumo: emissores processados, total `n_eventos`
   persistidos, destaques CRÍTICO/RELEVANTE.

O Worker aplica gate de verdade graduada (verificação adversarial + checagem real
de data/fonte). Eventos com fonte fraca ou fora da janela são descartados — isso é
esperado. **Não** force entrada nem invente URL para passar no gate.
