  ...
  if (payload.sem_eventos) {
    ...
    if (!_coberturaCompleta) {
      ...
      if (anterior && !anterior.sem_eventos && Array.isArray(anterior.eventos) && anterior.eventos.length > 0) {
        ...
        // FIX(FIN1): nao resetar _last_scanned_at no INCONCLUSIVO — preservar timestamp anterior
        // para que o staleness clock continue acumulando e eventualmente promova o emissor a FULL.
        // anterior._last_scanned_at = agora;  // REMOVIDO — impedia promocao por staleness
      ...
      if (anterior && anterior.sem_eventos && anterior._status !== "INCONCLUSIVO") {
        ...
        // FIX(FIN1): mesma protecao — nao resetar _last_scanned_at em preservacao de sem_eventos
        // anterior._last_scanned_at = agora;  // REMOVIDO
        anterior.timestamp = _tsVarredura;
        anterior._ultima_janela_fim = _fimJanela;
        anterior._ultima_janela_inicio = _inicioJanela;
        estado.results[empresa] = anterior;
      ...
      // FIX(FIN1): preservar _last_scanned_at anterior — nao resetar em INCONCLUSIVO
      estadoInc._last_scanned_at = anterior ? (anterior._last_scanned_at || anterior.timestamp || null) : null;
...
async function __coreFetch(request, env2222) {
    ...
    if (body.action === "registrar" || body.action === "login") {
      ...
      for (const it of _cvItens) {
        try {
          ...
          if (_cvAprovado) {
            ...
            await tel(env2222, request, { evento: "verificacao_async_rejeitado", empresa: String(it.empresa).slice(0, 40), motivo: (it.veredicto.motivo || "").slice(0, 128) });
