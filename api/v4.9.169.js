...
async function executarPipelinePreditivo(env2222, opts) {
  ...
  const payload = {
    ...
    model_version: "v4.9.169+lab_interno",
...
async function __coreFetch(request, env2222) {
    ...
    if (request.method === "GET") {
      ...
      if (op === "predictive_v1") {
        // v4.9.169: lab interno apenas. Pipeline continua nos crons (KV + export historico);
        // leitura HTTP restrita a JWT admin. Usuarios do produto nao veem score preditivo.
        const _pvAdm = await _exigeJwtAdmin(request, env2222);
        ...
        if (!pv1) return resp({ ok: false, source: "not_found", lab_interno: true, mensagem: "Pipeline preditivo lab ainda n\xE3o executado." }, 200, request);
        return resp({ ok: true, source: "kv", lab_interno: true, user_facing: false, ...pv1 }, 200, request);
      }
