...
6. **Veracidade da UI (obrigatorio, nao pular):**
   ```powershell
   node "$env:USERPROFILE\.claude\skills\vix-radar-general-audit\scripts\audit-ui-metrics.mjs" "E:\Diretorio\Claude\Monitoramento de Credito\app\index.html"
   ```
...
- **Providers no health (OPENROUTERVIVO, atualizado 2026-07-21):** nao tratar `openrouter`/`perplexity` como residuo de schema por padrao. Ha caminho vivo: `probeOpenRouterSonarPro` / `chamarOpenRouter` no cron e em health. Gemini permanece residuo de schema salvo evidencia nova de uso. Confirmar se `OPENROUTER_API_KEY` existe e se o probe gasta ou contamina sinal de saude.
