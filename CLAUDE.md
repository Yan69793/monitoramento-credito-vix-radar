...
| Verificador adversarial (CRITICO + amostra 20% RELEVANTE) | `claude-sonnet-4-6` via `claude -p` — desde v4.9.146. **Cobrança:** as 3 rotinas **removem `ANTHROPIC_API_KEY` do ambiente do processo filho** (`$env:ANTHROPIC_API_KEY = $null` antes do `claude -p`), forçando **assinatura via OAuth** — a chave User no registro fica neutralizada. Por isso o incidente de 18/07 (OAuth expirado) abortou a rotina. A nota 49 (não migrar para Fable, "custo 2,3x-4,3x") foi argumentada sobre aritmética metered que não é o caminho vivo, reavaliar. Avaliação de troca para `claude-fable-5` feita em 10/07: **não trocado** (sem ganho demonstrado, custo 2,3x-4,3x; critério de reversão na nota 49) | Fila KV `radar:verif_fila:{data}` drenada por 3 gatilhos: Task Scheduler `VIXRadar-Verificacao-Async` (10:20 BRT, trigger único) + dreno inline pós-matinal + dreno inline pós-noturno. Guardas nas 3 rotinas: auth-failure (exit 7 + abort, matinal incluída desde 08/07); dreno também detecta `stop_reason:refusal` (classificador Fable 5 — exit 8, rawout, métrica `refusals`, `--fallback-model` condicional preparado, 10/07). Gap conhecido: `Credit balance is too low` ainda não abortável (P2, incidente 10/07 10h). Ver `03 - Estado Atual.md`, notas 48 e 49. |
...
## Produção atual (fonte: Obsidian `03 - Estado Atual.md`)

| Componente | Produção | Repo local | URL |
|------------|----------|------------|-----|
| Worker | **v4.9.177** | `api/wrangler.toml` → **v4.9.177.js** (sincronizado automaticamente em 24/07 contra o health de producao) | https://api.vixradar.com |
| Frontend | **v201.86** | `app/index.html` → **v201.86** (sincronizado automaticamente em 24/07) | https://vixradar.com |
