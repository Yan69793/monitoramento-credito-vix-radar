...
Fonte única no repo: `.claude/skills/` (consolidado 2026-06-18 — não depender de `~/.claude/skills`).
...
2. **Endpoint de análise exige JWT.** `POST /` → 401 `Autenticação necessária`. O teste padrão anônimo deste CLAUDE.md (esperava HTTP 200) está **SUPERADO**. Validação end-to-end do cascade exige token autenticado.
...
Triggers agora versionados em `worker/wrangler.toml` (`[triggers] crons = [...]`) — defesa em profundidade conforme regra inviolável de Telemetria do CLAUDE.md.
...
- **Vault Obsidian (canônico, interno):** `E:\Diretorio\Claude\Monitoramento de Credito\Obsidian VIX Radar\` — 7 notas: 00 Índice, 01 Jornada, 02 Organograma/Arquitetura, 03 Estado de Produção, 04 Versões do Worker, 05 Consolidação, 06 Inventário. Inclui `_contexto_usuario\yan-master.md`.
- **Histórico de sessões:** `E:\Diretorio\Claude\Monitoramento de Credito\_historico\` (gitignored).
...
- ⚠️ **Caminhos antigos `C:\Users\szuch\...` / `C:\Projetos Claude\...` estão MORTOS** — projeto consolidado em `E:\Diretorio\Claude\Monitoramento de Credito\`.
