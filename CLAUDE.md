...
| NOMEMORTO1 | Emissor renomeado ficava cego por defeito de tabela. A Eletrobras virou AXIA ENERGIA em 10/11/2025, os documentos dela **estavam gravados** em `cvm:documentos`, e `buscarDocumentosCVM("Eletrobras")` devolvia 0 por nove meses, exibido como `sem_eventos`. Havia TRÊS tabelas de alias que precisavam concordar: `SYNC_ALIAS_NOMES_CVM` (deixa entrar), `SYNC_ALIAS_TO_EMPRESA` (atribui) e uma cópia privada dentro do leitor. Mesma família: alias sem acento contra razão social acentuada deixava a Sabesp órfã (ACENTOMATCH1). E `CCR` e `OMEGA` não existem mais no cadastro, viraram MOTIVA e SERENA, sem alias. | v4.9.211: leitor deriva de `SYNC_ALIAS_TO_EMPRESA` (`SYNC_EMPRESA_TO_ALIASES`) em vez de manter cópia; comparação de nome sempre sem acento nas duas pontas; aliases MOTIVA e SERENA; `admin_documentos_cvm` para inspecionar o que o sistema enxerga por emissor. Guarda: `scripts/check-emissores-cadastro.mjs` confere os 103 contra `cad_cia_aberta.csv` toda segunda em `.github/workflows/emissores-cadastro.yml`, com exceções declaradas e prova das duas pontas no CI. Medido: Eletrobras 0→28 docs, Sabesp 0→11. |
| CVMURL404 | Painel sem fato novo depois de 20/08 por 4 dias, com as 3 rotinas rodando, 103 emissores varridos toda noite e todo semáforo verde. Em 23/08 a CVM removeu `ipe_cia_aberta_2026.zip` do servidor (404, listagem só até `2025.zip`, catálogo CKAN ainda anunciando). `cvm:documentos` congelou em `Data_Entrega 15/08`, as rotinas perderam o gatilho primário de evento e passaram a reciclar o mesmo acervo de imprensa. Ninguém viu porque o 404 duro era tratado com a tolerância de 14 dias feita para cadência semanal, o caminho de erro apagava `max_data_entrega` (health devolvia `idade_dias:null`) e o `VIXRadar-Health-Watch`, único canal de alerta da fonte, estava `Disabled` desde 21/08. | v4.9.209 + v4.9.210: ano do ZIP pelo relógio BRT com fallback pelo catálogo CKAN, merge de meta no erro com `falhas_consecutivas`, falha dura separada de cadência e escalando para o `ok` agregado após 4 syncs falhos, TTL de `cvm:documentos` de 14 para 30 dias, alerta nomeando o campo no `frescor-check.yml`. Testes em `api/test/cvm-frescor.test.mjs`. **A CVM ainda não repôs o arquivo**, ingestão de Fato Relevante segue parada até lá. |
| EMAILSILENT1 | Falha de envio de e-mail transacional era invisível por construção. Quatro `catch {}` vazios em volta da Resend (`handleAdminAprovar`, `handleAdminRejeitar`, os dois ramos de `handleEmailActionConfirm`) e um `console.error` solto em `handleSolicitarReset`. Aprovar devolvia `ok:true` tivesse a mensagem saído ou não, e a única fonte autoritativa era o painel da Resend, fora do sistema. `enviarResend` já devolvia o `id` gerado e nenhum dos 16 call sites lia esse retorno. 24/08. | `enviarEmailRastreado` centraliza os 5 envios ao usuário final e nunca lança (a ação primária continua valendo). Quatro canais por papel: `console.error` para tail, Sentry para alerta, Analytics Engine para agregação, KV `email_envio:{email}:{ts}` (TTL 90d, igual bounce) para rastro por destinatário. Respostas ganharam `email_enviado`/`email_erro`/`resend_id`, tri-estado (`null` = não tentou). `solicitar_reset` mantém resposta genérica de propósito, anti-enumeração. Action `admin_email_envios` consulta envios + bounce + complaint. Testes `api/test/email-falha-silenciosa.test.mjs`, prova de 2 pontas medida. |
...
| SUBSTRINGDONO1 | Documento da CVM atribuído ao emissor errado por `includes()` cru, sem fronteira de palavra e sem desempate. Cada emissor perguntava "este documento contém meu nome?", pergunta que N emissores respondem sim ao mesmo tempo. Medido em produção 25/08: dos 28 documentos entregues para a Oi, 4 eram dela, o resto era Sequoia, Três Tentos, Saneamento de Goiás, Equatorial Goiás e Ecoponte, casando dentro de SEQU(OI)A, AGR(OI)NDUSTRIAL, G(OI)AS e NITER(ÓI). E a CSN recebia os 5 documentos da CSN Mineração, que é outro emissor. Somado a isso, CSN e Copasa nunca tiveram alias: a CVM registra "CIA SIDERURGICA NACIONAL" e "COMPANHIA DE SANEAMENTO DE MINAS GERAIS", nomes que não contêm "CSN" nem "COPASA", então documento nenhum delas entrava. A Fitch afirmou o rating da CSN em BB e revisou o outlook de estável para negativo em 04/08 (não foi rebaixamento, e nunca chegou a B ou CCC+, erro medido e corrigido em 30/08 via Exa), o relatório entrou na CVM em 05/08, e a rotina caiu para imprensa com a fonte primária disponível. | Árbitro único `_donoDocumentoCVM`: índice montado das 3 tabelas de uma vez, âncora no início de palavra (o fim pode cair no meio, senão alias prefixo como `SENDAS DISTRIB` quebra) e termo mais longo vence (`CSN MINERACAO` ganha de `CSN`). Os 4 call sites passam a perguntar de quem o documento é, não se contém o próprio nome. `SYNC_ALIAS_NOMES_CVM` aposentada: era 100% redundante e a divergência dela com a tabela de atribuição deixava Dasa, Natura, Vivo, TIM e Taesa sem receber documento nenhum. Aliases de CSN e Copasa declarados. Guardas: `api/test/cvm-atribuicao.test.mjs` (17 testes, prova de 2 pontas medida contra o código pré-correção, 7 falham lá) e checagem nova em `scripts/check-emissores-cadastro.mjs` que reprova emissor sem razão social própria, julgando só por `DENOM_SOCIAL` porque é o campo que o IPE publica. |
