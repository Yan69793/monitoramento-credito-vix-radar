...
| F021 | Documentation drift | `CLAUDE.md` | **Low** | S | Seção "Versões em produção (source of truth, 11/04/2026 — SUPERADO)" e múltiplas entradas históricas tornam o CLAUDE.md difícil de ler — seção canônica está no meio de 5 seções "SUPERADAS". | Mover blocos históricos para `docs/archived/CLAUDE_historico.md`. Manter no CLAUDE.md apenas as seções atuais + ponteiro para histórico. |
