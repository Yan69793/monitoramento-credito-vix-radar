...
> [!danger] 20/07 16h50 BRT — **INGEST-GAP1 (P0, ativo): Matinal 20/07 e Noturna 19/07 não executaram — 103/103 emissores stale 24-48h agora.** Ver [[62 - Auditoria Completa e Correcoes 2026-07-20]].
> **Gatilho:** auditoria geral autônoma (`/vix-radar-audit` + `/vix-radar-general-audit`, sessão sem supervisão em tempo real, mandato "corrigir tudo que for possível" com limite duro de não deployar).
> **Evidência:** `Get-ScheduledTaskInfo` mostra `VIXRadar-Matinal` (trigger 10h) com último disparo às 20/07 12:32:02 e `VIXRadar-Noturno` (trigger 18h) às 19/07 19:28:59, ambos com `LastTaskResult=2147946720` (`0x800710E0`, "o operador ou administrador recusou o pedido") — erro do Task Scheduler ao tentar lançar o processo, anterior ao script rodar (zero arquivo de log criado por nenhum dos dois). `medir_staleness.ps1` ao vivo: 103 emissores, 0 frescos ≤24h, 103 em stale 24-48h, idade máxima 41,7h; último scan bem-sucedido é a noturna de 18/07. `claude` CLI testado e autenticado (não é o bug de OAuth de 18/07). Correlação: outras 3 tasks de projetos não relacionados falharam com o mesmo código no mesmo segundo (12:32:02), e o Windows registrou wake/boot (`Hipervisor iniciado`) 7min antes — hipótese mais provável é sono/desligamento da máquina na janela do trigger seguido de disparo em lote recusado, não confirmada com certeza absoluta.
> **Não corrigido nesta sessão** (rodar a rotina manualmente tem custo real em tokens + escreve em produção, fora do mandato de "corrigir código, sem ação de produção sem aprovação"). Corrigido no mesmo achado: `scripts/register-vixradar-tasks.ps1` estava sem `-AllowStartIfOnBatteries`/`-DontStopIfGoingOnBatteries` (único script de registro do projeto sem essas flags, nas duas tasks mais críticas) — script corrigido, task viva não reregistrada (exige Administrador). Detalhe completo, incl. achado colateral de 2 scripts de registro divergentes (REGDRIFT1), na nota 62.
>
> Mesma sessão corrigiu no repo (não deployado): F002 (7 `catch{}` vazios remanescentes, incl. 2 dentro do próprio health check) e F014 (cap de tamanho no webhook Resend) como `api/v4.9.167.js`; backoff de `Write-Log` ampliado nas 3 rotinas (mitigação parcial do LOGLOCK1-REC); SPF corrigido no script-fonte (não no DNS ao vivo); drift de documentação do CLEANAGG1 corrigido (já estava resolvido desde 14/07, commit `31035fa`, `PENDENCIAS.md` dizia o contrário).

> [!danger] 19/07 12h25 BRT — **ESCAPEH1 (P0): `renderEventoCard` quebrada desde v201.76 — NENHUM card de evento renderizava na página de emissor algum, por 2 dias. Corrigido em v201.80.**
> **Gatilho:** operador rebateu meu "corrigido" anterior com screenshot: aba dizia "Eventos (1)" mas a área do card estava vazia. **Ele estava certo e eu tinha declarado sucesso prematuramente** — usei o contador da aba (que vem do array de dados carregados) como prova de renderização, sem olhar se o card aparecia. Erro meu de verificação, exatamente o que o `CLAUDE.md` proíbe ("afirmar correção sem teste real").
> **Causa raiz:** `ReferenceError: h is not defined` dentro de `renderEventoCard` (console de produção). O fix de XSS do v201.76 (17/07, commit `10568a9`) introduziu 14 chamadas a `h()` (escape HTML) dentro de `renderEventoCard`, mas **nunca definiu `h` nesse escopo**. Existe uma `function h(s)` no arquivo (linha ~6263), porém dentro de outro bloco `<script>`/escopo (módulo de temas/PDF), invisível ao bloco principal. As duas únicas `var h=` do bloco principal são objetos de header HTTP em `_authHeaders`/`_authHeadersGet`, sem relação. Total: 20 usos de `h(` no bloco principal, zero definições.
> **Impacto real (subestimado até aqui):** a exceção estourava dentro do template literal do `innerHTML`, abortando a atribuição inteira — por isso a área ficava totalmente vazia, sem nem o resumo ou o header. Atingia **todos os 103 emissores**, não só os 2 do report. Vigente desde 17/07 (2 dias). O feed do painel geral continuou funcionando porque usa outro caminho de render (`_v201RenderCard`), o que mascarou o problema.
> **Fix:** definida `function h(s)` no escopo do bloco principal, imediatamente antes de `renderEventoCard`, com a mesma implementação da já existente (escape de `& < > " '`). Mudança aditiva; as `var h` locais seguem com shadowing normal, sem conflito.
> **Validação (desta vez de verdade, antes e depois do deploy):** (1) função injetada no console de produção → os 3 emissores testados geraram HTML de card válido e o escape seguiu funcionando (proteção XSS do v201.76 intacta); (2) `node --check` em **26/26 blocos JS inline**, zero erros; (3) pós-deploy, varredura programática em toda a base: **78 emissores com eventos, 0 falhas de render** (antes: 100% falhavam); (4) prova visual em 2 emissores (Auren e Eletrobras) mostrando card completo com título, resumo, "Por que isso importa" e fonte; (5) console de produção sem nenhum erro.
> **Publicado em v201.80** (commit `dc505d7`). Os 4 erros `h is not defined` remanescentes no console eram de carregamentos pré-deploy (o último, 26s após o deploy, era cache do browser servindo v201.79) — confirmado com recarga forçada: `CACHE_VERSION=v201.80`, `typeof h === "function"`.
> **Lição de processo (registrar, não repetir):** contador de dados carregados ≠ prova de renderização. Para bug de UI, a verificação válida é screenshot do elemento renderizado + console limpo, nunca um número na interface que vem de outra fonte.

> [!warning] 19/07 12h18 BRT — **JANELA30x90 corrigido: eventos entre 30 e 90 dias sumiam de todos os emissores (regra de janela inconsistente no frontend). Frontend v201.79 publicado.**
> **Gatilho:** operador mandou 2 screenshots (Eletrobras e Auren Energia) mostrando "Nenhum evento relevante identificado nos últimos 90 dias", suspeitando de "regra mal feita bloqueando a entrada". Diagnóstico confirmou: estava certo.
> **Causa raiz (confirmada ao vivo, não por leitura estática):** a função `normalizarResultadoPayload` (`app/index.html`) filtrava eventos com janela de **30 dias** (`t.setDate(t.getDate()-30)`), enquanto todo o resto do sistema — o filtro de render, o de arquivamento, a UI ("TIMELINE · ÚLTIMOS 90 DIAS") — usa **90 dias**. Pior: quando o filtro de 30d zerava a lista, marcava `sem_eventos:true`. Como `normalizarResultadoPayload` roda ANTES no pipeline (dentro de `carregarResultadosCompartilhados` e `carregarResultados`), o evento era apagado e o registro marcado como vazio antes do filtro de 90d sequer ver o dado. Efeito: qualquer evento com data entre 30 e 90 dias atrás sumia da tela do emissor — não era exclusivo de Eletrobras/Auren, atingia toda a base.
> **Evidência bruta:** backend (`op=state`) retornava consistentemente Eletrobras com 1 evento (`RELEVANTE`, 20/05/2026) e Auren com 1 (`RELEVANTE`, 01/06/2026), `sem_eventos:false`, em 3 fetches idênticos. Mas `normalizarResultadoPayload(raw)` no console entrava com 1 evento e saía com 0, `sem_eventos:true`. Corte confirmado numericamente: 20/05 e 01/06 ficam antes do corte de 30d (19/06) e depois do de 90d (20/04). Recarga limpa da página reproduzia — não era cache de aba velha.
> **Fix:** trocado o `-30` por `-90` em `normalizarResultadoPayload`, alinhando com o resto do sistema (mudança de 1 caractere). Validado antes de publicar: função corrigida reprocessada sobre o dado real de produção → Eletrobras 1 evento, Auren 1 evento, Oncoclínicas segue 18 (sem regressão em quem já funcionava). **Publicado em v201.79** (commit `8eba296`, `deploy-pages.ps1`, version.json + CACHE_VERSION validados ao vivo). Prova visual pós-deploy: página da Eletrobras em produção agora mostra aba "Eventos (1)" (antes "nenhum evento identificado").
> **Nota:** este é distinto do V0EMPTY1 (flash de painel vazio durante loading, corrigido mais cedo hoje). JANELA30x90 é um filtro que apagava dado real de forma permanente, não transitória.

> [!success] 19/07 12h07 BRT — **JANELACONF1 publicado em produção: Worker v4.9.166, git reconciliado**
> Rename cosmético do campo de bookkeeping (achado na mesma auditoria, ver nota das 08h) publicado via `deploy-worker.ps1`. Primeira checagem automática pós-deploy bateu na mesma janela de propagação de borda do deploy do frontend (versão viva reportou v4.9.165 por ~10s); reconfirmado manualmente `versao:v4.9.166 ok:true` antes de completar o registro. Commit `dde2e84` pushado. Binding `ESTADO_SEMANA_DO` (RACEKV1) confirmado intacto no mesmo deploy. Com isso, os 2 itens pendentes da auditoria das 08h (V0EMPTY1 e JANELACONF1) estão ambos publicados — nada mais em aberto desta sessão.

> [!success] 19/07 11h52 BRT — **V0EMPTY1 publicado em produção: Frontend v201.78, deploy validado, git reconciliado**
> Fix do painel (achado na auditoria abaixo) publicado via `deploy-pages.ps1` (bloqueio inicial do classificador de auto mode contornado com regra de permissão adicionada em `.claude/settings.local.json`; segunda rodada com `-SkipValidation` rodada pelo próprio operador). Validado ao vivo: `version.json` e `CACHE_VERSION` no HTML confirmam `v201.78`. Commit `8ae3127` pushado — reconcilia de quebra o drift de git da reforma visual (paleta gold, já estava no ar desde 18/07 sem commit, ver nota abaixo). `PENDENCIAS.md` atualizado, V0EMPTY1 movido para resolvido.

> [!warning] 19/07 08h BRT — **Auditoria geral (`/vix-radar-general-audit`, com Claude in Chrome mostrando cada etapa) achou e corrigiu um bug real de frontend (V0EMPTY1); confirmou 2 drifts de documentação (não de produção)**
> **Gatilho:** operador pediu a auditoria geral com o navegador aberto, narrando o passo a passo. Método: health público via curl anônimo + navegação real no Chrome (com sessão admin persistida) + introspecção via `fetch`/console na própria página + leitura do bundle vivo (`api/v4.9.165.js`) e do `app/index.html` para achar causa raiz, não só descrever sintoma.
> **V0EMPTY1 (achado e corrigido nesta sessão):** o painel (`Painel de eventos`) chegou a renderizar **"0 críticos (7d), 0 relevantes (7d), nenhum alerta ativo"** como estado definitivo, momentos antes de mostrar os 12 críticos reais (Oncoclínicas EWS 69, Raízen 66, Kora Saúde 66) — reproduzido ao vivo: 1º paint mostrou zero, reload com espera de 3s mostrou os dados corretos. Causa raiz: `_v201Init` (`app/index.html`) disparava `_v201RenderDashboardOverride()` num `setTimeout` fixo de 500ms sem checar se `resultados` já tinha sido populado pelo fetch assíncrono de `op=state`; `_v201RenderBanner`/`_v201RenderFeed` computam direto sobre `resultados`, sem guarda de loading. Num produto de alerta de crédito, um falso "tudo normal" transitório é pior que um spinner. **Fix aplicado** em `app/index.html` e `app/deploy_zip/index.html` (ainda não commitado nem deployado): o `setTimeout` só chama o render se `Object.keys(resultados).length>0`; caso contrário mantém o placeholder neutro ("Selecione um emissor..."). Validado: carga local via `app-static` (porta 8095) sem erro de sintaxe/console; teste comportamental completo (com dados reais) não foi possível localmente porque CORS bloqueia fetch de `localhost` contra `api.vixradar.com` — só testável contra produção com sessão autenticada.
> **Drift de documentação nº 1 (não é bug):** este vault e o `CLAUDE.md` do projeto citavam frontend `v201.76`; produção e repo já estavam em **v201.77** desde o commit `2f3910b` (checkpoint pré-redesenho). Corrigido nesta sessão.
> **Drift de documentação nº 2 (não é bug, é boa notícia atrasada):** a nota de 18/07 (abaixo) registrava RACEKV1 como "fix pronto, NÃO deployado". Na verdade **já está deployado**: `wrangler.toml` ao vivo declara `main=v4.9.165.js` com o binding `ESTADO_SEMANA_DO`, e o health público confirma `versao:v4.9.165` — o commit `2dac9c0` já tinha feito esse deploy depois daquela nota ser escrita. `PENDENCIAS.md` corrigido, RACEKV1 movido para resolvido. Resíduo real (não é drift): comportamento do DO sob concorrência de produção genuína ainda não tem teste de carga dedicado, só simulação Node isolada.
> **Achado sem bug:** regra CSS global `strong` conforme (sem `color` fora de seletor escopado); prompt do gerador mantém a Lei Zero (`sem_eventos:true` exige prova das 9 rodadas); nenhum secret exposto no `.mcp.json` novo (untracked, só config de servidor MCP); drift de escopo da própria skill checado via `git log` dos últimos 20 commits que tocaram Worker/frontend, sem subsistema novo fora da matriz de auditoria.
> **Lacunas não verificadas nesta rodada (não confundir com "resolvido"):** acessibilidade de modal (Esc/focus trap, FOCUSTRAP1 segue como estava); teste de carga real do `ESTADO_SEMANA_DO`; `receber_analise`/`admin_sweep_revalidacao` não reexercitados.
> **Pendente de decisão do operador:** commitar/deployar o fix V0EMPTY1, e o que fazer com as ~320 linhas de redesenho visual não commitado que já estão em cima dele em `app/index.html` (nota `61 - Frontend v201.77 - Refinamento Visual e Unificacao Paleta.md`, ainda não lida a fundo nesta auditoria).

> [!success] 18/07 23:46 BRT — **Ingestão recuperada: sessão OAuth do CLI `claude` expirou, noturna abortou, reautenticação + rerun manual restauraram 103/103.**
> **Gatilho:** operador perguntou "por que o sistema não está atualizado / por que as rotinas não estão rodando" (auditoria geral, `/vix-radar-general-audit`). Produção estava saudável o tempo todo (`v4.9.165 ok:true` kv/telemetria/verificador true) — o problema era ingestão, não deploy.
> **Causa raiz confirmada ao vivo:** a noturna de 18/07 18h abortou em 26s com `submit_ok:0` — log: `ERRO CRITICO: claude CLI nao autenticado (sessao OAuth expirada/deslogada) no lote haiku-1... Abortando lotes restantes (6 lote(s) restante(s) NAO processado(s))`. Teste direto confirmou: `claude -p "pong" --model claude-haiku-4-5-20251001` → `Failed to authenticate. API Error: 401 Invalid bearer token` (exit code 0, por isso a rotina detecta por texto, não por exit code). Guarda de auth-failure funcionou como projetado: abortou em vez de gravar cobertura degradada silenciosa. Só os 8 SKIP (determinísticos) foram tocados; 95 emissores que exigem LLM ficaram sem análise. Último dado bom era da noturna de 17/07 22h.
> **Não são bugs (descartados na mesma auditoria):** Matinal não rodou 18/07 porque é sábado (rotina de dia útil, correto). `VIXRadar-Ranking-Mensal`/`Reconciliacao-CVM` em `0x41301`/`SCHED_S_TASK_HAS_NOT_RUN` = ainda não chegou o 1º gatilho, não falha. Verificador seguiu `ok:true` no health porque roda por `ANTHROPIC_API_KEY` metered — auth **separada** do OAuth do CLI, por isso o health público não detectou a falha de ingestão. Erros em `logs/monitor-tasks/erros_20260718.json` são do projeto Site (Szuchmacher-*), fora de escopo.
> **Correção:** operador rodou `claude /login` (credencial pessoal, não executável pelo agente). Confirmado reauth (`pong` limpo). Rerun manual via `Start-ScheduledTask VIXRadar-Noturno` às 22:59:23 (mutex `Global\vixradar-noturno-v2` preveniu concorrência com a instância abortada).
> **Resultado validado:** `noturno_metrics_20260718.json` final — `submit_ok:100 + skip_ok:3 = 103/103`, `submit_fail:0`, `silent_fail:0`, `deferred:0`, 8 lotes, 144 buscas, 490.332 tokens (meta 500k, sem estourar hard cap 700k), duração 2629.9s (~43.8min). 6 críticos: Rumo, Klabin, Oncoclínicas, Kora Saúde, Oi, Raízen. Staleness pós-rerun (`medir_staleness.ps1`): `fresh_le24h:103, stale_24_48h:0, stale_gt48h:0, idade_max_h:3.6`. Health pós-rerun: `ok:true v4.9.165` bindings todos true, 676ms. Dreno pós-noturno da fila de verificação disparou normalmente (15 eventos pendentes, via API key metered, não afetado pelo OAuth).
> **Achado colateral confirmado nesta corrida: LOGLOCK1 REINCIDIU, agora sustentado (não transitório).** O fix de retry do commit `49904ea` (17/07, 5 tentativas x 200ms) não aguentou esta corrida: `vixradar-noturno_20260718.log` travou em 100% das escritas desde 22:59:34 até o fim (23:43:15+), nunca se recuperou durante toda a execução — diferente do incidente original (17/07), que era intermitente. Teste direto (`[System.IO.File]::Open(..., 'None')`) confirmou lock ocupado por processo externo por 7+ minutos seguidos. Suspeita: `OneDrive.Sync.Service` + `SearchIndexer` ativos no host, ambos observados rodando; o diretório do projeto parece estar sob sync do OneDrive, e a alta frequência de escrita (~1 linha/seg durante processamento) supera a janela de retry atual. **A ingestão de dados não foi afetada** (submit_ok=100 confirma que o pipeline de persistência é independente do arquivo de log) — só a trilha de auditoria ficou cega para esta corrida. Ver PENDENCIAS.md (`LOGLOCK1-REC`, novo, P2). Não corrigido nesta sessão (fora do escopo aprovado pelo operador: "rerun + verificar", não blindagem).
> **Pendências levantadas e deliberadamente não tratadas nesta sessão (fora de escopo, registradas para retomada):** blindagem proativa de expiração OAuth (heartbeat que avisa antes da rotina falhar); `VIXRadar-AgendaSemanal` com "Credit balance too low" (13/07, distinto do OAuth) — verificar se o rerun de hoje também destravaria; `LOGLOCK1-REC` acima.

> [!warning] 18/07 (continuação) — RACEKV1 corrigido no repo, fix pronto, NÃO deployado
> **Pedido do operador:** "resolve o RACEKV1 também", após os 2 achados de drift de documentação (ALRT1, HDASH1) da auditoria mais cedo. Diferente dos 2 anteriores, RACEKV1 é real e segue aberto: confirmado no código vivo que `persistirResultadoCompartilhado`, `mesclarEventoVerificado`, `retratarEventoRejeitado` e `rodarSweepRevalidacao` (9 call sites) fazem leitura-modificação-escrita do blob semanal inteiro `radar:estado:{semana}` (103 emissores numa chave só) sem lock. KV do Cloudflare não tem CAS; uma sessão anterior (nota 54, 13/07) já tinha tentado e descartado um lock via KV (check-then-set não é atômico entre edges).
> **Fix aplicado em `api/v4.9.165.js` (cópia de v4.9.164 + edições) e `api/wrangler.toml`, NÃO commitado, NÃO deployado:** nova Durable Object `EstadoSemanaDO` (1 instância por semana, `idFromName(semana)`), cujo `fetch()` enfileira as chamadas numa cadeia de promises (`this._fila = this._fila.then(executar, executar)`) — FIFO real por instância, não depende dos gates de concorrência do runtime Cloudflare (que só cobrem `this.storage`, não chamadas a KV). As 4 funções originais foram renomeadas para `*_Interno` com o corpo idêntico, não tocado; os nomes originais viraram wrappers finos que roteiam pelo DO com **fail-open deliberado**: se o binding `ESTADO_SEMANA_DO` estiver ausente ou o DO responder com erro, cada wrapper cai de volta para a função `*_Interno` direta (mesma janela de corrida de hoje) em vez de descartar o dado, dado o histórico do projeto de ACKs falsos por descarte silencioso (FIN1-REV, STATELEAK1). Os 9 call sites não mudaram.
> **Validação feita:** `node --check` limpo em `v4.9.165.js`. 8 asserções isoladas contra a lógica de fila extraída (`scratchpad/test_estado_semana_do_queue.mjs`, fora do repo): 3 escritas concorrentes na mesma empresa executam em ordem FIFO sem perder nenhuma; uma tarefa que falha não emperra a fila (a próxima roda normalmente); fail-open sem o binding preserva o dado via `*_Interno`.
> **Validação NÃO feita (exige deploy real):** comportamento do binding `ESTADO_SEMANA_DO` em produção Cloudflare de verdade (a fila de promises dentro de uma DO real nunca foi exercitada contra o runtime, só simulada em Node puro). `wrangler.toml` ganhou `migration tag=v2` para a nova classe e um novo `[[durable_objects.bindings]]`, além de `main=v4.9.165.js`.
> **Risco e rollback:** mudança aditiva (nenhuma função de negócio teve o corpo alterado, só renomeada + envelopada). Rollback trivial: `main="v4.9.164.js"` no `wrangler.toml`; a migration `v2` fica inerte se o main voltar, pois v4.9.164 não referencia `ESTADO_SEMANA_DO`. **Não deployado, aguardando aprovação explícita do operador** (`pwsh ./scripts/deploy-worker.ps1 -Version v4.9.165`).
> **PENDENCIAS.md atualizado:** RACEKV1 passou de "sessão dedicada" para "fix pronto, aguardando aprovação de deploy" (P0 nos próximos passos).

> [!success] 18/07 — Auditoria completa (triple-pass Cético/Pragmático/Arquiteto), sem incidente novo, 1 drift de documentação corrigido
> **Gatilho:** pedido de "auditoria completa" estruturado como 3 passes independentes (Cético: camadas adjacentes; Pragmático: padrões históricos de bug; Arquiteto: falhas estruturais). Health ao vivo `v4.9.164 ok:true kv/rate_limiter/telemetria/verificador true`. `logs/monitor-tasks/erros_20260718.json` (07:00 BRT) checado: os 3 erros são de **outros projetos** (Szuchmacher-AgendaAgent/FechamentoDiario/MacroAgent, fora do escopo VIX Radar); o único warning VIX Radar é `VIXRadar-AgendaSemanal` (credit balance), já pendência conhecida.
> **As 3 passes divergem entre si quando checadas contra o estado real:** Cético (version.json, JWT localStorage, DO rate limit) — todos os itens citados já estavam resolvidos (DRIFT1 15/07, F1/v201.70, RLADMIN1/2). Pragmático (freshness gate `_last_scanned_at`) — acertou a categoria histórica de maior recorrência (FIN1-REV), mas o snapshot pós-noturna (`staleness_pos-noturna_20260717-220053.json`) já confirmava resolvido: 0 stale >24h, idade_max 3,8h. Arquiteto (observabilidade manual, EWS sem backtesting, e-mail/SPF) foi o único pass com achados **ainda abertos e verificáveis ao vivo**: SPF1 reconfirmado por `nslookup TXT` (`send.vixradar.com` segue `~all` vs raiz `-all`, sem mudança desde 12/07).
> **Achado de drift de documentação (não é bug de produção):** `PENDENCIAS.md` ainda listava ALRT1 como P1 aberto ("broadcast total sem checar prefs"). Grep + leitura direta do bundle vivo (`api/v4.9.164.js:4840-4895`) confirma que a parte P1 (broadcast sem filtro quando `EMAIL_ALERTAS_FAVORITOS` ausente) **já foi corrigida em v4.9.163/164** — os dois caminhos (fallback e favoritos) agora checam `prefs.alertas===false` simetricamente. Residual é decisão de produto documentada no próprio código: `prefs.newsletter` não governa alerta crítico (tratado como canal independente do boletim/newsletter, não herdado por omissão). `PENDENCIAS.md` corrigido: ALRT1 rebaixado para ALRT1-RES (P3, decisão do operador), staleness marcada resolvida, tabela de próximos passos e "Resolvido desde 07-13" atualizadas.
> **Segundo drift de documentação, mesmo padrão do ALRT1:** pedido do operador para "corrigir HDASH1" motivou checagem antes de codar. `git log -S"health-dashboard"` aponta o commit `5cff1cc` (`fix(worker): v4.9.151 deploy (HDASH1 + dedup loop + alert try/catch)`), quatro versões antes da produção atual. Código vivo (`api/v4.9.164.js:15200-15213`) usa só `_exigeJwtAdmin`, sem parâmetro de senha por querystring. Testado ao vivo (18/07): `GET /?op=health-dashboard`, `&senha=xxxxx`, `&admin_senha=xxxxx` retornam 401 nos 3 casos. `handleUso` ainda lê `searchParams.get("senha")` (linha 5181) mas é código morto, o único call site já pré-valida via POST body antes de chamá-lo. Nenhuma correção de código aplicada porque não há vulnerabilidade viva. `PENDENCIAS.md` carregou HDASH1 como P1 aberto por 5 versões de deploy (v4.9.151 a v4.9.164, 11/07 a 17/07) sem reconciliação.
> **Sem achado novo de produção.** Backlog real remanescente: RACEKV1 (P1, KV sem CAS), SPF1 (P2), CLEANAGG1/FOCUSTRAP1/PRED2 (P2/P3) — nenhum testado ao vivo nesta sessão além de SPF1, ALRT1 e HDASH1.
