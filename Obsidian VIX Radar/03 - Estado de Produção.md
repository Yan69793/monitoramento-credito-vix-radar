...
Atualizado: 2026-07-07 ~22:20 BRT (prod: Worker **v4.9.148** + Frontend **v201.73**, ambos deployados — `GET /` `versao:"v4.9.148"`; `vixradar.com/version.json` v201.73; fixes XSS + mensagem de sessão confirmados no HTML servido). Sem drift repo/prod. **Contexto:** lançamento público no LinkedIn em 07/07 — sessão desta noite foi varredura de prontidão pós-lançamento, não auditoria de rotina.
