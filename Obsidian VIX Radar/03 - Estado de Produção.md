...
Atualizado: 2026-07-09 ~04:40 BRT (deploy segue v4.9.148 + v201.74, sem mudança; health `ok:true`/`verificador_ok:true`; investigação de causa raiz do incidente "painel sem notícias novas desde 06/07" concluída — ver callout abaixo). Última varredura de deploy: 2026-07-07 ~22:47 BRT (prod: Worker **v4.9.148** + Frontend **v201.74**, ambos deployados — `GET /` `versao:"v4.9.148"`; `vixradar.com/version.json` v201.74; fixes XSS + mensagem de sessão + a11y de 5 modais confirmados no HTML servido). Sem drift repo/prod. **Contexto:** lançamento público no LinkedIn em 07/07 — sessão daquela noite foi varredura de prontidão pós-lançamento, não auditoria de rotina.
