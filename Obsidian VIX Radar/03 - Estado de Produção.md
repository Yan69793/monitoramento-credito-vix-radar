...
Atualizado: 2026-07-08 ~18:40 BRT (deploy segue v4.9.148 + v201.74, sem mudança desde 07/07; health `ok:true`/`verificador_ok:true` confirmado após incidente de auth do `claude` CLI local — ver callout abaixo). Última varredura de deploy: 2026-07-07 ~22:47 BRT (prod: Worker **v4.9.148** + Frontend **v201.74**, ambos deployados — `GET /` `versao:"v4.9.148"`; `vixradar.com/version.json` v201.74; fixes XSS + mensagem de sessão + a11y de 5 modais confirmados no HTML servido). Sem drift repo/prod. **Contexto:** lançamento público no LinkedIn em 07/07 — sessão daquela noite foi varredura de prontidão pós-lançamento, não auditoria de rotina.
