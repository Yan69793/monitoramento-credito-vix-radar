...
> [!success] 20/07 16h00 BRT — **INGEST-GAP1 RESOLVIDO: recovery manual completo, deploy v4.9.167, fix estrutural aplicado.**
> **Recovery:** Noturno 103/103 emissores (9 críticos, 535k tokens) + Matinal 13/13 re-scaneados (7 críticos, 132k tokens). Sistema 100% atualizado.
> **Deploy:** v4.9.167 em produção (F002: 7 `catch{}` vazios com `console.error` + F014: cap 1MB webhook Resend).
> **Estrutural:** `register-vixradar-tasks.ps1` reexecutado como Admin. Tasks vivas: `StartWhenAvailable=true`, `AllowStartIfOnBatteries=true`, `DontStopIfGoingOnBatteries=true`, `RunLevel=HighestAvailable`. Se a máquina desligar no horário do trigger, o Scheduler agora executa assim que possível após o boot.
> **Causa raiz confirmada:** máquina desligada às 00:25 (não suspensa), cold boot 12:24, Task Scheduler disparou tarefas perdidas às 12:32. `VIXRadar-Matinal` falhou com `0x800710E0` porque `StartWhenAvailable=false` + `InteractiveToken` sem sessão pronta 8 min após boot. Tasks irmãs com `StartWhenAvailable=true` (AgendaSemanal, Verificacao-Async) rodaram normalmente no mesmo segundo.
> **Críticos ativos:** Oncoclínicas (REX R$ 5,1 bi, AGD pendente), Kora Saúde (RE, AGD 21/07), Raízen (reestruturação R$ 65 bi, penny stock), Oi (RJ, venda UPI, inadimplência CVM), Cosan (2 eventos).

> [!danger] 20/07 16h50 BRT — **INGEST-GAP1 (P0): histórico da auditoria autônoma que detectou o gap.** Ver [[62 - Auditoria Completa e Correcoes 2026-07-20]] para o diagnóstico completo pré-recovery.
