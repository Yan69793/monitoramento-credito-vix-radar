...
> [!success] Atualização Fase B D1, 2026-09-05 (madrugada): OpenRouter virou o transporte do runtime automático, 5/5 rotinas no adapter
> A Fase B D1 da CLAUDE-FREE-MIGRATION migrou o transporte LLM das rotinas automáticas do `claude -p` CLI para o adapter HTTP próprio `scripts/lib/vixradar-openrouter.ps1` (OpenRouter, `deepseek/deepseek-v4-flash`), **sem tocar em prompt, parser ou contrato de saída** (transport-first). Provider efetivo persistido: `VIXRADAR_LLM_PROVIDER=openrouter` em escopo User. Despacham pelo adapter: o motor (`run_vixradar_varredura.ps1`, cobre matinal e noturno), `verificacao_async`, sentinela e `agenda_semanal`. O ramo `claude`/Anthropic (CLI local, `Initialize-VixClaudeAuth`, escalação paga) ficou como `else` só acessível sob `claude-manual` + `-ForceClaude` manual do operador, fora do scheduler. Exit 86 / `BLOQUEADO_SEM_PROVIDER` / fail-closed / Gate 8 / `check-claude-free.ps1` / guarda incidente-73 preservados. Prova offline: parse PS 5.1 exit 0, `test-openrouter-adapter.ps1` 45/45, `test-openrouter-contratos.ps1` 24/24, `test-openrouter-dispatch.ps1` 20/20 (pwsh 7 e PS 5.1), `check-claude-free.ps1` exit 0, smoke real e canário real do motor PASS. Produção Worker/Pages intocada. O corpo datado abaixo permanece como registro; a verdade nova de agendamento vive em `routines/README.md` e `status/ESTADO.md`, detalhe em `PENDENCIAS.md` (bloco 05/09).

> [!warning] SUPERSEDIDA pela CLAUDE-FREE-MIGRATION Fase A (2026-09-04), quando os portões do operador fecharem
> Esta matriz descreve o regime CCD (Claude Desktop scheduler + tasks nativas `Disabled`), que deixa de ser o motor na Fase A: rotinas LLM passam a ser tasks nativas **Enabled** executadas pelo Task Scheduler, cada uma carregando o gate de provider `BLOQUEADO_SEM_PROVIDER` (exit 86) e saindo bloqueada sem `VIXRADAR_LLM_PROVIDER` habilitado. Determinísticas e watchdogs seguem. **O corpo datado abaixo não foi reescrito** (registro do que se acreditava até aqui); vale como fotografia do regime anterior e como base de comparação. **Verdade nova do agendamento vive em `routines/README.md`** (bloco Fase A no topo). Estado corrente e portões G1-G5 em `status/ESTADO.md`. Detalhe e inventário A/B/C/D/E em `100 - CLAUDE-FREE-MIGRATION Fase A 2026-09-04.md` e no item `PENDENCIAS.md`. Até o operador rodar G1-G3, o scheduler real segue o regime da matriz abaixo, com as rotinas LLM já saindo 86.
...
> [!info] Hardening HTTPS integral, 2026-09-04 (infra de borda, não reabre a matéria de rotinas deste documento)
> Fechada a exposição HTTP de `api.vixradar.com`, por configuração na zona Cloudflare, sem deploy de Worker/Pages e sem código novo. Estado relido da API: `ssl=strict`, `always_use_https=on`, `min_tls_version=1.2`, `automatic_https_rewrites=on` (já era on), HSTS de zona permanece `disabled`. HTTP agora redireciona para HTTPS (301) preservando host/path/query, sem loop, certificado válido.
> HSTS aplicado somente em `api.vixradar.com`, por Response Header Transform Rule (`http.host eq "api.vixradar.com"`), valor exato `max-age=31536000; includeSubDomains`, sem preload. `radar-credito-api.prospects-intel.workers.dev` não recebe o header. Semântica registrada: o redirect impede a aplicação de ser servida em HTTP, mas um request HTTP explícito pode trafegar headers/body em claro antes de receber o redirect; HSTS só protege depois de recebido validamente por HTTPS, e o header HSTS via HTTP é ignorado (RFC 6797). Nenhum segredo real foi usado nos testes. Evidência e backup fora do repo em `C:\Users\User\.claude\https-integral-2026-09-04\`.
> Pendência separada, não executada: remover o token `preload` do HSTS do frontend em `app/_headers`. Exige deploy de Pages. O bloqueio da data (árvore do frontend suja em v202.40) já caiu, sessão paralela deployou v202.40 e v202.41, segue apenas não executada.
