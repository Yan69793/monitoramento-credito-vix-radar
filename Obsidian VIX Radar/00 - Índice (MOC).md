...
- [[62 - Auditoria Completa e Correcoes 2026-07-20]] — `/vix-radar-audit` + `/vix-radar-general-audit` em sessão autônoma sem supervisão, mandato "corrigir tudo que for possível" sem deploy. **Achado novo INGEST-GAP1 (P0, ativo)**: Matinal 20/07 e Noturna 19/07 não executaram (Task Scheduler recusou lançamento, `0x800710E0`), 103/103 emissores stale 24-48h — não corrigido (ação de produção fora do mandato). Corrigido no repo, não deployado: F002 (7 catches vazios remanescentes, `api/v4.9.167.js`) e F014 (cap webhook Resend). Corrigido e já efetivo: battery settings em `register-vixradar-tasks.ps1`, backoff de log ampliado (mitigação parcial LOGLOCK1-REC), script de SPF. Drift de documentação corrigido: CLEANAGG1 já resolvido desde 14/07 (commit `31035fa`), `PENDENCIAS.md` estava desatualizado.
- [[61 - Frontend v201.77 - Refinamento Visual e Unificacao Paleta]] — pesquisa da rotação automática; virou auditoria de dados. **Achado P0 novo**: `Raízen` e `Light` mapeados para a subsidiária errada em `cnpj_emissores.json` com `match=forte` (Altman da Light devolve `z_em 4.71`, zona saudável, medindo a Light Energia em vez da holding em RJ; os 6 FR do Plano de RE da Raízen vêm por CNPJ não mapeado). `_RJ_FLOOR` tem `as_of`/`fonte_url` `null` nos 21 e diz Kora "avaliando RE" com a RE deferida desde 04/05. Dataset IPE da CVM validado end-to-end (1.078 docs oficiais casam por CNPJ, 73/80 emissores) como ground truth de reconciliação, lag de 5 dias. Eleva PRED3 de P3 para P1
