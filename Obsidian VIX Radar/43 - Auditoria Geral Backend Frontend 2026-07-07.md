...
3. **P1** — Encoding no caminho Task nativa: reproduzir com a noturna de hoje 18h; se `RaÃ­zen` reaparecer no WARN, corrigir leitura do plano/lista (UTF-8) + comparação normalizada.
4. **P1** — Deploy Pages com bump `CACHE_VERSION` (v201.70) levando fix XSS **e** fix F1 completo (editar 2× `localStorage` da senha admin no `index.html` antes). Requer autorização.
