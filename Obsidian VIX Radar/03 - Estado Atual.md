...
> [!success] 21/09 BRT — **Noturna de 21/09 fechada depois da correção do provider: 104/104 no ledger, `AUTH_MODO: assinatura-token`, `degradados_402=0`. Worker `v4.9.260` e frontend `v202.44` em produção.** Janela medida de `13:48:50` a `14:25:26` BRT (`16:48:50Z` a `17:25:26Z`). A linha `FIM:` de `logs/routines/vixradar-noturno_20260921.log`, literal: `Total do dia 104/104 (ledger analisados=45 skip=10 deferidos=49). analisados_execucao=45 skip_execucao=10 deferidos_execucao=49 submits_aceitos=104 submit_ok=45 submit_fail=0 tokens=415218 cap_efetivo=700000 lotes=4 lotes_com_trabalho=4 buscas=122 silent_fail=0 degradados_402=0 criticos=3 auth_escalou=nenhum eventos_avanco_data=52 chaves_novas=85 descartados=7 deferidos_cap=0 deferidos_auth=0 deferidos_rotacao=49 motivo_deferimento=rotacao_semanal lotes_nao_processados=0 dreno_exit=0 duracao_sec=1822.8`. Os 4 lotes fecharam sem falha e o dreno da fila de verificação saiu `exit=0`. Críticos do dia: CSN Mineração, Tupy e Embraer. O provider é o caminho `claude-subscription`, aberto por `AVISO AUTH: chave paga Anthropic NAO autorizada ... Escalada paga desarmada.`, com zero `openrouter` e zero `402` no arquivo. Produção medida às 17:34Z: `HTTP:200`, `versao:"v4.9.260"`, `ok:true`, `kv:true`, `rate_limiter:true`, `telemetria:true`, `sentry_ok:true`, `verificador_ok:true`, `admin_email_ok:true`, `providers_configurados:"2/2"`, `fonte_externa_ok:true`, `painel_fresco:true`, `feed_fresco:true`, `feed_evento_mais_novo:"2026-09-21"`, `feed_idade_du:0`. Frontend confirmado em `version.json` (`v202.44`, `deployed_at 2026-09-20T03:27:52Z`). **A rodada não veio do agendador.** Medido só por leitura: `VIXRadar-Noturno` com `State=Ready`, `LastRunTime=09/18/2026 18:05:00`, `NextRunTime=09/21/2026 18:05:00` e `NumberOfMissedRuns=0`, então as 13:48 foram o motor canônico chamado à mão e a rodada agendada das 18:05 BRT de hoje ainda está por vir, o que a idempotência do ledger absorve sem duplicar cobertura. Zero código, configuração, Scheduler, produção ou rotina tocados nesta tarefa, e sem commit ou push.
> **Status:** FECHADO, medido no log da execução e no health vivo. **Data:** 2026-09-21. **Origem do Registro:** fechamento do estado canônico depois da noturna validada, a pedido do operador.
> **Condição de Obsolescência:** cai se `versao` do health deixar de ser `v4.9.260`, se o frontend de produção deixar de ser `v202.44`, ou se a próxima noturna registrar `degradados_402>0` ou `AUTH_MODO` diferente de `assinatura-token`.

> [!warning] 21/09 BRT — **Cobertura parcial da noturna de 21/09: registrada à parte, NÃO corrigida nesta tarefa.** Dos 45 analisados, **27 fecharam `INCONCLUSIVO`** por famílias de busca incompletas, com a linha `RECHECK_PENDENTE|<emissor>|motivo=familias_incompletas|faltantes=emissor,divida,fato` e `sem_eventos` proibido. Outros **25 foram rebaixados de `FULL` para `LIGHT`** já no plano, por `deferred_prioritario` (14), `inconclusivo_stale_breakout` (9), `imprensa_recente_7d` (1) e `setor_financeiro_full` (1). O contrato de cobertura fechou em `35 emissor(es) por cobertura (recheck_pendente=27 resolvidos=8)`, em `logs/routines/cobertura_20260921.json`. Os 49 `DEFERIDO` **não** são defeito, saem com `motivo_deferimento=rotacao_semanal`, `deferidos_cap=0` e `deferidos_auth=0`, que é o desenho da profundidade adaptativa semanal. Não medido se essa população é a mesma CARTEIRA FAMINTA de 19/09, então os dois itens ficam separados. Detalhe em [[PENDENCIAS]] e no bloco de 21/09 de `status/ESTADO.md`.
> **Status:** ABERTO, com decisão explícita de não mexer agora. **Data:** 2026-09-21. **Origem do Registro:** a própria execução da noturna, antes do fechamento do estado.
> **Condição de Obsolescência:** cai quando os 27 `RECHECK_PENDENTE` do contrato de 21/09 forem resolvidos por execução real.

> [!success] 21/09 BRT — **DRIFTPROVIDER1: o drift entre a decisão de provider e a configuração viva foi fechado. `VIXRADAR_LLM_PROVIDER` volta a `claude-subscription` no escopo User.** Em 05/09 a Fase B D1 da CLAUDE-FREE-MIGRATION migrou deliberadamente as rotinas locais para a OpenRouter e persistiu `openrouter` no escopo User: decisão consciente, provada e válida à época, registrada em [[10_Estado_Atual_Validado]]. Em 12/09 ela foi supersedida: OpenRouter descartada por saldo zerado, `PROVIDER-SPLIT1` revertido em `ab0e6cd` e a assinatura Claude Code Pro de volta como caminho oficial. A configuração não andou junto, então o runtime seguiu despachando para um provedor sem saldo e todo o motor local passou a falhar por `OPENROUTER_HTTP_STATUS=402` (a noturna de 17/09 abortou em `AUTH_MODO: nenhum`, e em 21/09 a sentinela fechou 100% das execuções em `resultado=FALLO_PROVEEDOR` com a matinal em 12 degradados 402). Corrigido em 21/09 no escopo User, um ponto único, sem tocar código, Scheduler, Worker ou Pages. Nenhum script do repositório persiste a variável e o XML da task da noturna não tem override, então ela herda o escopo User. Smoke runtime em processo com ambiente limpo: `AUTH_MODO: assinatura-token`, `CLAUDE exit=0`, zero `openrouter` e zero `402` no log. Backup e rollback em um passo em `_backup-env-provider-20260921-132703.json`.
> **Status:** fechado na configuração local (escopo User), sem deploy · **Data:** 2026-09-21 · **Origem do Registro:** rodada delta-first de 21/09 e correção do P0 AUTH_MODO.
> **Condição de Obsolescência:** cai se `VIXRADAR_LLM_PROVIDER` no escopo User deixar de ser `claude-subscription`, ou se uma execução agendada voltar a logar `AUTH_MODO: openrouter` ou `OPENROUTER_HTTP_STATUS=402` no motor local.
...
> [!success] 11/09 BRT — **EMAILSILENT1 residual FECHADO no repositório, sem deploy.** Os dois avisos de e-mail ao admin dentro de `handleRegistrar` (`api/src/worker.js`) saem de `enviarResend` cru e passam a usar `enviarEmailRastreado`: rastro por destinatário, alerta na Sentry, destinatário lido de `env.ADMIN_EMAIL || ADMIN_EMAIL`, dedup `cadastro:notif_reenvio:*` gravado só quando o envio devolve `ok === true` e resposta HTTP 200 de anti-enumeração preservada nos dois caminhos. Commit `dd37c8e` (2 arquivos, +284/−16), CI `Worker Tests` run `34646171658` completed/success, push `112ffbe..dd37c8e`, `HEAD == origin/main`. Guarda: `api/test/email-registro-rastro.test.mjs` (6 testes). Sem deploy, sem rotina e sem mudança de scheduler.
> **Status:** fechado no repositório · **Data:** 2026-09-11 · **Origem do Registro:** fechamento da sessão de 11/09 (commit exclusivo dos 2 arquivos que estavam no índice).
> **Condição de Obsolescência:** volta a aberto se um dos dois envios ao admin em `handleRegistrar` deixar de registrar rastro, se o dedup voltar a ser gravado em falha de envio, ou se o commit `dd37c8e` for revertido.

> [!success] 10/09 BRT — **PREDICTIVE-PRIVACY1 preparado e validado, sem deploy.** Os 47 `data/historico/*/predictive.json` deixam o próximo HEAD, permanecem fisicamente locais para backtests e ficam ignorados pelo Git. `.gitignore`, `scripts/hooks/pre-commit` e `scripts/run_vixradar_export_historico.ps1` preservam o dump local e bloqueiam entrada por staging explícito. `prob_30d` e `prob_90d` continuam somente no laboratório administrativo protegido, com `user_facing:false`; o frontend publicado não contém esses campos nem `predictive_v1`. A calibração estatística por emissor continua pendente.

> [!success] 10/09 BRT — **BUSCADEGRADADA1 fechado/PASS em `main` (commit `990dbb0`, `fix: persist degraded-search recheck until resolution`, 2 arquivos, +193/−47), sem deploy e sem backfill.** Push fast-forward `edbfe69..990dbb0`, `HEAD == origin/main == 990dbb0`. **O que mudou:** `RECHECK_PENDENTE` de cobertura vale ATÉ SER RESOLVIDO, sem a janela de 2 dias que a nota abaixo descreve ("fila prioriza contrato de hoje/ontem" fica superado aqui). A leitura varre TODOS os `cobertura_*.json` do mais novo ao mais antigo e resolve cada emissor pelo registro mais recente, então `RESOLVIDO` mais novo tira da frente, `RESOLVIDO` anterior não tira, e silêncio (fim de semana, dia sem execução) não resolve nada. Resolução só grava com submit aceito e cobertura válida, então busca degradada não vira `sem_eventos` nem sai da fila. **Causa raiz do defeito antigo:** `.Values` não existe no `PSCustomObject` devolvido pelo `ConvertFrom-Json`, então contrato gravado nunca priorizava ninguém; a leitura agora é por `PSObject.Properties`. **Prova:** `test-cobertura-contrato` 29/29 (PS 5.1 e pwsh 7), com pendência de 5 dias ainda priorizada (D1) e prova reversa do `.Values` (D1b), `RESOLVIDO` mais novo limpa (D2), `RESOLVIDO` anterior não limpa (D3), fim de semana não resolve (D4), leitura idempotente (D5), contrato corrompido ignorado (D6), wiring no motor (D7/D7b); mais `test-busca-degradada` 31/31, locks-overlap 13/13, dryrun-metrics 18/18, `check-claude-free` EXIT=0, `lint-encoding` RISCO 0. **Produção (10/09):** `HTTP:200 TEMPO:3.010440s`, Worker v4.9.244 `ok:true`, `feed_fresco:true`, `feed_idade_du:2`, `painel_fresco:true`, `feed_evento_mais_novo:2026-09-08`; frontend v202.43 em repo e produção, sem drift. **Backfill 09/09 = NENHUM:** três rodadas dirigidas sobre os 16 emissores INCONCLUSIVO por busca degradada em 09/09 não acharam fato de crédito datado 09/09 ausente do feed (a Vale de 09/09 é negativa sem impacto e com fonte fora da lista confiável, a Hypera é fato de 08/09, já na fronteira), evento artificial não se cria. **`cobertura_20260909.json` ausente é legado operacional:** a noite de 09/09 registrou os 16 no log e não persistiu contrato, então a priorização não tem o que ler para aquela data e eles voltam pela rotação normal, não reconstruir à mão. Detalhe em `status/ESTADO.md`.
> **Status:** vigente · **Data:** 2026-09-10 · **Origem do Registro:** fechamento do item aberto no bloco BUSCADEGRADADA1-REV/COBERTURA1 abaixo.
> **Condição de Obsolescência:** cai se a priorização voltar a ter janela de data ou se `RESOLVIDO` passar a ser gravado sem submit aceito.

> [!info] 09/09 BRT — **BUSCADEGRADADA1-REV/COBERTURA1 fechado e publicado (commit `e793fd3`, main).** RCA: Vibra (11ª emissão debêntures R$1,4 bi) e Natura (14ª emissão R$700 mi) de 08/09 não entraram no estado — noturna de 08/09 os deixou DEFERIDO (submit_ok=15/104) e a de 09/09 certificou `sem_eventos` com prova frágil: Vibra com única busca degradada ("sem retorno - limite backend", fora do léxico do guard) e Natura com queries só de rating/RI. Fix definitivo no motor (`run_vixradar_varredura.ps1` + prompts matinal/noturno): contrato persistido de cobertura por emissor/data (`logs/routines/cobertura_*.json`, fora do Git) com prova MECÂNICA por família — cada `fontes_consultadas` exige `familia`(F1 emissor|F2 dívida/emissão/captação|F3 CVM/RI/fato) + `query/timestamp/provedor/status_http/resultado/classificacao`; PROVAFALSA1 (texto do modelo sem esses campos) não conta; `NENHUM` só com as 3 famílias ok; família ausente/degradada/429/rate limit/limite backend/vazia anômala ⇒ `DEFERIDO_COBERTURA` (INCONCLUSIVO) com faltantes, reprocessado antes de itens novos (fila prioriza contrato de hoje/ontem), merge idempotente sem duplicar. Backfill real dirigido (evidência, não correção): Vibra e Natura persistidos (eventos 08/09), `repor-varredura_20260909.log` FIM submit_ok=2. Testes: cobertura-contrato 12/12, busca-degradada 31/31, suíte oficial vitest 327/327, parse PS5.1 0, `git diff --check` limpo. Produção: Worker v4.9.244 ok:true, Frontend v202.43. Sem mudança em Worker/Scheduler; deploy N/A (mudança local). Observável pendente: 1º ciclo real (10/09) deve mostrar `DEFERIDO_COBERTURA` com reprocessamento prioritário quando houver família faltante.
> **Status:** SUPERADO no ponto da priorização em 10/09 (janela de 2 dias removida, `990dbb0`; ver bloco acima). O resto segue vigente. · **Data:** 2026-09-09 · **Origem do Registro:** fechamento RCA Vibra/Natura (eventos perdidos 08/09).
> **Condição de Obsolescência:** cai se o contrato de cobertura mudar de forma (famílias/campos) sem esta nota acompanhar.

> [!info] 09/09 BRT — **OR429-FIX fechado e publicado (commit `d8c7820`, main).** Hardening do failover OpenRouter: `provider.allow_fallbacks=true` (failover nativo entre providers do mesmo modelo) com `require_parameters=true` preservado e sem `provider.only/order/ignore`; retry de 429/transporte agora em camada única dentro do adapter (Retry-After respeitado, 0..120s); retry duplicado do caller removido no ramo OpenRouter (`retryDelays=@(0)`); `-TotalTimeoutSec` passado pelo motor da varredura (default 26 min, override `VIXRADAR_OPENROUTER_LOTE_TIMEOUT_MIN`, logado como `teto_lote_s`). Prova real: noturna 09/09 18:05→18:59, Scheduler Result=0, FIM + ROTINA_RESUMO presentes, submit_ok=50/50, submit_fail=0, silent_fail=0, POS-NOTURNO exit=0, lock removido, `teto_lote_s=1560` no log. Testes: adapter 0 falhas (incl. T18-T21 novos), payload 0, contratos 24/24, dispatch 20/20, locks-overlap 13/13, parse PS 5.1 0 erros, `git diff --check` limpo. Sem deploy, Scheduler intocado. Detalhe em `status/ESTADO.md`.
> **Status:** vigente · **Data:** 2026-09-09 · **Origem do Registro:** fechamento do incidente OR429 (matinal 09/09 morta em backoff de 429 sem FIM).
> **Condição de Obsolescência:** cai quando o Worker/sistema passar a depender de outro regime de failover que esta skill não alcance, ou se um 429 fatal reaparecer com a nova camada única.

> [!info] 09/09 BRT — **Roteamento LIGHT/FULL publicado em `main` (`d8c7820`, junto do OR429-FIX).** `LIGHT = deepseek/deepseek-v4-flash-0731`. `FULL` continua resolvendo para Flash (modelo de LIGHT) enquanto `VIXRADAR_OPENROUTER_MODEL_FULL` não estiver configurado — pendência de configuração, não de código. A noturna real de 09/09 (18:05→18:59) executou sob esse código e fechou com Result=0, submit_ok=50/50, sem deploy de Worker necessário.
> **Próximo passo:** localizar amanhã a origem segura da credencial OpenRouter usada pelo VIX Radar, sem criar chave nova, e só depois validar e configurar o modelo FULL.
...
|---|---|---|
| Worker | **v4.9.260** | `ok:true`: kv/rate_limiter/telemetria true, `admin_email_ok:true`, `sentry_ok:true`, `verificador_ok:true`, `providers_configurados` 2/2, `fonte_externa_ok:true`, HTTP 200 em 21/09, medido às 17:34Z. |
| Frontend | **v202.44** | `version.json` de produção devolve `v202.44` (`deployed_at 2026-09-20T03:27:52Z`). O carimbo separa última atualização da base e último evento. |
| Git | v4.9.260 / v202.44 | `HEAD` em `90c02a8` (`chore(data): historico 2026-09-20`) e `origin/main` em `86802fb`, 1 commit à frente. O fechamento de estado de 21/09 ainda está no working tree, sem commit. |

## Cobertura

| Metrica | Valor |
|---|---|
| Emissores | 104 (universo). Era 103 antes da entrada da Usina Pampa Sul (CARTEIRA-PISO1, 04/09) |
| Noturno 21/09 | submit_ok=45, skip_ok=10, deferidos=49 por `rotacao_semanal`, submit_fail=0, 415.218 tokens (meta 500k, hard 700k sem hit), 3 criticos, 4 lotes, dreno verif exit=0. Os 27 INCONCLUSIVO e os 25 rebaixamentos de FULL para LIGHT ficam registrados a parte, no topo desta nota |
...
*2026-09-08: apresentacao institucional renovada — `docs/apresentacoes/VIX_Radar_Apresentacao_Institucional_2026-09-08.html` (HTML unico, sem dependencias alem de Google Fonts): 19 cenas (11 do filme + 8 de aprofundamento), modo cinema autoplay ~2min, roteiro/locucao escrita em legendas, trilha ambiente gerada via Web Audio, dados medidos no dia (Worker v4.9.243, Frontend v202.42, 104 emissores). Previews PNG em `docs/apresentacoes/screenshots/10_vix_institucional_2026-09-08_slide*.png`. Arquivo de 2026-07-27 mantido como historico.*

*2026-09-08 (follow-up): contato padronizado para `yan@szuchmacher.com.br` em todas as apresentacoes da pasta (setembro, julho, Bradesco, Manual — o Manual foi renomeado para `VIX Radar Manual Utilizacao.html` pelo operador). Trilha da apresentacao de setembro: Intermezzo de Cavalleria Rusticana (Mascagni, EMI 1940, regencia do compositor), dominio publico, `docs/apresentacoes/trilha-opera-foco.mp3` (5,4 MB) — toca em modo cinema, com fallback para trilha gerada em Web Audio se o mp3 nao estiver junto. Filme ~1min54s (11 cenas, 2s a menos por cena).*
