...
> [!success] 19/09 BRT: **HARNESSMORTO1 fechado no repositório, sem deploy.** `scripts/test-frescor-cvm.mjs` saiu com exit 1 em toda execução desde 20/08 (`ReferenceError: _cvmDiasCorridosApos is not defined`) e nenhum gate acusou, porque o `run-all-tests.ps1` só enumera `test-*.ps1` e nenhum workflow nem hook roda `.mjs` de `scripts/`. **Causa medida por execução, não por leitura de diff.** O harness de cada commit contra o `worker.js` do próprio commit (`git archive`), 11 pontos: `a4a0b47` passa 31/31 e `73c3a5d` (CVMCADENCIA1, 20/08) já morre. A hipótese de que a quebra veio do `6e0dda8` (AVANCOFEED1) foi refutada, ele só reutilizou o helper. Havia uma segunda camada escondida atrás do crash, medida acrescentando só as extrações faltantes: 5 dos 31 casos falhavam, quatro com expectativa da regra antiga de 2 dias úteis (o CVMCADENCIA1 trocou por 2 ciclos semanais) e um efeito do `2c8acc6` de 13/09, ver a pendência de 19/09 em `PENDENCIAS.md`.
> **Correção.** A lista de extração ganhou os 2 helpers e as 6 constantes que `avaliarFrescorCVM` usa hoje, todos extraídos do `worker.js` real. Duas dessas constantes eram cópia à mão no prelúdio, e `CVM_FONTE_MAX_DU` sobrevivia no teste um mês depois de sumir do Worker. As expectativas foram alinhadas ao contrato vigente e à suíte vitest, e o harness foi de 31 para 46 casos (dias corridos, próxima publicação, ciclo no payload, falha dura do CVMDURA1). **Guarda 1.** Antes de rodar caso nenhum, o script confere que todo identificador do código extraído que é declaração de topo do `worker.js` está na lista ou no prelúdio, e sai com `FALHA extracao: avaliarFrescorCVM usa X (api/src/worker.js:N), que nao esta na lista de extracao nem no preludio`. **Guarda 2.** `scripts/test-frescor-cvm.ps1` põe o harness no `scripts-tests.yml` via `run-all-tests.ps1`, e node ausente reprova em vez de pular.
> **Prova de duas pontas, saída crua.** Ponta boa: `node scripts/test-frescor-cvm.mjs` fecha `TUDO VERDE em 46 casos.` com exit 0, e pelo runner em `powershell.exe` 5.1 `OK   test-frescor-cvm.ps1   RESULTADO: TUDO VERDE em 46 casos.  [0.4s exit=0]`. Ponta ruim do conserto: o harness antigo contra o `worker.js` de hoje morre com `ReferenceError: _cvmDiasCorridosApos is not defined`, exit 1. Ponta ruim da guarda, com o helper fora da lista e depois com um helper novo no `worker.js`: `FALHA extracao: avaliarFrescorCVM usa _cvmDiasCorridosApos (api/src/worker.js:8042), que nao esta na lista de extracao nem no preludio` e `EXTRACAO INCOMPLETA: 1 dependencia(s) fora da lista, nenhum caso rodou.`, exit 1, e o runner na bancada quebrada fecha `REGRESSAO NOVA (nao esta no baseline): 1` com exit 1. Portão do dia em produção: `HTTP:200`, `ok:true`, `versao:v4.9.258`, `kv:true`, `telemetria:true`, `sentry_ok:true`.
> **Não verificado.** O `scripts-tests.yml` não foi executado, só o runner local no mesmo `powershell.exe` 5.1 que o CI usa. A suíte completa de `scripts/` não foi rodada. A guarda de dependência só enxerga declaração de topo na coluna zero do `worker.js` e referência lexical, não `globalThis[...]` nem `eval`.
> **Aberto, decisão do operador.** O `ok` do frescor da CVM deixou de acompanhar a idade da fonte em 13/09 e o `CLAUDE.md` diz o contrário. Detalhe em `PENDENCIAS.md`, seção de 19/09.
> **Mesma classe, medido e não corrigido.** Outros 4 `.mjs` de `scripts/` também estão fora de qualquer gate. Neste worktree, `test-dedup-eventos.mjs` exit 0, `test-scan-emergencia-provider.mjs` 9 pass e 0 fail, `test-lab-preditivo-policy.mjs` 9 pass mas ele testa uma cópia da função e não o `worker.js` (verde por construção), `test-scanfallback-email.mjs` morre em `ERR_MODULE_NOT_FOUND` de `@sentry/cloudflare` porque o worktree não tem `api/node_modules`, falha de ambiente e não de código. Fazer o runner enumerar `test-*.mjs` muda o escopo do gate e o CI precisaria de `npm ci` em `api/`, decisão do operador.
> **Status:** fechado no repositório, sem deploy. **Data:** 2026-09-19. **Origem do Registro:** falha do harness medida pelo operador em 19/09, ao trabalhar o EMISSORSTALE2.
> **Condição de Obsolescência:** cai se o harness voltar a sair com exit diferente de zero no `run-all-tests.ps1`, se o wrapper sair do runner, ou se a guarda de dependência deixar de nomear a função ausente.
