...
|---|---|---|
| Worker | **v4.9.192** | `ok:false` desde ~19h33Z de 12/08 por `verificador_ok:false` (fila de verificacao com itens >20h, 5 remanescentes enfileirados 11/08 23h33Z). kv/rate_limiter/telemetria true, `admin_email_ok:true`, `sentry_ok:true`, providers 2/2. Health check 12/08 20h09Z: HTTP 200, 0,20s. CALVAL-V2 ativo. |
| Frontend | **v202.8** | `version.json` deployed_at 2026-08-13T17:18:52Z. XSS do Market Overview fechado (escape + strip no write path). |
| Git | v4.9.192 | main local em `832b72d` (13/08), 8 commits à frente do origin: push bloqueado por token sem Contents write (403). Worker v4.9.193 commitado, deploy pendente do health verde. |
