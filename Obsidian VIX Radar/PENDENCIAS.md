...
> **Status:** ABERTO (P1 pendente de decisão do operador). P2 e P3 implementados no working tree, sem commit e sem deploy. **Data:** 2026-09-21. **Origem do Registro:** validação independente do achado CVM reportado pelo TRAE, medida ao vivo, e depois implementação de P2 e P3 sem tocar a política de ingestão.
...
> **Efeito esperado depois do deploy.** Produção segue em `v4.9.260` e ainda publica os valores antigos. Depois do deploy e até o próximo sync ENET, os campos de origem saem `null` e `cvm_ingestao_descartados_sem_dono` já sai com o valor do meta atual. A guarda manual passa a reprovar (3) enquanto a fila estiver vazia e o descarte for positivo. Ela não roda em workflow nenhum, então não gera alerta sozinha. A série 9265, 9328, 9426 e a série nova não são comparáveis.
