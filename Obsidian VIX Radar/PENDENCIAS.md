...
**Producao no momento da auditoria (17:34Z):** Worker `v4.9.261`, frontend `v202.44`, `ok:true`, `kv/rate_limiter/telemetria:true`, `sentry_ok/verificador_ok/admin_email_ok:true`, `fonte_externa_ok:true`, `painel_fresco:true`, `feed_fresco:true` (`feed_idade_du:0`). `curl.exe -s https://radar-credito-api.prospects-intel.workers.dev` colado integralmente no chat da sessao.

**Achado 1 (corrigido nesta sessao) — a skill de auditoria estava medindo o sistema com regua de 3 semanas atras.** O bloco de governanca do `SKILL.md` e do `references/audit-matrix.md` dizia Worker v4.9.228 (01/09); a producao real e v4.9.261 (22/09), **33 versoes de diferenca**. Consequencia pratica, nao so cosmetica: a matriz nao tinha secao nenhuma para o subsistema ENETWeb inteiro (v4.9.247-253, hoje o caminho PRIMARIO de ingestao de documento CVM, substituindo dependencia exclusiva do ZIP bulk), documentava o motor de agendamento das rotinas como o CCD store do Claude Desktop quando o motor real e o Task Scheduler nativo desde o cutover de 12/09 (`logs/monitor-tasks/motor.json`), e nao mapeava 14 campos novos do health publico (`reconciliacao_zip_*`, `painel_atualizado_em`/`_fresco`, `feed_evento_mais_novo`/`_fresco`, `cvm_atribuicao_*`, `cvm_ingestao_descartados_sem_dono`). O glossario de dominio tambem tinha "103" gravado como numero fixo do universo de emissores; o universo real e 104 desde a entrada da Usina Pampa Sul, e o proprio detector automatico da skill (`audit-ui-metrics.mjs`) chegou a imprimir esse "103" como a formula esperada enquanto auditava uma tela que mostra 104. **Correcao:** os 3 arquivos de governanca (`SKILL.md`, `references/audit-matrix.md`, `references/glossario-dominio.md`) e o detector (`scripts/audit-ui-metrics.mjs`) foram atualizados para v4.9.261/v202.44, com secao nova documentando ENETWeb, tabela nova mapeando os 14 campos do health e o que cada um reprova (nenhum deles reprova o `ok` agregado), correcao do motor de agendamento, e o termo "Emissores" do glossario deixou de fixar um numero (aponta para a fonte, `TOTAL_EMISSORES`, em vez de copiar o valor). **Causa raiz:** a propria skill ja tinha essa falha documentada como recorrente (achado P3-1 de 01/09, quando ja estava 1 versao atrasada) e a condicao de obsolescencia so dispara se alguem le o bloco de governanca antes de auditar; ninguem leu por 3 semanas porque nada força a leitura. **Guarda sistemica:** o `SKILL.md` agora chama o passo 3 ("mapa de versoes") de PORTAO, nao etapa, e a propria matriz ganhou um paragrafo dizendo que esta e a segunda reincidencia do mesmo defeito — proxima vez que acontecer, o item precisa virar checagem automatizada (comparar `main` do `wrangler.toml` com o header do bloco de governanca por script), nao continuar dependendo de leitura humana atenta.

**Achado 2 (registrado, nao corrigido) — a fila de verificacao ficou sem cobertura a maior parte do dia por cota da assinatura Claude esgotada.** Medido nos logs de hoje: `vixradar-matinal_20260922.log` fechou `FIM: ... 23/23 ... submit_ok=8` as 10:31, mas `dreno_exit=5` (a fila de verificacao NAO foi drenada); o dreno dedicado (`vixradar-verificacao-async_20260922.log`, 11:03) e a `vixradar-sentinela_20260922.log` (14:25) abortaram os dois com `ERRO: nenhuma credencial Claude disponivel` apos `LIMITE DE SESSAO da assinatura (cota)`, sem fallback pago autorizado (`ANTHROPIC_API_PAYG=NAO AUTORIZADO`, por decisao do operador). Este e o comportamento fail-closed correto e ja documentado (COTAESGOTADA1-GENERICO, fechado 16/09); nao e bug de codigo, e estado operacional do dia. Fica registrado porque a proxima janela de dreno agendada (19:15) e a rede de seguranca, e se ela tambem cair no mesmo limite de sessao a fila acumula sem alerta novo (o Worker ja dedupa o alerta de "sem_credencial" por dia).

**Achado 3 (achado de codigo em diff pendente, nao commitado) — `scripts/run_vixradar_varredura.ps1` tem uma janela onde a variavel de ambiente `VIXRADAR_LLM_PROVIDER` do processo pode ficar contaminada apos uma excecao.** O working tree tem uma feature em andamento (fallback OpenRouter→assinatura Claude quando o OpenRouter devolve HTTP 402), ~79 linhas novas em `Invoke-ClaudeBatch`. A funcao seta `VIXRADAR_LLM_PROVIDER` no escopo do processo antes de invocar o provider (linha ~460) e restaura o valor anterior no fim da funcao (linha ~720), mas essa restauracao fica FORA do bloco `try { } finally { }` que engloba a chamada ao CLI (o `finally` fecha na linha 695, a restauracao esta 25 linhas depois). Se algo lancar excecao nao capturada nesse trecho intermediario (parse do JSON de uso, por exemplo, que ja tem seu proprio try/catch — entao a janela de risco real e pequena, mas existe), a variavel de ambiente do processo fica com o valor do override e contamina a proxima chamada na mesma sessao PowerShell. Nao e achado bloqueante (nao esta em producao, nao foi commitado, o trecho vulneravel ja tem try/catch interno cobrindo a parte mais provavel de falhar) mas fica registrado para quem fechar esse diff: mover a restauracao para dentro de um `finally` que cubra a funcao inteira, nao so a chamada ao CLI.

**Pendencias que este achado NAO reabre:** CFG-02 P1 (ingestao CVM descarta documento sem dono, decisao do operador, ver entrada de 21/09), COBPARCIAL21 (cobertura parcial da noturna de 21/09), CARTEIRA FAMINTA (orcamento, 19/09). Nenhum dos tres foi tocado nesta auditoria.

**Achado 4 (implementado apos aprovacao do operador, sem commit) — CLAUDEFALLBACK-OR1: fallback simetrico Claude-assinatura -> OpenRouter, para o achado 2 acima nao se repetir sem rede de seguranca.** O operador pediu explicitamente "fazer fallback para openrouter" depois de ver o Achado 2 (verificacao-async e sentinela abortando por limite de sessao sem fallback pago). Confirmado por duas perguntas: cobrir os dois pontos de falha (preflight E cota esgotada no meio do lote) e ativar+documentar, nao so deixar gated. Implementado: funcao pura `Get-VixClaudeFallbackOpenRouterHabilitado` (`scripts/lib/vixradar-llm-provider.ps1`), le `VIXRADAR_CLAUDE_FALLBACK_PROVIDER` (Process>User>Machine, so `'openrouter'` liga); usada no preflight de `run_vixradar_verificacao_async.ps1`, `run_vixradar_sentinela.ps1`, `run_vixradar_agenda_semanal.ps1`, `run_vixradar_varredura.ps1` (Matinal+Noturno), e no ponto COTAESGOTADA1 dentro de `Invoke-ClaudeBatch` (recursao com `ProviderOverride 'openrouter'`, guarda `-not $providerOverrideAtivo` contra loop). So desvia se `Test-VixOpenRouterPronto` confirmar o adapter pronto, senao cai no fail-closed original. Variavel ativada no escopo User. **Prova:** AST limpo nos 5 arquivos, `test-session-limit-policy.ps1` 19/19, `test-monitor-provider-gate.ps1` 9/9, `test-openrouter-adapter.ps1` e `test-openrouter-contratos.ps1` verdes, `test-quota-esgotada-failclosed.ps1` **64/64 com a variavel ja ativa** (prova que o fail-closed sobrevive sem adapter pronto), suite nova `test-claude-fallback-openrouter.ps1` 10/10, `lint-encoding.ps1` 130 arquivos OK. `CLAUDE.md` atualizado no trecho que dizia "falha ou cota da assinatura continua sendo erro". Detalhe completo no bloco de 22/09 em `status/ESTADO.md`. **Causa raiz:** o fallback pago so existia num sentido (OpenRouter->assinatura, so para 402), entao qualquer falha da assinatura quando ela e o provider ativo nao tinha desvio. **Guarda sistemica:** suite dedicada a decisao (`test-claude-fallback-openrouter.ps1`) mais a suite de fail-closed existente rodando com a variavel ativa, provando as duas pontas (desvia quando pronto, nao desvia e nao finge sucesso quando nao esta).

**Achado 5 (achado de UI, causa raiz medida, sem fix aplicado) — ENETASSUNTOCOL1: titulo de evento CVM pode sair errado quando a origem e ENETWeb, por leitura de coluna fixa numa grade cujo layout depende da categoria.** Reportado pelo operador por captura de tela: card da Vale (21/09, CVM/B3) com titulo "4", contra card da Vibra Energia com titulo completo, mesmo layout. **Causa raiz:** `titulo: doc.assunto` em `costurarCvmEmEventos` (`api/src/worker.js:19319`) e `emitirAlertaTier1` (`api/src/worker.js:19033`); para documentos de origem ENETWeb, `doc.assunto` vem de `_enetLinhaNormalizada` (`api/src/worker.js:8495`), que sempre le a coluna de indice 4 da grade do portal. Reproduzido ao vivo contra o mesmo endpoint da rotina (`POST rad.cvm.gov.br/ENETWeb/frmConsultaExternaCVM.aspx/ListarDocumentos`, janela 15-22/09, 2188 registros parseados): a coluna 4 nao e fixa, depende da categoria. Para "Comunicado ao Mercado" (a categoria do card da Vale) ela quase sempre traz codigo curto de formulario SEC do emissor estrangeiro ("8-K", "424B2", "144") ou traco, e o assunto real esta na coluna 3 (evidencia colada: JPMorgan Chase, col3="Outros Comunicados Nao Considerados Fatos Relevantes", col4="424B2 -"). Para "Fato Relevante" e o oposto, col3 vazia e col4 com o assunto real (evidencia: JBS N.V., col4="JBS Viva - Acordo de associacao"). **O que nao foi possivel confirmar:** a linha bruta exata do documento da Vale de 21/09, porque a janela de consulta do proprio portal ja rolou (a mesma consulta hoje so devolve 1 registro da Vale, datado de 22/09); leitura da chave `cvm:documentos` no KV via token de API deu 401 (limitacao ja conhecida). A causa esta demonstrada pelo padrao geral medido ao vivo, nao pelo registro literal da Vale. **Escopo:** nao e caso isolado da Vale, qualquer documento ENETWeb de "Comunicado ao Mercado" com coluna 4 curta mostra titulo errado. **Nao corrigido:** exige mudanca em `api/src/worker.js` e deploy de Worker, fora do escopo desta sessao sem aprovacao explicita. **Guarda sistemica proposta (nao implementada):** `_enetLinhaNormalizada` deveria escolher entre col3 e col4 pelo conteudo (nao-traco, mais descritivo) em vez de indice fixo, ou mapear por categoria, com teste de duas pontas cobrindo Comunicado ao Mercado e Fato Relevante.

## 21/09: BUILDWORKER-EXIT0 — `build-worker.ps1` sai 0 mesmo quando o passo do hash falha

> **Status:** ABERTO (P3), registrado sem correção por decisão explícita do operador, para não misturar com o deploy do v4.9.261. **Data:** 2026-09-21. **Origem do Registro:** execução real do build durante a preparação do v4.9.261.
> **O que aconteceu.** `scripts/build-worker.ps1` roda com `$ErrorActionPreference = 'Continue'`. Nesta máquina o `Get-FileHash` não resolve no PowerShell 5.1 — o `PSModulePath` tem o caminho de módulos do PowerShell 7.6.6 (`…WindowsApps\microsoft.powershell_7.6.6.0_x64…\Modules`) injetado, o que quebra o autoload dos módulos do 5.1 —, então a linha 52 do script falha com `CommandNotFoundException`.
> **Por que é defeito.** O erro não interrompe nada: o script segue, a linha `ARTEFATO_OK path=… sha256=…` nunca é impressa, e o script termina no seu próprio `exit 0`. O chamador (`deploy-worker.ps1`, passo 1) confere apenas `$LASTEXITCODE -ne 0` e `Test-Path $bundlePath`, então aceita o build como bom. O artefato em si sai correto — o `node --check` roda logo antes e é o que valida o bundle —, mas o operador perde a única evidência de integridade que o build produz, sem nenhum aviso. Um hash que não é impresso é indistinguível de um hash que confere.
> **Não é o deploy que está errado.** `Get-FileHash` não é usado em nenhum outro ponto do caminho de deploy, então isso não afeta gate nenhum; o defeito é local ao `build-worker.ps1` e à sua política de erro. O contraste está no `deploy-worker.ps1`, que usa `Fail` em todos os passos e não deixa passo falho passar.
> **Condição de Obsolescência:** cai quando o `build-worker.ps1` tratar o passo do hash como falha dura (ou usar um caminho de hash que não dependa do autoload do 5.1) e provar isso com um teste de duas pontas, no mesmo padrão de `test-wrangler-pin.ps1`.

## 21/09: CFG-02 — os ~9 mil de "quarentena" eram descarte de ingestão, a quarentena real é 0, P1 aberto e não autorizado

> **Status:** ABERTO (P1 pendente de decisão do operador). **P2 e P3 CONCLUÍDOS e EM PRODUÇÃO desde 21/09, no Worker v4.9.261.** **Data:** 2026-09-21. **Origem do Registro:** validação independente do achado CVM reportado pelo TRAE, medida ao vivo, e depois implementação de P2 e P3 sem tocar a política de ingestão.
> **O que os números eram.** `cvm_atribuicao_quarentena` (9265 em 19/09, 9328 e 9426 em 21/09) e `cvm_atribuicao_cobertura_pct` (13,3 a 13,4) não mediam quarentena. O health fazia `quarentena = portal.total - portal.resolvidos`, que é `descartados_allowlist`, o descarte da ingestão ENET no último sync. É fluxo, recalculado a cada sync. E `cvm_atribuicao_por_cnpj` e `por_nome` recebiam contagens das vias zip e portal, não origem de atribuição. Este item substitui a leitura de CFG-02 como "quarentena crescendo" dos blocos de 18/09 e do parágrafo `Relacionado` de COBPARCIAL21, que ficam como registro do que se acreditava naquela data.
> **O que foi medido em 21/09 (19:05Z).** Health de produção: `cvm_atribuicao_por_cnpj 796`, `por_nome 654`, `quarentena 9426`, `cobertura_pct 13.3`. Fila real (`admin_cvm_quarentena`, lida por `scripts/check-quarentena-emissores.mjs`): `acervo=796 cobertura={cnpj:441, nome:355, quarentena:0, sem_dono:0} pct=100 entidades_em_quarentena=0`. A guarda saía 0 com "nenhuma das 0 entidades", aprovando conjunto vazio.
> **Por que a quarentena real é 0.** Documento sem dono continua sendo descartado na ingestão, antes de chegar a `cvm:documentos`, nas duas vias: `api/src/worker.js:8415` (sync do ZIP) e `:8597` (sync ENET), ambos com `if (!allow.emissor)`. `_atribuirDocumentoCVM` devolve `emissor:null` tanto para `quarentena` quanto para `sem_dono`, então as duas classes caem. Veio do commit `1459387` (13/09/2026, "aplicar allowlist e teto ordenado") e contradiz o desenho de SUBSTRINGDONO1, cujo comentário logo acima manda gravar o documento e deixar a atribuição para a leitura. O acervo caiu de 2252 documentos (01/09) para 796. Não medi se a queda atravessou o gate `enet_encolhimento_bloqueado` por rebase silencioso do piso de metodologia.
> **P2 e P3, implementados.** P2, o health passa a expor `cvm_atribuicao_por_cnpj/por_nome/quarentena/sem_dono/cobertura_pct` com origem medida sobre o acervo gravado (`_coberturaAtribuicaoAcervo`, invariante de soma igual ao total), e o descarte em campo próprio, `cvm_ingestao_descartados_sem_dono`. Meta legado no formato `{portal,zip}` sai `null`, nunca um número derivado do descarte. P3, `check-quarentena-emissores.mjs` reprova fila vazia com descarte positivo (código 3) e com descarte não medível (código 4), decisão pura em `scripts/lib/quarentena-guarda.mjs`. Testes T2, T3 e T4 novos em `api/test/cvm-atribuicao-health.test.mjs` e `api/test/quarentena-guarda.test.mjs`. Prova reversa do T2 medida contra o `worker.js` de `HEAD` (3 falhas, `expected 9426 to be +0`). Suíte da `api/` inteira: `Test Files 46 passed (46)`, `Tests 460 passed (460)`. `git diff --check` limpo.
> **P1, ABERTO e NÃO AUTORIZADO.** Parar de descartar documento sem dono na ingestão (`:8415` e `:8597`) muda a política de ingestão e o volume gravado em `cvm:documentos`. O `ESTADO.md` de 19/09 registra o filtro como intocado por ordem do operador. Nada disso foi alterado. **Pendente de decisão do operador.**
> **Deployado em 21/09 (medido, não esperado).** O v4.9.261 entrou em produção às 20:20:10Z: versão Cloudflare nova `edcf6f72-b311-477a-b080-de1809f9a318`, anterior `b2b88a39-344b-44c4-b85c-a2823154fbd3` (v4.9.260). O caminho foi `wrangler versions upload` → deployment `eaf44929` com a antiga a 100% e a nova a 0% → smoke no domínio real com `Cloudflare-Workers-Version-Overrides` (que devolveu `versao:"v4.9.261"`, `ok:true`, `cvm_atribuicao_quarentena:null` e `cvm_ingestao_descartados_sem_dono:9447`) → rollout `9b542e76` a 10% → 50% → 100%. Nas baterias sob tráfego real: 60 requests a 10% (divisão 55/5) e 60 a 50% (27/33), com zero não-200 e zero `ok:false`; analytics da Cloudflare com `errors:0` em 128 requests. Health final às 20:21Z: 40 de 40 em `v4.9.261`. **Até o próximo sync ENET NATURAL**, os campos de origem (`cvm_atribuicao_por_cnpj/por_nome/quarentena/sem_dono` e `cobertura_pct`) saem `null`, porque o meta em produção ainda tem a forma legada `{portal,zip}`; nenhum sync manual foi disparado. **Esse sync natural já chegou, às 20:27:28Z**, disparado pelo `VIXRadar-Sentinela` às 17:25 BRT ao detectar ZIP novo da CVM, e não por ação manual. O meta novo está populado e o health publica `cvm_atribuicao_por_cnpj 441`, `por_nome 357`, `quarentena 0`, `sem_dono 0`, `cobertura_pct 100` e `cvm_ingestao_descartados_sem_dono 9452`, com 441 + 357 = 798 igual ao `documentos` do acervo, que é a invariante do `_coberturaAtribuicaoAcervo`. A guarda manual passa a reprovar (3) enquanto a fila estiver vazia e o descarte for positivo. Ela não roda em workflow nenhum, então não gera alerta sozinha. A série 9265, 9328, 9426, 9431, 9447 e a série nova não são comparáveis. **Rollback:** `wrangler rollback b2b88a39-344b-44c4-b85c-a2823154fbd3`.
> **Condição de Obsolescência:** cai quando P1 for decidido e aplicado (documento sem dono passar a ser gravado e a fila `admin_cvm_quarentena` voltar a refletir o estoque), ou se o health voltar a publicar como `cvm_atribuicao_quarentena` um valor igual a `descartados_allowlist`.
...
> **Status:** FECHADO em produção (Worker `v4.9.259`), commits `8dd0066` (correção + testes), `75cac38` (guarda + script da skill) e `94398bd` (bundle), `main` = `94398bd`. **Data:** 2026-09-19. **Origem do Registro:** auditoria do handoff OPUS 103-104, que confirmou o diagnóstico operacional do 104 e provou que a seção 3 dele estava errada.
> **O defeito.** A guarda EMISSORSTALE1 (`frescor-check.yml`, em `main` desde `e7d200a`) lia `horas_stale` do `listar_plano_rotina`. Até a v4.9.258 esse campo saía de `_last_scanned_at` (`worker.js:12233` e `:12239`), carimbo que SKIP e varredura deferida também renovam, nos seis caminhos de escrita (`:10083, :10107, :10131, :10150, :10165, :10234`). O carimbo de análise real, `_ultima_analise_at`, existia desde sempre e só é gravado por `_carimbarAnaliseReal` (`:9907-9915`), que nunca carimba deferido. O rótulo `"Ultima analise: "` (`:12341`) e o `dados_para_analise` (`:21636`) mostravam a varredura como se fosse análise.
> **Por que nunca tocou, e é isto que importa para quem ler depois.** A guarda dispara por volta das **06:30Z** (o cron declara `37 1 * * *`, mas os runs agendados de 11 a 19/09 saíram entre 06:25Z e 06:58Z). A noturna roda 18h BRT, ou seja 21:00Z. Nesse horário o relógio de varredura marca cerca de **9h** para a carteira inteira, muito abaixo do limite de 24h, porque a varredura acabou de reiniciar o carimbo de todo mundo, inclusive de quem ela só olhou e não analisou. A guarda media o relógio que a própria rotina zera. Ela era estruturalmente incapaz de reprovar. Uma medição feita fora desse horário engana no sentido contrário: às 19:53Z de 19/09 o relógio de varredura marcava até 22,8h e ainda assim 0 de 104 acima do limite, enquanto o relógio de análise marcava 127,3h e 68 de 104 acima.
> **A correção (A1, Worker).** `horas_stale` passa a sair de `_ultima_analise_at` por `_horasStaleAnalise`. Sem carimbo, o fallback falha fechado com sentinela nomeada `HORAS_SEM_REGISTRO` (9999) e base própria `sem_analise_registrada`, nunca diluída no atrasado. `_carimbosAnaliseForaDaMescla` lê as semanas fora da janela de mescla de 3 semanas e rotula `ultima_analise_fora_da_mescla`. `horas_desde_analise` passou a derivar da mesma função, então não existem dois relógios de análise discordando no mesmo item. O relógio de varredura continua exposto em `horas_desde_varredura`. **Tiering e ranking da matinal seguem no relógio de varredura por decisão do operador**, não por acidente.
> **A correção (A2, guarda).** O gate exige `horas_stale_base` e falha alto se o campo não vier, em vez de medir varredura achando que mede análise. Atrasados contam só quem tem carimbo e passou de 24h; `sem_analise_registrada` e `ultima_analise_fora_da_mescla` viram contagens próprias; máximo e mais antigo ignoram a sentinela. O script `audit-routine-staleness.ps1` usa a mesma régua.
> **Causa raiz.** O campo prometia análise no nome e entregava varredura, e o consumidor leu pelo nome em vez de ler pelo escritor. Mesma família do `guarda_deve_ler_o_campo_que_o_pipeline_consome`: guarda tem que usar a régua do pipeline, conferida no escritor do dado, não no rótulo dele.
> **Guarda sistêmica.** `api/test/emissor-stale-analise.test.mjs`, 13 casos, com prova de duas pontas medida contra a v4.9.258 (`Tests 12 failed | 1 passed`, e o único que passa é o de tiering, que precisa passar nas duas versões). Depois da correção, `Tests 13 passed (13)`; suíte completa no gate de deploy, `Test Files 43 passed (43) / Tests 444 passed (444)`.
> **Validação semântica do jq, exigida pelo operador.** `bash -n` prova sintaxe, não comportamento, e o gate novo manda e-mail diário. Instalado o jq 1.7.1 (mesma versão do `ubuntu-latest`, `jq 1.7.1-3ubuntu0.24.04.2`), os trechos foram extraídos do próprio `.yml` entre os marcadores `EMISSORSTALE2-GATE` e `EMISSORSTALE2-ALERTA` e rodados com `curl` trocado por shim: 3 planos reais de produção e 4 sintéticos, **7 fixtures e 0 divergências**, com os três ramos exercitados (atrasado, sem registro e fora da mescla).
> **Medido depois do deploy, 21:08Z:** plano vivo com `horas_stale_base` em **104 de 104**, todos `ultima_analise`, `horas_stale_sem_registro=0`, `horas_stale_fora_da_mescla=0`, `horas_desde_analise` sem nenhuma divergência contra `horas_stale`. Script da skill: `stale_24h_real=75`, `stale_24h_inconclusivo=17`, `max_stale_hours=128.6` (Cosan, última análise em 14/09 12:36), com `horas_desde_varredura=24` nos dez mais antigos. Os dez foram varridos ontem e o relógio velho diria que estão frescos.
> **Consequência aceita pelo operador.** A carteira está faminta de verdade, então o `frescor-check` agendado passa a reprovar como CARTEIRA FAMINTA com cerca de 75 de 104 e a mandar 1 e-mail por dia até o P0 de orçamento ser resolvido. É o alarme funcionando, não regressão.
> **Condição de Obsolescência:** cai se `versao` do health deixar de ser `v4.9.259` ou se o plano voltar a sair sem `horas_stale_base`.

### Pendências abertas que nasceram ou ficaram visíveis nesta apuração

- **EMISSORSTALE3 (P3), tiering e ranking da matinal continuam no relógio de varredura:** decisão do operador em 19/09 foi corrigir só o campo e a guarda. O tiering (`montarPlanoRotina`, agora em `horasVarredura`) e o `stalenessBoost` de `selecionarEmissoresPrioritarios` (`worker.js:11877-11880`) seguem medindo varredura, então quem é deferido todo dia não ganha prioridade por estar sem análise. Mexer nisso muda quem entra na matinal e no aprofundamento, e deve andar junto com a decisão de orçamento.
- **EMISSORSTALE4 (P3), a exclusão de INCONCLUSIVO da guarda perdeu a justificativa:** o gate conta os inconclusivos à parte (17 hoje) e não os soma aos atrasados. A razão original caiu com a FIN1-REV. Decidir se inconclusivo há dias é atraso ou é categoria própria.
- **FRESCORCVM-TESTE1 (P3), `scripts/test-frescor-cvm.mjs` quebrado:** falha com `ReferenceError: _cvmDiasCorridosApos is not defined`. O mesmo arquivo extraído do `c0582bb` por `git archive` falha igual, então é anterior a esta sessão, provável desde a AVANCOFEED1 (01/09). Não é regressão do EMISSORSTALE2.
- **JANELADELTA1 (P2), commit B parado por decisão do operador:** `janela_delta_inicio` do prompt de rotina. Medido em 19/09 às 20:10Z, a janela muda em 64 de 104 emissores, de 1 a 4 dias a mais (mediana 3), nenhum no piso de 30 dias, e 343 dos 386 documentos enviados já são anteriores ao delta atual. Fica em branch lateral, com remedição depois do deploy A e decisão entre v4.9.260 ou esperar o P0 de orçamento.

## 18/09: auditoria Fases 1 e 3 corrigidas, Worker v4.9.258 e frontend em produção

> **Status:** FECHADO em produção (Worker `v4.9.258`, frontend `v202.43`), medido em `https://api.vixradar.com/` e por conteúdo em `https://vixradar.com/`. **Data:** 2026-09-18. **Origem do Registro:** auditoria de débito técnico (`PENDENCIAS.md` da raiz do repo), 39 achados em 5 fases.
> **Worker — o que subiu.** Commits `13f5573` (correções) e `f2eff04` (bundle). **CFG-01:** `wrangler` declarado em `dependencies` com versão exata 4.118.0. Antes ele entrava na árvore só por aresta de terceiro (devDependency do `@cloudflare/vitest-pool-workers` e peer **opcional** do `@sentry/cloudflare`), ou seja, a versão da ferramenta de deploy não era escolhida por ninguém. Guarda nova `scripts/lib/vixradar-wrangler-pin.ps1`, chamada no passo 0.0 do `deploy-worker.ps1` **antes de qualquer chamada ao wrangler**, e o arquivo não tem mais `npx` em linha executável. **LLM-01:** resposta HTTP 200 sem bloco de texto virou erro explícito de provedor, com contador próprio no KV e no Analytics Engine. **LLM-02/03:** política única de retry com backoff exponencial, jitter e orçamento de 120s por chamada.
> **Achado grave que a revisão adversarial pegou.** A correção do LLM-01, como estava, alimentava o disjuntor de provedor: 3 respostas vazias em 5 minutos abriam o circuito por 600s e a varredura passava a pular o provedor para **todas** as empresas seguintes. No código anterior isso não acontecia porque a resposta vazia não lançava. Corrigido com a marca `__respostaVazia`, que o catch do cascade lê para não registrar falha de disponibilidade. Prova de duas pontas com provedor controlado.
> **Frontend — o que subiu.** Commit `a44da04`. FE-01, FE-05, FE-06, FE-07, FE-08, FE-09 e FE-10 corrigidos. Validado **por conteúdo**: produção e repo dão **0 divergência** em 8 marcadores, e o HTML publicado é byte a byte igual ao do repo.
> **CFG-02 continua ABERTO, e crescendo.** `cvm_atribuicao_cobertura_pct = 13.3` e `cvm_atribuicao_quarentena = 9943` (eram 9811 na varredura da manhã de 18/09). É a prioridade de 19/09.
> **Riscos aceitos.** 429 não honra `Retry-After` — o Worker não lê esse header em lugar nenhum do caminho Anthropic, e o orçamento de tempo limita o estrago a 3 requisições por chamada. O contador de resposta vazia é aproximado: é `get` seguido de `put`, sem atomicidade, porque o KV não tem incremento.
> **Fragilidade observada.** `api/test/login-timing.test.mjs` falhou uma vez sob suíte completa (3755ms); passa 3/3 isolado e a suíte fechou verde na re-execução. Teste sensível a carga, sem relação com o diff desta rodada.
> **Guarda que ficou cega, para não repetir.** O `deploy-pages.ps1` detecta "conteúdo mudou sem bump de versão" comparando `app/index.html` com `app/deploy_zip/index.html`. Como a Fase 3 exigiu manter os dois sincronizados byte a byte, essa comparação deixou de acusar, e o frontend subiu com `CACHE_VERSION` **v202.43 inalterada**. Sem impacto prático medido: o HTML é servido com `Cache-Control: no-cache, must-revalidate`, então o navegador revalida sempre. Mas a guarda precisa de outro sinal.
> **Condição de Obsolecência:** cai se `versao` do health deixar de ser `v4.9.258`, ou se o HTML de produção divergir do repo em qualquer um dos 8 marcadores.

## 15/09 (madrugada): CVMSTITCH1 — feed congelado desde 11/09, causa raiz e correção

> **Status:** FECHADO em produção (Worker `v4.9.254`), medido em `https://api.vixradar.com/`. **Data:** 2026-09-15. **Origem do Registro:** card `t_63f950e8` do quadro, aberto depois de o gate `Frescor da Ingestao` reprovar em 14/09 com `FONTE ANDOU E O FEED NAO (pipeline_nao_persistiu)`.
> **Causa raiz (arquivo + linha).** `api/src/worker.js`, bloco `21369`-`21371` do handler `receber_analise`: o caminho anexava `cvm_documentos` (`_raSaneado.cvm_documentos = _dpaCvmDocs`) e persistia (`persistirResultadoCompartilhado`) **sem** chamar `costurarCvmEmEventos`. Era o único dos 5 caminhos que persistem payload sem costurar; os outros 4 (`executarVarreduraBatch`, `executarVarreduraBatchComFila`, `executarVarreduraMatinal`, `consulta_empresa`) chamam. Com `VARREDURA_CRON_AI_ENABLED=false` desde a v4.9.143, a varredura de IA saiu do cron do Worker para as rotinas locais e `receber_analise` passou a ser o único caminho de produção — e é justamente o que não costurava.
> **Efeito medido.** Documento oficial da CVM com dono na carteira, dentro da janela de 30 dias e material — a mesma régua que o gate usa como teto ELEGIVEL — entrava no estado e morria ali; só virava evento se o LLM resolvesse narrá-lo. `_cobertura_cvm` e `_sintetico` com **zero ocorrências** nos estados `radar:estado:2026-W28` a `W38`, contra **58 e 118 em W19/W18**. Em 14/09 a ISA Energia entrou no lote noturno por `cvm_delta_1` (Aviso aos Debenturistas de 14/09) e o estado dela ficou só com um evento de imprensa de `2026-09-08`.
> **Resolve a contradição do bloco de 11/09** deste arquivo ("não é dado inconsistente nem escritor parado; o mais recente é ausência real de evento novo com fonte dentro da cadência"): por medição, havia documento de 12/09 a 14/09 entregue, atribuído (`sem_dono=0`) e dentro da janela, e ele não virava evento por defeito do caminho de persistência. A entrada de 11/09 permanece como registro daquele dia.
> **Correção.** Uma linha: `costurarCvmEmEventos(_raSaneado, _raEmp, _raSaneado.setor, _raHoje, _raJanelaInicio)` depois de `cvm_documentos` ser preenchido e antes do `persistirResultadoCompartilhado`. Mesma janela, mesmos parâmetros dos outros 4 caminhos, nada de arquitetura nova, régua de frescor e `frescor-check.yml` intocados.
> **Provas.** Suíte completa local no worktree de deploy: **38 arquivos / 355 testes verdes**. Guarda de duas pontas `api/test/cvm-stitch-receber-analise.test.mjs` (5 testes), com **prova reversa medida**: desligando a costura, os 2 testes de ponta boa falham (`expected +0 to be 1`) e os 3 de ponta ruim passam. `Worker Tests` run `34935169965` verde. Produção: health antes `feed_evento_mais_novo:"2026-09-11"`, `feed_idade_du:2`, `feed_fresco:false`; depois `versao:v4.9.254`, `feed_evento_mais_novo:"2026-09-14"`, `feed_idade_du:1`, `feed_fresco:true`, `feed_ultimo_evento_novo_em:"2026-09-15T06:01:24.762Z"`; submissão real pelo contrato de rotina devolveu `n_eventos:2`, `max_data_evento_antes:"2026-09-08"`, `max_data_evento_depois:"2026-09-14"`; `Frescor da Ingestao` run `34935179373` **verde** com `AVANCO_FEED estado=saudavel_sem_fato_novo feed_max=2026-09-14 teto_elegivel=2026-09-14` e `FRESCOR_OK empresas=104 idade_h=0 evento_mais_novo=2026-09-14 idade_du=1 eventos=22`.
> **Pendência independente que ficou aberta (lado da rotina, não do Worker).** A noturna de 14/09 abortou no lote `light-4` por limite de sessão da assinatura: `analisados=45` de 104, `DEFERIDOS: ok=24 motivo=cap_efetivo`, `analisados=45 skip=35 deferidos=24`, 1 lote não processado e `dreno FALHOU (exit=5)`. Não foi a causa do feed parado (o feed andou com o mesmo quadro de rotina depois da correção), mas limita cobertura.
> **Condição de Obsolecência:** cai se `receber_analise` voltar a não costurar, ou se `feed_evento_mais_novo` ficar atrás de `teto_elegivel` com `pipeline_escreveu_apos_lote=true`.

## 11/09 (triagem readonly): classificação das pendências, nada fechado por documento

> **Status:** TRIAGEM REGISTRADA (o índice dessa rodada foi commitado depois em `dd37c8e`, CI `Worker Tests` run `34646171658` PASS, push `112ffbe..dd37c8e`), sem deploy, sem `git add`/`reset`/`checkout`/stash, sem scheduler tocado, sem secret criado ou rotacionado, sem workflow disparado. **Data:** 2026-09-11. **Origem do Registro:** rodada de triagem e validação pedida pelo operador com o índice congelado em 2 arquivos. **Condicao de Obsolecencia:** cai item a item, quando cada um fechar por execução real ou decisão registrada do operador.

As contradições documentais medidas estão detalhadas no bloco de 11/09 em `status/ESTADO.md`, seção `## Itens abertos`, com comando e saída de cada uma (VERIFHORARIO1, Braskem, TOKENCHAT1, 396×397, CLAUDE-FREE G1/G3, FALLBACKTTL1, tarefas `Disabled`×`Enabled`, Case F, `frescor-check` reprovando 3 dos últimos 8 runs). Classificação por natureza:

- **RESOLVIDO POR EVIDÊNCIA, sem ação pendente:** SENTINELA-SYNC1 (`api/src/worker.js:20295`, `admin_sync_cvm_auto`, v4.9.220); EMAILSILENT1 principal (v4.9.214, guarda `api/test/email-falha-silenciosa.test.mjs`); BRASKEMDETECT1/Braskem (guarda `api/test/gatilho-recuperacao.test.mjs`, deploy `af2abb19` da v4.9.227, produção hoje v4.9.244); fix do FALLBACKTTL1 em produção desde v4.9.225; e os dois fechamentos de 10/09 já pushados, `8b26d74` (Actions `checkout`/`setup-node`/`upload-artifact` @v4→@v6 + correção do quoting do Case F) e `112ffbe` (drift Pampa Sul: Usina Pampa Sul declarada no canônico `emissores-cnpj.mjs`), CI 100% verde (4/4 push-triggered), conforme `status/ESTADO.md`.
- **CÓDIGO PENDENTE:** QUARENTENACOB1 (hunk unstaged no `worker.js`, fora do índice); segunda metade do resíduo do EMAILSILENT1 (`api/src/worker.js:5989-5997`, lote com 1 falha retorna `enviados: 1` sem denunciar o destinatário perdido); DRIVERMORTO1 (pipeline de `market_cap` não existe; `upload_volatilidade_kv.ps1:94` proíbe o campo no payload); recuperação dos 101 (CURADORIA1 Marco 2); PISODIFF1 estrutural; SCANFALLBACK-MORTO1 (corrigir o consumo de `empresa`/`setor` antes de qualquer execução de prova).
- **CÓDIGO PRONTO, VALIDAÇÃO OPCIONAL EM FALTA:** Case F já corrigido em código no commit `8b26d74` (mesma forma de G/H) e a suíte gratuita `scripts/test-locks-overlap.ps1` passa `31/31 asserts OK` com essa correção; resta apenas a prova opcional do caso com `-ComTokens`, que a suíte padrão não exercita. E a ressalva que vale para toda esta rodada, suíte e teste focado rodam contra o working tree completo, nunca contra uma árvore isolada.
- **DECISÃO DO OPERADOR:** CCDOFFLINE1 (toggle de inicialização do Windows), TOKENCHAT1 (`Pages: Edit`), ROUTINEKEY-PLAIN1 (rotação pendente desde 07/08) e PLAIN2 (rotação recusada em 03/09), aprovação estética P4 de 06/09, DEFERIDO-BACKLOG1 se o backlog não drenar (aí é orçamento), e o provider pago do paraquedas (`scripts/scan-emergencia.mjs` chama `api.anthropic.com`, contra o `ANTHROPIC_API_PAYG = NÃO AUTORIZADO` do `CLAUDE.md`).
- **CONGELADO, SEM MEDIÇÃO RECENTE:** DEFERIDO-BACKLOG1, sem re-medição de `pontual_candidatos` desde 25/08.
- **MEDIÇÃO BLOQUEADA NESTA SESSÃO:** o Task Scheduler respondeu `Acesso negado` a `Get-ScheduledTask`, então G2, os triggers de VERIFHORARIO1 e o estado do CCDOFFLINE1 não foram confirmados por leitura direta; o que está registrado ali vem de documento, não de medição de hoje.

## 11/09, EMAILSILENT1 residual: entrega fechada em `dd37c8e` (CI PASS), ressalva de escopo da validação

> **Status:** FECHADO NO REPOSITÓRIO em `dd37c8e` (2 arquivos, +284/−16), CI `Worker Tests` run `34646171658` completed/success, push `112ffbe..dd37c8e`, `HEAD == origin/main`, sem deploy. **Data:** 2026-09-11. **Origem do Registro:** rodada de validação pré-commit da entrega EMAILSILENT1 residual. **Condicao de Obsolescencia:** satisfeita em 11/09 — o commit foi feito (`dd37c8e`) e a suíte canônica rodou verde no CI contra essa árvore, que tem só esses 2 arquivos de delta; o item cai.

**Entrega commitada em `dd37c8e` (o índice congelado desta rodada):** exatamente `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs` (`git diff --cached --name-status`, 2 linhas). `git diff --cached --check` sem saída, exit 0. `HEAD = 382e8c1`. Fora da entrega, de propósito: **QUARENTENACOB1** (hunks unstaged no `worker.js`, `scripts/check-quarentena-emissores.mjs`, `api/test/cvm-atribuicao.test.mjs`), **FALLBACKTTL1** (`api/test/fallback-ttl.test.mjs`, intent-to-add), **teste do monitor 402** (`scripts/test-monitor-degradado402.ps1`, intent-to-add) e os demais 23 unstaged / 112 untracked. Nenhum `git add`, `reset`, `checkout`, `commit`, `push` ou deploy foi executado.

**Ressalva de escopo, para não virar afirmação errada:** os `Test Files 34 passed (34)` / `Tests 339 passed (339)` de 11/09 e o focado `6/6` do `api/test/email-registro-rastro.test.mjs` foram medidos contra o **working tree completo**, nunca contra uma árvore isolada com só os dois arquivos do índice. O `worker.js` testado ainda carregava a mudança de QUARENTENACOB1 não staged, e a suíte também pegou testes intent-to-add/untracked. Prova: `git diff --cached` descreve o que o commit levaria, não o que rodou. **Não declarar a entrega validada isoladamente.** Isolar pediria worktree separado ou stash/checkout, que não foram feitos por proibição explícita nesta sessão.

**SCANFALLBACK-MORTO1 segue ABERTO (bloco de 29/08 abaixo), com o enunciado corrigido por medição em 11/09.** O registro antigo dizia que nenhuma execução com `prosseguir=true` tinha acontecido; não é verdade. `gh run list --workflow=scan-emergencia.yml` e `gh run view` (só leitura, nada disparado) mostram o run `34176257297` (08/09 01:19 UTC) com o passo `Scan de emergencia` **executado** e reprovado: `Fallback incompleto: 19 emissor(es) falharam.` e `[falha] undefined: Worker dados_para_analise: HTTP 400: {"ok":false,"erro":"empresa e setor obrigatorios."}`. Os runs de 10/09 (`34424970595`) e 11/09 (`34550366477`) têm o passo pulado, gate em `prosseguir=false` porque o estado estava fresco. O run de 29/08 (`33233411974`) falhou por `ANTHROPIC_API_KEY ausente`, antes da criação do secret.

**Causa raiz do 08/09, medida por leitura de código:** `scripts/scan-emergencia.mjs:207-208` lê `emissor.nome` e `emissor.setor`; `listar_emissores_prioritarios` (`api/src/worker.js:21041-21045`) devolve itens de `selecionarEmissoresPrioritarios` (`api/src/worker.js:11372`) como `{empresa, ews_score, materialidade_max, eco_count, staleness_boost, score_combinado}`, sem `nome` e sem `setor`; `dados_para_analise` (`api/src/worker.js:21059-21062`) exige os dois. Paraquedas armado e quebrado, não "não exercitado". Critério de saída é execução real com `prosseguir=true` chegando a `Processados: 19/19` sem `::error::Fallback incompleto`. Correção não feita nesta sessão, só diagnóstico. **Comando do operador para a prova real:** `gh workflow run scan-emergencia.yml --ref main` e depois `gh run watch`; alternativa é esperar o `schedule` (~01:20 UTC) e conferir `gh run view <id>`, exigindo que o passo `Scan de emergencia` apareça executado, não pulado. Observação para decisão dele: o script chama `api.anthropic.com` com `ANTHROPIC_API_KEY`, enquanto o `CLAUDE.md` registra `ANTHROPIC_API_PAYG = NÃO AUTORIZADO`.

## 11/09 (continuação), SCANFALLBACK-MORTO1 + EMAILSILENT1 residual: correção no working tree, sem commit

> **Status:** CÓDIGO CORRIGIDO NO WORKING TREE, testes adicionados, sem commit e sem deploy. **Data:** 2026-09-11. **Origem do Registro:** continuação da triagem após a causa raiz medida no bloco anterior. **Condição de Obsolescência:** SCANFALLBACK-MORTO1 só cai com execução real com `prosseguir=true` e `Processados: 19/19` sem `::error::Fallback incompleto`; EMAILSILENT1 residual cai quando a suíte canônica rodar verde na árvore que contém a correção.

**SCANFALLBACK-MORTO1:** `listar_emissores_prioritarios` passou a devolver `setor` somente quando ele existe em `SETOR_DE_EMPRESA` (sem fallback `|| "Outros"`); item sem setor canônico sai em `sem_setor` e não é enviado, e `scripts/scan-emergencia.mjs` passou a usar `emissor.empresa` + `emissor.setor`, contando ausência como falha. O item permanece **ABERTO**: a correção foi testada localmente, mas ainda não houve execução real autorizada com `prosseguir=true`; o critério de saída não é leitura de código.

**Prova determinística:** `node scripts/test-scanfallback-email.mjs` executou o handler real e reproduziu os dois lados do HTTP 400 antigo (`empresa: emissor.nome` e `setor: ""`) e o HTTP 200 do payload corrigido. A mesma cobertura foi adicionada em `api/test/scan-emergencia-contrato.test.mjs`. O runner Vitest não fechou neste ambiente (`esbuild/workerd` com `spawn EPERM`); a evidência local é o teste direto.

**EMAILSILENT1 residual:** `api/src/worker.js:5989-5997` agora acumula falhas por destinatário em lote e devolve `total`/`enviados`/`falhas`/`ids`/`erros`; `email_enviar` usa esse retorno e responde 207 em lote misto, preservando os sucessos. `api/test/email-lote-misto.test.mjs` cobre sucesso, falha e lote misto, e `scripts/test-scanfallback-email.mjs` passou 6/6, cobrindo ausência de setor canônico e falha total de lote. **Escopo:** esta metade de lote (`enviarResend` + `email_enviar` 207) segue no working tree, sem commit; a metade de `handleRegistrar` foi fechada em `dd37c8e`.

**Diagnóstico 396 × 397, sem correção:** `node scripts/check-metricas-curadas.mjs` devolveu `Carteira: 104 | Com card: 104 | No menu: 104`, `Pendencia declarada: 396 card(s) sem as_of/source_date/metric_type`, `OK` e zero falhas. O número 396 é a contagem de cards sem trio, não déficit de cobertura; 397 não foi reproduzido em 11/09. Fica registrado como contradição documental encerrada por medição, não por edição de histórico.

**Diagnóstico frescor-check, sem correção:** `.github/workflows/frescor-check.yml:71-105` mostra que a reprovação é `ev_idade > ev_limite`; `avanco_feed` é diagnóstico. O run mais recente registra `3 DU` contra `limite 2`. Não houve consulta live (`api.vixradar.com` falhou SSL/offline) nem execução de workflow; nada aqui altera a frente vermelha.

**Índice nesta continuação:** `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs` staged. Nenhum `git add`, `reset`, `checkout`, `stash`, commit, push, deploy, secret ou workflow foi executado nesta rodada; as alterações novas ficaram unstaged. **Fechamento posterior (11/09):** esses 2 arquivos foram commitados em `dd37c8e` (CI `Worker Tests` run `34646171658` PASS, push `112ffbe..dd37c8e`); o restante segue unstaged.

## 11/09 (segunda continuação), frescor-check apurado, 396×397 fechado, revisão de pendências

> **Status:** REVISÃO READONLY, sem deploy/`git add` nesta rodada; o fechamento da entrega veio depois, em `dd37c8e` (CI PASS). Índice desta rodada com `M api/src/worker.js` + `A api/test/email-registro-rastro.test.mjs`, commitados depois em `dd37c8e` (CI PASS, push `112ffbe..dd37c8e`). **Data:** 2026-09-11. **Condição de Obsolescência:** frescor-check cai quando a semântica da régua for decidida e testada; SCANFALLBACK-MORTO1 cai só com execução real autorizada.

**frescor-check — causa do vermelho mais recente.** Run `34570605397`, job `103171682021`: `companies=104`, `updated_at=2026-09-10T23:42:40.811Z`, `evento_mais_novo=2026-09-08`, `idade_du=3`, `limite_du=2`, `total_eventos=90`, `avanco_feed=saudavel_sem_fato_novo`. O run `34445952403` (10/09) passou com `idade_du=2` e `feed_max=2026-09-08`, `teto_elegivel=2026-09-05`, `max_data_entrega=2026-09-05`, `dentro_da_cadencia=true`, `escreveu_apos_lote=true`, `proxima_prevista=2026-09-13`. Runs de 08/09 (`34194909759`) e 09/09 (`34319869392`) tinham diagnóstico `pipeline_nao_persistiu`; o feed voltou a 09-08 até 10/09. Conclusão: **não é dado inconsistente nem escritor parado**; o mais recente é ausência real de evento novo com fonte dentro da cadência, e a régua de 2 DU dispara. Correção da régua exige decisão de produto e teste determinístico; fica **ABERTO**.

**396 × 397 — fechado por medição atual.** `node scripts/check-metricas-curadas.mjs --json` → `carteira=104`, `com_card=104`, `no_menu=104`, `cards_sem_datacao=396`, `falhas=[]`. 396 é o número corrente de cards sem trio; 397 não se reproduz. Nenhum dado foi alterado.

**FALLBACKTTL1 — fechado por prova direta atual.** Handler real com cascade falhando e KV em memória: 36h → 200, `_de_cache:true`, idade 36, score 0.3; 60h → 503 mesmo com chave presente. A suíte Vitest segue bloqueada por `spawn EPERM`, mas a prova direta cobre as duas pontas.

**EMAILSILENT1 residual — fechado por prova direta atual.** `node scripts/test-scanfallback-email.mjs` 6/6: sucesso total, falha unitária, lote misto 207, falha total 500 sem 207 parcial e ausência de setor canônico. A suíte Vitest não rodou neste ambiente; no CI do commit `dd37c8e` ela passou (`Worker Tests` run `34646171658`, completed/success).

**Pendências abertas revisadas:** `SCANFALLBACK-MORTO1` (falta execução real `prosseguir=true`/`19/19`); `QUARENTENACOB1` (código unstaged, sem validação do runner); `VERIFHORARIO1` (`Get-ScheduledTask` → `Acesso negado`); `Case F` (código já corrigido em `8b26d74` e suíte gratuita verde; resta a prova opcional com `-ComTokens`); `DRIVERMORTO1`, `CURADORIA1`, `PISODIFF1`, `ROUTINEKEY-PLAIN1`, `TOKENCHAT1`, `CCDOFFLINE1` sem fechamento nesta rodada. Nenhuma pendência foi encerrada por leitura de código.

**Gates:** Vitest focado falhou com `spawn EPERM`; `node --check` dos arquivos tocados = 0; `git diff --check` = 0; `git diff --cached --check` = 0; `scripts/lint-encoding.ps1` = 112 .ps1 + 5 shell OK, RISCO 0; `scripts/check-claude-free.ps1` = exit 0.

## 11/09, FALLBACKTTL1: lacuna de teste fechada (`api/test/fallback-ttl.test.mjs`)

> **Status:** FECHADO no repositório, sem deploy e sem tocar código de produção. **Data:** 2026-09-11. **Origem do Registro:** fechamento da lacuna declarada no bloco de 31/08 abaixo ("FALLBACKTTL1 não tem teste automatizado para o par `salvarCacheUltimoResorte`/`buscarCacheUltimoResorte`"). **Condicao de Obsolecencia:** cai se o corte de 48h da leitura ou o `expirationTtl: 86400 * 3` da escrita mudarem sem teste correspondente.

O fix do FALLBACKTTL1 (TTL de 24h → 72h na escrita, corte de 24h → 48h na leitura) está no ar desde a v4.9.225, mas **não tinha nenhum teste**: varredura de `api/test/` não achava arquivo tocando `CacheUltimoResorte` nem a chave `fallback:`. Regra 5 do `CLAUDE.md` pede guarda de duas pontas, e o par escritor/leitor do cache de último recurso era o único ponto do ciclo de 28/08 sem guarda.

**Prova de duas pontas, pelo caminho real do Worker** (`consulta_empresa`, que lê o fallback em `worker.js:21613` quando o cascade não devolve análise; no ambiente de teste o cascade falha porque `wrangler.test.jsonc` traz `ANTHROPIC_API_KEY` dummy):
- **ponta boa:** `fallback:Simpar` com `_fallback_ts` a **36h** → HTTP 200, `_de_cache:true`, `_cache_idade_horas` entre 35 e 37, `_score_confianca >= 0.3`. É o caso de uma varredura perdida (28/08): com o corte antigo de 24h isto devolvia 503.
- **ponta negativa:** o mesmo emissor com `_fallback_ts` a **60h** → HTTP 503 e `_de_cache` ausente, com o teste **provando antes que a chave existe no KV** (senão o 503 seria indistinguível de chave ausente).

**Execução:** `api$ ./node_modules/.bin/vitest run test/fallback-ttl.test.mjs` → `Test Files 1 passed (1)`, `Tests 2 passed (2)`, 6.78s. Nenhuma alteração em `api/src/worker.js`, nenhum deploy, nenhuma rotina executada.

**Nota de ambiente:** o `api/.env` local traz `ANTHROPIC_API_KEY` com texto de placeholder, e o fetch de teste reclamou de header não-ASCII (`x-api-key`). Não é incidente nem vazamento — valor de placeholder, request de teste isolado por Miniflare. Registrado para quem for mexer no `.env` depois.
...
**CASE F — CÓDIGO JÁ CORRIGIDO; pendente só a VALIDAÇÃO com `-ComTokens`.** A forma sem aspas saiu do código no commit `8b26d74`, o mesmo par de G/H: medido por leitura em 11/09, `scripts/test-locks-overlap.ps1:379` hoje usa `('-File', ('"' + $Runner + '"'), ...)`, idêntico às linhas 440 e 496. Ou seja, o defeito descrito abaixo **não está mais no arquivo**. O que permanece aberto é a execução que o caso exige: o Case F só roda atrás de `-ComTokens`, e a suíte padrão (`31/31 asserts OK` sem essa flag) não o exercita, então ninguém viu o caso passar desde a correção. Enquanto a execução com `-ComTokens` não acontecer e o caso não passar, este item fica aberto **como validação pendente** (P4, não bloqueante) — não como código a corrigir.
...
**Portões:** G1 setar `VIXRADAR_LLM_PROVIDER=none` em User; G2 desligar CCD/Remote; G3 `cutover-motor.ps1 -Acao Ativar` (elevação); G4 1 disparo real de cada rotina bloqueada; G5 monitor `-DryRun` + ciclos; G7 portão de verificação final — todos ABERTOS, com o operador. G6 FECHADO: commit `37bf0ec` (24 arquivos, push `main`), CI `claude-free` run 33930018633 verde, Gate 8 instalado (`install-hooks.ps1 -Force`; blobs do commit validados exit 0, reintrodução reprova R1). Nota: o commit inicial rodou o hook antigo sem Gate 8, coberto por validação pós-hoc. Prova de cada um no fechamento da sessão. Referências: `routines/README.md` (bloco Fase A), `status/ESTADO.md`, `CLAUDE.md`.
...
**O que a Fase 2 entregou e provou.** A rotina passou a receber `ultimo_evento_data`, `eventos_conhecidos` e `janela_delta_inicio` por emissor, nos cinco pontos de prompt dos dois motores. No dry-run, 4 de 5 emissores devolveram `eventos: []` citando exatamente o fato que já está no estado (Braskem 28/08, Oncoclínicas 27/08, Oi 25/08, Raízen 25/08), que é o comportamento que faltava. A janela de busca efetiva da Cemig caiu de 30 dias para 1.

**Defeito 1, CARTEIRA-PISO1 (P1, travava a noturna hoje à noite).** O commit `da4c2ba` adicionou a Usina Pampa Sul e o plano passou de 103 para 104 emissores. Os dois motores exigiam **exatamente** 103 e abortavam: o headless com `exit 5` em `run_vixradar_varredura.ps1:563` (medido, `ERRO: plano noturno com total 104 (esperado 103)`), e o CCD por instrução literal no Passo 3 da `SKILL.md`. Três testes do Worker também ficaram vermelhos pelo mesmo motivo. **Causa raiz:** o gate existia para barrar plano truncado, mas foi escrito como igualdade exata, o que na prática congela o tamanho da carteira e espalha um número mágico por dois motores mais a suíte. **Correção:** virou piso de 100 mais uma linha `CARTEIRA:` quando o total diverge da base conhecida. **Guarda:** prova de duas pontas medida, 12 e 99 abortam, 100/103/104 seguem.

**Defeito 2, FONTEDIVERG1 (P1, o feed podia mentir verde).** Na primeira passagem do dry-run, a Kora Saúde devolveu um evento com `data_evento: 2026-09-04` citando matéria cujo `article:published_time` no HTML é `2026-05-05`, quatro meses antes. Na segunda passagem o **mesmo prompt** devolveu a data certa e `eventos: []`, ou seja o erro é intermitente. **Causa raiz:** `validarDatasFontes` já detecta a divergência e marca o evento (`_data_fonte_divergente` + `_verif_forcar`), aceitando-o de propósito quando o domínio está em `_ehFonteConfitavelBloqueada`. Essa lista inclui InfoMoney, Valor, Exame, NeoFeed, Money Times, Seu Dinheiro, Braz Journal e Estadão, que é a imprensa que a rotina mais cita, então o caminho de aceite com marca é o comum, não a exceção. A fronteira do feed da Fase 1 não lia essa marca, então um fato velho carimbado com data de hoje avançava `feed_frontier_data` e deixava `feed_fresco` verde. **Correção (v4.9.240):** dois máximos separados, `_feedMaxNovoLocal` cru para a resposta e `_feedMaxCertificavel` sem os marcados para a fronteira e o carimbo, com `n_eventos_avanco_data` na mesma régua e log `[feed][FRONTEIRA_IGNORADA]`. O evento marcado continua persistindo e indo para verificação adversarial, ele só não atesta frescor. **Guarda:** casos D e E em `api/test/health.test.mjs`, com uma única variável isolada entre eles. Segunda guarda, nos cinco prompts: `data_evento` sai da fonte citada, lida no conteúdo, nunca da busca, porque ancorar a query no mês corrente estreita a busca e não data o resultado.

**Lição que fica.** O dry-run achou o que a revisão de código não achou, e a segunda passagem achou o que a primeira não achou. Rodar duas vezes e comparar não é redundância, é o único jeito de ver não determinismo em rotina que escolhe o próprio trabalho.

---

## 04/09 (madrugada), FEEDRETRO1. O feed estava parado há 5 dias úteis com tudo verde, a guarda foi corrigida, provada nas duas pontas e o feed reposto

> **Status:** FECHADO nas Fases 1 e 0 (guarda + reposição), com Fases 2 e 3 do plano ainda abertas (rotina incremental e resiliência de execução). **Data da Versão:** 2026-09-04. **Origem do Registro:** reclamação direta do operador ("o sistema não atualiza"), com print do painel em 04/09 01:14 BRT mostrando "Evento mais recente 28 de agosto de 2026 (5 dias úteis atrás)". Medição própria em log do Worker (Cloudflare Observability), logs de rotina de 28/08 a 04/09, transcripts das sessões CCD e 14 buscas web com verificação de data na fonte. **Condição de Obsolescência:** cai quando as Fases 2 e 3 entrarem, e a primeira noturna com o prompt de delta fechar com `eventos_avanco_data` registrado.

**O defeito não era o dado, era a régua.** Em 03/09 uma auditoria desta mesma casa fechou o feed parado como "correto, sem fato novo material", olhando a fonte CVM, onde de fato não havia Fato Relevante dos 103 na janela. **Essa conclusão estava errada e a reposição de hoje prova.** Havia quatro fatos de crédito materiais entre 31/08 e 03/09, todos fora do lote da CVM e todos em imprensa: Eneva concluiu a venda da Pecém II por R$ 839,2 milhões (31/08), Gerdau trocou a linha global de US$ 875 milhões por US$ 1,125 bilhão (31/08), AXIA Energia aprovou a 12ª e a 13ª emissões de debêntures de R$ 1,5 bilhão cada (02/09) e a Cemig D fechou o bookbuilding de R$ 2 bilhões (03/09). A rotina varreu os 103 emissores toda noite nesse período e não trouxe nenhum deles. A lição é que "a CVM não publicou" não é o mesmo que "não houve fato", e a auditoria anterior tratou como se fosse.

**Cadeia medida.** A noturna de 03/09 rodou (recuperação das 09h06, 103/103, `submits_aceitos=103`) e mandou 17 itens para a fila de verificação, todos datados de 05/08 a 28/08, nenhum de 31/08 em diante. O Worker não descartou nada: log de 01 a 04/09 tem `[sanitizar][FORA_JANELA]`=0, `SEM_DATA`=0, `DATA_APROXIMADA`=0, `RUIDO`=0, `DATA_FUTURA`=0, `DESCARTADO_FONTE_INACESSIVEL`=0, contra `[persistir][MERGE]`=54 e `[cobertura][PRESERVADO]`=66. Ou seja, evento velho sendo fundido com evento velho, com `updated_at` subindo a cada escrita, que é o que fazia "Painel atualizado em" avançar sem fato novo. Causa raiz do lado da rotina: o prompt pede evento na janela de 30 dias e nada pergunta "o que saiu desde a última análise", então o modelo devolve o fato mais relevante do mês, que é sempre o de agosto. Causa raiz do lado da guarda: `painel_fresco` mede escrita, `avanco_feed` (AVANCOFEED1, decisão minha de 01/09) declarava `saudavel_sem_fato_novo` porque o feed estava no teto da CVM com a CVM dentro da cadência semanal, e a régua de idade de evento (`EVENTO_MAX_DU=2`) tinha virado `::warning::`. As três diziam verde ao mesmo tempo.

**Correção, Worker v4.9.237 (commit `dea0e15`, deploy `da3dbaa`).** GET público expõe `feed_evento_mais_novo`, `feed_idade_du`, `feed_fresco` (mesma régua de `checks.evento_mais_novo`, `EVENTO_MAX_DU=2` inalterado) e `feed_ultimo_evento_novo_em`. Este último é carimbo de avanço REAL: `persistirResultadoCompartilhadoInterno` mantém `estado.feed_frontier_data`, fronteira GLOBAL do feed, e só carimba quando a data máxima nova supera a anterior. Chave nova de fato já conhecido não carimba, e isso é o ponto, porque o dedup é por `data|empresa|host+path` e a mesma notícia com URL diferente entra como chave nova sendo fato velho. `carregarEstadoCompartilhado` passou a devolver os dois campos, senão o carimbo seria escrito e perdido na leitura seguinte. Tudo fora de `_okHealth` (HEALTHSPLIT1). `frescor-check.yml` reverteu a demoção do AVANCOFEED1: idade do evento volta a dar `::error::` e `exit 1`, e `avanco_feed` segue calculado, entrando no texto do erro como diagnóstico. Frontend v202.37 (bump pelo `bump-cache-version.ps1`, CACHEBUMP1): rodapé mostra âmbar e "Sem fato novo há N dias úteis" quando `feed_fresco:false`, em vez de "Sistema operacional" lendo só `d.ok`.

**Prova negativa, exigida pelo operador antes de qualquer reposição** (guarda no ar, feed ainda parado): health devolveu `feed_evento_mais_novo:"2026-08-28"`, `feed_idade_du:5`, `feed_fresco:false`; `gh workflow run frescor-check.yml --ref main` (run `33844055516`, conteúdo do workflow conferido em `origin/main` antes do disparo) reprovou com `##[error]Feed sem evento novo ha 5 dias uteis (limite 2), mais novo em 2026-08-28, 211 eventos no estado. avanco_feed=saudavel_sem_fato_novo` e `exit code 1`; rodapé do painel em âmbar `rgb(245, 158, 11)` com o texto "Sem fato novo há 5 dias úteis". A linha do erro é a prova mais limpa do incidente, porque mostra a guarda antiga dizendo `saudavel_sem_fato_novo` no exato momento em que o feed estava parado há 5 dias úteis.

**Reposição (Fase 0 da skill `repor-varredura`).** Quatro fatos com data verificada na própria fonte (`article:published_time`/`datePublished` lidos no HTML, nunca do resumo da busca), todos HTTP 200: Cemig 03/09 (`2026-09-03T22:08:30Z`), AXIA/Eletrobras 02/09 (`datePublished 2026-09-03T00:04:47-03:00`, data do fato no corpo), Eneva 31/08 e Gerdau 31/08 (infomoney, `article:published_time 2026-09-01`). `repor-varredura.ps1 -EnvioDireto` fechou `FIM: submit_ok=4 de 4 (eventos persistidos)`, com `n_eventos=1` e `removidos_pre_verificador=0` em cada, exit 0. Nenhum evento foi forçado, nenhuma data foi ajustada e nenhuma fonte foi trocada só para passar no sanitizador.

**Prova positiva.** Health: `feed_evento_mais_novo:"2026-09-03"`, `feed_idade_du:1`, `feed_fresco:true`, `feed_ultimo_evento_novo_em:"2026-09-04T06:32:42.125Z"`. `frescor-check` run `33844843116` passou, `FRESCOR_OK empresas=103 idade_h=0 evento_mais_novo=2026-09-03 idade_du=1 eventos=215`. Rodapé do painel voltou a verde `rgb(16, 185, 129)`. `dados_para_analise` confirma o evento de setembro em cada um dos quatro emissores. **E a prova mais fina, em produção:** o log do Worker registrou `FRONTEIRA_AVANCOU` exatamente **1 vez** nas quatro submissões, não quatro. A Cemig (03/09) avançou a fronteira de 28/08 para 03/09, e as outras três (02/09, 31/08 e 31/08) não carimbaram porque suas datas não superavam a nova fronteira. É o comportamento pedido pelo operador funcionando com dado real, não em teste.

**Guardas.** `api/test/health.test.mjs` ganhou 5 casos, prova de duas pontas: `feed_fresco` com evento de 10 dias (false) e de hoje (true), e os três casos do carimbo via POST `receber_analise` real, evento velho com chave nova não avança, data maior avança, duplicata da mesma data máxima com URL alternativa não avança. Suíte completa 26 arquivos, 246 testes, verde. O gate que impede a repetição é o `frescor-check` reprovando por idade de evento, provado reprovando de verdade antes da reposição.

**FASE 2 ENTREGUE em 04/09 (v4.9.238, commit `e4d85af`, deploy `709931a`).** O plano do Worker passa a entregar, por emissor, `ultimo_evento_data` (maior `data_evento` já salva), `eventos_conhecidos` (até 10, com data, título em 80 chars e host da fonte) e `janela_delta_inicio` (última análise menos 2 dias, nunca antes da janela de 30 dias). Medido em produção logo após o deploy: 103/103 emissores com `janela_delta_inicio`, 79/103 com `ultimo_evento_data` (os 24 sem são os que não têm evento salvo, o que é correto), e a CEMIG voltou `ultimo_evento_data=2026-09-03`, `janela_delta_inicio=2026-09-02` contra `janela_inicio=2026-08-05`, com 7 eventos conhecidos listados. A janela de busca efetiva caiu de 30 dias para 1 dia nesse emissor, que é exatamente o ponto.

Os 5 pontos de prompt mudaram juntos (SKILL.md do noturno e da matinal, que ficam fora do repo em `~/.claude/scheduled-tasks`, mais `noturno-batch-haiku.md`, `matinal-batch-sonnet.md` e o cabeçalho de `New-BatchPrompt`): âncora de recência na primeira busca, proibição de recriar fato já listado em `eventos_conhecidos` mesmo com URL diferente, e continuação de saga tratada como evento NOVO com a data de agora.

`receber_analise` devolve o que antes só existia em `console.log`: `n_eventos_avanco_data`, `n_chaves_novas`, `n_eventos_conhecidos`, `max_data_evento_antes`/`depois` e `descartes` com os 5 motivos do saneamento mais `fonte_inacessivel`. **Correção de semântica feita durante a implementação, a pedido do operador:** `n_eventos_avanco_data` conta DATAS DISTINTAS que superam o máximo anterior, não eventos. Dois fatos legítimos e diferentes na MESMA data nova contam 1, não 2. A primeira implementação contava eventos e teria dobrado nesse caso, contradizendo o próprio comentário que eu havia escrito ao lado. Quem responde "quantos fatos novos" é `n_chaves_novas`, medida separada.

Motor local: ledger ganhou `n_avanco_data` como ÚLTIMO campo (não desloca os 6 índices que `Get-VixLedgerEmissoresNaJanela` e `Test-VixLedgerEntregueNaJanela` leem por posição), a linha `FIM:` ganhou `eventos_avanco_data`, `chaves_novas` e `descartados`, e a guarda `DESCARTADO|` de `repor-varredura.ps1` foi portada: submit com evento enviado e `n_eventos=0` agora aparece no log com o motivo, em vez de contar como sucesso pleno.

Guardas: 5 casos novos em `api/test/health.test.mjs`, exatamente os pedidos (evento repetido dá 0, chave nova com data antiga dá 0, data maior dá 1, segundo fato na mesma data nova não dobra, `n_chaves_novas` testado separado). Suíte 26 arquivos e 251 testes, verde. O caso 5 reprovou na primeira execução e a causa era o fixture, não o código: `_chaveDedupEvento` usa `data|empresa|fonte_base` e o seed não tinha `empresa` dentro do evento, campo que o pipeline real sempre preenche antes de persistir. Corrigido o teste, não o código.

**Aberto, Fase 3 do plano aprovado.** Fase 2, rotina incremental (ENTREGUE, ver acima): `montarPlanoRotina` passa `ultimo_evento_data`, `eventos_conhecidos` e `janela_delta_inicio`, os 3 prompts ganham âncora de recência e proibição de recriar fato conhecido, `receber_analise` devolve `n_eventos_avanco_data` (nome escolhido pelo operador, significa "eventos que avançam o máximo temporal do emissor", nunca "quantidade de fatos novos") e `n_chaves_novas` separado, e a guarda `DESCARTADO` de `repor-varredura.ps1` é portada para o motor e para as duas SKILLs. Fase 3, execução: política de session limit com espera pelo reset até 150 min e escalada forçada para a chave paga (a regex de `Test-VixClaudeAuthFailure` não cobre `hit your ... limit`, por isso a escalada nunca disparou em 03/09 às 21h38), `ExecutionTimeLimit` PT4H e `MultipleInstances IgnoreNew` na task de retry, idempotência de janela deixando de contar `DEFERIDO` como entregue, e RECOVERY-JANELA1 portado para a matinal.

---

## 03/09 (noite), AUTOMODE-SETUP1 aplicado no settings do projeto, ROUTINEKEY-PLAIN2 achado e redigido no settings.local.json, AUTOMODE-SHELL1 aberto

> **Status:** AUTOMODE-SETUP1 FECHADO. ROUTINEKEY-PLAIN2 REDIGIDO e **rotação RECUSADA pelo operador em 03/09/2026 20h55 BRT**, a chave exposta continua em uso por decisão explícita dele. AUTOMODE-SHELL1 com pré-condição CUMPRIDA em 03/09 20h57 BRT, falta só a prova de execução agendada. **Data da Versão:** 2026-09-03 (atualizada às 21h05 BRT). **Origem do Registro:** duas propostas do wizard `/auto-mode-setup --wizard posture=mixed scope=project depth=both --propose` (request-ids `da06e099` e `9176573e`), decisão delegada ao agente pelo operador. **Condição de Obsolescência:** PLAIN2 só reabre se o operador mudar de ideia sobre a rotação ou se houver sinal de uso indevido da chave. SHELL1 cai quando uma matinal ou noturna agendada fechar sem `permission_denials` de PowerShell.

**AUTOMODE-SETUP1.** `.claude/settings.json` do projeto (estava `{}`, ignorado pelo git por `.gitignore:39`, então a postura não vai ao repo público) recebeu bloco `autoMode` mesclado das duas propostas. Base P2 (repo PÚBLICO, `main` sem proteção nem ruleset, domínios do projeto, deploy só pelos dois scripts, nota de que o bloco antigo do settings do usuário está obsoleto) com os acertos de P1 (Key internal services = Worker, KV, DOs e Analytics Engine, org CLIs wrangler e claude headless) e correções próprias (dados.cvm.gov.br, ANBIMA, SEC e B3 como fontes externas read-only e não serviço interno, definições vivas de rotina no CCD store e em `C:\Users\User\.claude\scheduled-tasks\vixradar-*\SKILL.md`, tasks VIXRadar-* que devem ficar Disabled, sessões paralelas no mesmo repo). `allow` com 9 regras (defaults, deploy scripts, npm test e vitest, curl GET do portão nos dois hosts, scripts test/lint/check, leituras de logs e dados, gh read-only, dot-source da lib). `soft_deny` com 11 (defaults, wrangler deploy cru, `producao/`, `git commit --no-verify`, `git push` direto, KV/secret/DNS delete, Enable/Register de task VIXRadar-*, editar SKILL.md de rotina, POST mutante fora das rotinas, apagar logs, matar processo alheio). `hard_deny` com 4 (segredo em argumento/URL/log/repo/prompt, commit de arquivo sensível, force-push em main, deploy de `producao/`). JSON validado por `ConvertFrom-Json` (environment=25 allow=9 soft_deny=11 hard_deny=4). Sem `permissions.defaultMode` no projeto, o `auto` já vem do usuário.

**ROUTINEKEY-PLAIN2 (P1, redigido, rotação pendente).** Ao ler `.claude/settings.local.json` para conferir sobreposição com o projeto, achei o VALOR do `ROUTINE_API_KEY` em texto puro dentro de 21 regras de `permissions.allow`, comandos `curl` de sessões antigas aprovados com a chave no corpo `-d`, mesma família do ROUTINEKEY-PLAIN1 (07/08, 4 arquivos vivos redigidos, chave não rotacionada). O arquivo é gitignored, então não foi ao repo, mas fica em disco em claro e é carregado por toda sessão. Correção, round-trip JSON removendo 32 regras, 21 com `routine_key`, 1 `Bash(npx wrangler:*)` (allow cru que furava o portão de deploy e o classificador), 9 kills por PID fixo ou `taskkill /F /IM python.exe` (PID reciclado hoje pode ser a própria noturna), 1 `Enable-ScheduledTask 'VIXRadar-Matinal'` (reabilita task que tem que ficar Disabled). De 266 para 234 regras, estrutura intacta (additionalDirectories 1, enabledMcpjsonServers 2, skillOverrides 29, hooks.Stop 1). Backup `.claude/settings.local.json.bak-20260903-190053` com o valor substituído por `REDACTED` no próprio backup. Varredura local em seguida (Select-String, case-insensitive) achou o valor também em 25 backups de prompt de junho e julho em `C:\Users\User\.claude\scheduled-tasks\backups\` (67 ocorrências, inclusive linhas `ROUTINE_KEY=<valor>` em maiúsculas que a primeira passada case-sensitive deixou passar, lição registrada), todos redigidos para `REDACTED`, e zero nos arquivos vivos de `scheduled-tasks`. Mais 5 cópias do `settings.local.json` dentro de `.claude\worktrees\*\.claude\` (worktrees de isolamento de agente), cada uma com as mesmas 21 ocorrências, limpas com o mesmo round-trip (32 regras fora em cada). Varredura final case-insensitive em scripts, routines, logs/routines, vault, status, .github, api/test, api/tools, wrangler.toml, api/src, app/js, CLAUDE.md e README.md, zero ocorrências. Nenhum valor foi impresso em log, comando ou chat. **A única correção real é rotacionar a chave**, o valor sobrevive em backups e transcripts append-only de sessões anteriores. Decisão do operador, afeta noturna, matinal, verificação, sentinela, agenda semanal, retries e as duas Claude Code Routines remotas. **Decisão tomada em 03/09/2026, 20h55 BRT, o operador recusou a rotação e mandou manter a chave atual** ("pode deixar essa routine ai"). O risco residual fica aceito e declarado, quem tiver o valor antigo consegue POST autenticado nos 8 handlers de rotina do Worker até uma rotação futura. Não reabrir este item em sessão nova como se fosse pendência de agente, é postura escolhida. Reabrir só com fato novo, uso indevido da chave, vazamento para fora do disco local ou pedido do próprio operador.

**AUTOMODE-SHELL1 (P3, aberto).** As duas propostas pediam remover `Bash` e `PowerShell` crus de `permissions.allow` do settings do USUÁRIO para o classificador julgar shell. Não feito agora. Motivo medido, as sessões agendadas do Claude Desktop (`vixradar-noturno`, `vixradar-matinal`, `vixradar-verificacao-async-11h` e `-1845`) não têm PowerShell em `approvedPermissions` no `scheduled-tasks.json` e rodam sem humano, hoje funcionam porque o allow cru do usuário cobre. O caminho headless do retry não depende disso (`run_claude_routine.ps1` passa `--permission-mode dontAsk --allowedTools PowerShell,Bash,Read,Write,Edit,Agent,WebSearch,WebFetch`). Pré-condição para remover, adicionar PowerShell (e Bash) às `approvedPermissions` das 4 tasks CCD pelo MCP `update_scheduled_task`, observar uma execução agendada completa sem `permission_denials`, então remover os dois allows crus do usuário e reexecutar o wizard com scope=user para trocar o bloco `autoMode.environment` obsoleto de lá (ainda diz repo privado, sem remotes, branch desconhecida).

**Pré-condição cumprida em 03/09/2026, 20h57 BRT, a pedido do operador ("aprove o powershell para as rotinas").** Correção de fato na frase acima, o MCP `update_scheduled_task` NÃO consegue conceder permissão, o schema dele só aceita `prompt`, `description`, `cronExpression`, `fireAt`, `enabled` e `notifyOnCompletion`. `approvedPermissions` só existe no store `%APPDATA%\Claude\claude-code-sessions\d41350ae-0294-4c9f-be6e-23616abe4c52\793c6475-0ea8-4556-907c-0b449cf4bc92\scheduled-tasks.json`, então a concessão foi por edição do arquivo, round-trip JSON em pwsh 7, `{ "toolName": "PowerShell" }` sem `ruleContent` (equivale a liberar a ferramenta inteira) nas 4 tasks. Só PowerShell, Bash ficou como estava (a noturna segue com `Bash` restrito a `npx vitest *`). Contagens depois, matinal 8 permissões, noturno 7, verificação-11h 6, verificação-1845 6, uma entrada PowerShell em cada, JSON revalidado, backup do arquivo original no scratchpad da sessão. **Medição que derruba o medo do restart**, editar o store à mão sobrevive a escrita do app, forcei uma escrita pelo MCP logo depois (mudança de `description` idêntica na task de 11h, arquivo regravado às 20:57:46) e as 4 entradas continuaram lá, ou seja o processo faz read-modify-write do arquivo, não despejo cego da memória. Isso prova durabilidade, não prova efeito, se o launcher lê o store a cada disparo a permissão já vale, se cacheou na ativação só vale depois de reiniciar o Claude Desktop. **Prova pendente**, a matinal agendada de 04/09 às 10h06 BRT é a primeira execução CCD depois da mudança, conferir o log dela e a ausência de negação de permissão de PowerShell. Passo seguinte segue igual, com a prova verde, remover `Bash` e `PowerShell` crus do `permissions.allow` do settings do usuário e rodar o wizard com scope=user.

---

## 03/09 (noite), AUDITORIA INDEPENDENTE DO FEED PARADO EM 28/08. Conclusão B confirmada por medição própria (nenhum fato material 29/08 a 03/09 para os 103) e dois resíduos novos, RECOVERY-JANELA1 CORRIGIDO na mesma noite (skill + lib + teste, sem deploy), SENTINELA-DIVIDA1 aberto

> **Status:** FECHADO como "feed correto, sem fato novo material". Refaz o INCIDENTE-FRESHNESS1 por medição independente em vez de releitura do registro, e chega ao mesmo número. Nenhum backfill, nenhum deploy, nenhuma data forçada. Dois resíduos novos abaixo (RECOVERY-JANELA1 P2, SENTINELA-DIVIDA1 P3), nenhum toca o dado. **Data da Versão:** 2026-09-03. **Origem do Registro:** missão `/vix-radar-general-audit` do operador. Medição ao vivo em health público (`versao:v4.9.236`), CSV bruto `data/cvm/ipe_2026/ipe_cia_aberta_2026.csv` (LastWriteTime 30/08 07:00:38), run `33723201815` do `frescor-check.yml`, logs `logs/routines/*_20260829..20260903`, 11 buscas web independentes. **Condição de Obsolescência:** cai no lote CVM de 06/09 (se trouxer Fato Relevante dos 103, `feed_max` tem que andar) ou quando RECOVERY-JANELA1 for tratado.

**Prova por fonte (item 7 da missão).**

1. CVM, fonte primária, lote semanal. Arquivo bruto regenerado em 30/08 07:00 (bate com `cvm_fonte_last_modified`), 33.026 linhas. Filtro `Data_Referencia >= 2026-08-29` devolve 585 documentos, 442 Calendário de Eventos Corporativos, 96 Assembleia, 14 Dados Econômico-Financeiros, 10 cadastrais, 7 Comunicado ao Mercado, 7 Reunião da Administração, 4 Aviso aos Acionistas, 2 Partes Relacionadas, 1 Informação a Bolsas Estrangeiras, 1 Relatório Proventos e um único "Fato Relevante", de ÁGUAS DO RIO 1 SPE S.A. (fora dos 103), com `Data_Referencia=3026-03-30` (erro de digitação da fonte) e `Data_Entrega=2026-03-30`. Máximo real de `Data_Entrega` do lote inteiro, 2026-08-29. Zero Fato Relevante dos 103 no período.
2. Régua do próprio pipeline, em produção (run `33723201815`, 03:26 BRT de 03/09): `AVANCO_FEED estado=saudavel_sem_fato_novo feed_max=2026-08-28 teto_elegivel=2026-08-28 teto_acervo=2026-09-01 max_data_entrega=2026-08-28 dentro_da_cadencia=true escreveu_apos_lote=true referencia_lote=2026-08-30T23:59:59.000Z proxima_prevista=2026-09-06 docs=2111 elegiveis=753 sem_dono=1358 data_futura=0` e `FRESCOR_OK empresas=103 idade_h=16 evento_mais_novo=2026-08-28 idade_du=4 eventos=198`. O `teto_acervo=2026-09-01` é Calendário de empresa fora dos 103, o mesmo que o item 1 mostra.
3. Imprensa e rating, 11 buscas web nesta sessão. Nada material de 29/08 a 03/09 para os 103. Os fatos grandes da vizinhança já estão no estado com a data do FATO, não da publicação. Oi, falência confirmada pelo TJ-RJ em 25/08 (`OK|2026-08-25|oi|rad.cvm|APROVADO` na verificação de 03/09). Braskem, Fitch RD em 17/08, pedido de recuperação extrajudicial em 24/08, deferimento judicial em 28/08 (`OK|2026-08-28|braskem|rad.cvm|APROVADO`), e o Form 6-K da SEC de 31/08 é data de publicação, não virou `data_evento`, como manda. Oncoclínicas, minuta do plano em 26/08. Esse 28/08 da Braskem é exatamente o `feed_max`. Os 17 emissores sem análise desde 31/08 no máximo (lista no resíduo 2) também sem notícia material.
4. Cadeia interna, medida pelos logs. Noturna `FIM 103/103` em 29/08, 30/08, 31/08 e 01/09 (33 analisados, 69 deferidos), falhou em 02/09 (FRESHNESS2) e foi recuperada em 03/09 09:33 (58 analisados, 45 deferidos, 39 eventos gravados, 3 CRITICO, Braskem, Oi e Oncoclínicas). Matinal com FIM todo dia. Verificação aprovou 18 de 20 (30/08), 15 de 18 (31/08), 10 (01/09), 16 de 17 (03/09), e todos os itens têm `data_evento <= 2026-08-28`. Frontend calcula "Evento mais recente" como `MAX(data_evento)` do `op=state` (`app/index.html:3755-3776`), mesma régua do Worker, e separa esse carimbo do "Painel atualizado em" (`updated_at`).

**Estágios inspecionados e descartados por medição.** Coleta (lote de 30/08 ingerido, `cvm_fonte_ultimo_sync_ok_em` de hoje), parsing e normalização de data (`normalizarDataEvento` rejeita formato ruim e data futura, o FR de 3026 cai aí), mapeamento de emissor (`por_cnpj=724`, `quarentena=1325`, os documentos >= 29/08 são de fora dos 103), filtro dos 103 e materialidade (585 documentos são calendário e assembleia), dedup e persistência (`escreveu_apos_lote=true`, 39 eventos gravados hoje), leitura de estado, API e frontend (mesma `MAX`). Nenhum estágio quebrado. Não existe evento válido >= 29/08 sendo perdido.

**Item 8 da missão (telemetria).** Já atendido pelo FRESHNESS2 de hoje. `painel_atualizado_em`, `painel_idade_min`, `painel_fresco` e `painel_regra` no `GET /` medem a ROTINA, `checks.evento_mais_novo` e `checks.avanco_feed` no health detalhado medem o FATO, `canonical-test` reprova por `painel_fresco=false` e `frescor-check` por `pipeline_nao_persistiu`, e a tela mostra os dois carimbos lado a lado. `painel_fresco:true` com `feed_max=28/08` é o estado esperado de "rotina rodou hoje, fato continua antigo". Melhoria opcional, P3, não feita, espelhar `evento_mais_novo` e `avanco_feed.estado` no `GET /` público lendo um resumo em KV gravado pelo healthcheck diário, para não custar leitura do estado a cada health.

### Resíduos novos (nenhum toca o dado)

1. **RECOVERY-JANELA1, P2, CORRIGIDO 03/09 à noite (ver parágrafo "Correção" no fim deste item). Registro original do achado, como escrito às 18h30:** A recuperação manual da noturna às 09:07 a 09:33 fechou o ledger do DIA com 103 linhas `OK|`, e a invocação agendada das 18:15 saiu por idempotência sem submeter nada (`logs/routines/vixradar-noturno_20260903.log:168-173`, "Nenhum submit novo nesta invocacao"). `updated_at` do estado ficou em `2026-09-03T14:17:01Z` (verificação das 11h), confirmado no health às 21:33Z. Consequências previstas. (a) `Szuchmacher-RetryVixNoturno` às 21:30 julga por `Test-VixLedgerEntregueNaJanela` (`scripts/lib/vixradar-watchdog.ps1:99-111`, só conta linhas >= 18:00), não acha nada, relança, o relançamento cai na mesma idempotência (exit 0, zero submit) e o retry alerta "não entregou". Controle positivo da mesma régua hoje de manhã em `retry-vixradar-matinal_20260903.log`, "entrega confirmada dentro da janela (ledger=20/12 fim_na_janela=True)". (b) `_painelSlaAtivo` (`api/src/worker.js:11990-11994`) passa a exigir `updated_at >= 03/09 18:00 BRT` a partir de 04/09 01:30, `painel_fresco` vira `false` e o `canonical-test` (`15 */6 * * *` UTC, 03:15 e 09:15 BRT) reprova duas vezes até a matinal das 10h escrever. Só não acontece se a verificação das 18h45 ou a remota das 02h07 processar pelo menos um item (às 18:33 a das 18h45 ainda não tinha rodado). Causa raiz, a idempotência da skill é por dia civil e o SLA e o vigia são por janela. Os dois lados do FRESHNESS2 (A4/H e PAINEL_SLA) entraram hoje, e hoje foi o primeiro dia com recuperação matinal, o refutador não tinha como ver. Correção recomendada, idempotência por JANELA agendada (a invocação 18h ignora `OK|` anteriores a 18:00 e processa quem falta, o que ainda dá a vez aos 45 deferidos com cap novo de sessão). Alternativa mais barata e pior, vigia e SLA aceitarem ledger completo do dia, perdendo a cobertura da tarde. Exige editar o SKILL.md da noturna (CCD store, fora do repo), decisão do operador. Guarda proposta, teste do watchdog com ledger matinal completo e janela 18h vazia continua reprovando (comportamento atual, correto), e teste da skill com o mesmo cenário tem que SUBMETER.
   **Correção (03/09 noite, decisão do operador via `/goal`).** Idempotência da noturna passou a ser por JANELA, mesma régua do vigia e do PAINEL_SLA. (a) `scripts/lib/vixradar-watchdog.ps1` ganhou `Get-VixJanelaInicioRotina` (a partir das 18h a janela é 18:00 até o fim do dia, antes disso é o dia inteiro, comportamento antigo preservado para reexecução manual de manhã) e `Get-VixLedgerEmissoresNaJanela` (mesmo regex de linha e mesmo corte por timestamp de `Test-VixLedgerEntregueNaJanela`, devolve o conjunto de emissores em vez da contagem). (b) `SKILL.md` da noturna (CCD store, `C:\Users\User\.claude\scheduled-tasks\vixradar-noturno\SKILL.md`), Passo 4 reescrito para chamar a lib com fallback inline da MESMA regra, Passo 0, Passo 6 e Passo 11 alinhados (`<N>` e `<A>+<S>+<D>` só contam carimbo >= `janelaInicio`, e `pendentes=0` encerra com `FIM_INVOCACAO:` sem `FIM:` novo). (c) Guarda `scripts/test-idempotencia-janela.ps1`, 20/20 no `powershell.exe` 5.1, prova de duas pontas. Caso A é o incidente real (103 `OK|` 09:20 a 09:33, invocação 18:15) e dá 103 pendentes na regra nova contra 0 na antiga, reproduzida dentro do teste. Caso B (103 `OK|` 18:20 a 18:45, retry 21:30) dá 0 pendentes. Caso D (recuperação de manhã mais noite interrompida no 40, retry 21:30) retoma com 63. Caso E (reexecução manual 11:00) preserva o comportamento antigo. Em A, B e D o vigia concorda com a skill. `lint-encoding.ps1` OK nos dois arquivos, `test-retry-janela.ps1` 14/14 sem regressão. (d) Guarda rodada contra o alvo real, o log de hoje às 18:48, pela lib e pelo fallback: `{"janelaInicio":"2026-09-03 18:00:00","naJanela":0,"foraDaJanela":103,"linhasOK":103}`. Segurança conferida no Worker antes de mudar, submit SKIP ou DEFERIDO nunca sobrescreve evento real (`persistirResultadoCompartilhadoInterno`, `api/src/worker.js:9496-9508`), `_carimbarAnaliseReal` ignora deferido (L9384), o plano credita análise do dia com tier SKIP (CREDITODIA1, L10739) e devolve deferido como FULL `deferred_prioritario` (L10703). **Efeito hoje.** O retry `Szuchmacher-RetryVixNoturno` das 21:30 relança pela skill nova, vê 0 na janela e faz a passada da noite de verdade (45 deferidos como FULL, analisados da manhã como SKIP barato), gravando `OK|` e `updated_at` depois das 18:00, o que satisfaz o vigia e o `painel_fresco` das 01:30. Custo de uma sessão normal da noturna (cap 700k), sujeito ao 429 de sessão, que a política de espera do FRESHNESS2 cobre. Para não rodar hoje, desabilitar a task de retry antes das 21:30. Sem deploy de Worker. Lateral não tocado, a matinal tem a mesma regra por dia civil (Passo 5, janela 10:00 no vigia), MATINAL-JANELA1 P3 se recorrer.

2. **SENTINELA-DIVIDA1, P3.** O portão da sentinela só abre por acervo CVM novo ou por backlog DELA MESMA (`scripts/run_vixradar_sentinela.ps1:312-319`, `$pendingAnterior` vem do estado gravado pela própria sentinela). Os 45 `DEFERIDO` da noturna não abrem o portão, então a sentinela saiu 18 vezes por dia com `sem_novidade` e 0 token em 02 e 03/09, e o gatilho DIVIDA do SENTINELA1 na prática só vale para deferido da própria pontual. Resultado medido, 17 emissores sem análise desde 31/08 no máximo, deferidos em 01/09 e em 03/09, fora das matinais e sem sentinela. São eles, Banco Pan, BTG Pactual, Camil Alimentos, CBA, Cielo, Direcional Engenharia, Gerdau, Iguá Saneamento, LWSA, MRS Logística, Neoenergia, PRIO, Rede D'Or, Santos Brasil, Trisul, Vibra Energia e Vivo (Telefônica Brasil). Hoje sem fato material (buscas acima), mas é latência de 3 dias sem rede de segurança. Correção, no portão, quando `$pendingAnterior` for false, consultar `listar_plano_rotina modo=pontual` (custo zero de LLM, 18 chamadas por dia) e abrir se `pontual_candidatos > 0`, ou a noturna gravar o pending da sentinela ao deferir.

**Laterais, uma linha cada.** Três subagentes desta sessão morreram juntos em 429 de limite de sessão (reset 14h), mesma família do elo 2 do FRESHNESS2, o trabalho foi refeito no processo principal. Governança da skill `/vix-radar-general-audit` e `audit-matrix.md` ainda dizem v4.9.235 e estão em edição por outra sessão, não tocadas aqui. `git log origin/main..HEAD` vazio, nada por empurrar.

---

## 03/09 — P1 FECHADO, DEPLOYADO E CONFIRMADO EM PRODUÇÃO (INCIDENTE-FRESHNESS2, v4.9.236): painel parado 12h41 com `ok:true`

> **Status:** FECHADO. Código, testes, gate adversarial de causa, gate adversarial de produção e agente refutador final concluídos; deploy confirmado em produção (`versao:"v4.9.236"`, portão de verificação colado às 14:47Z de 03/09: `HTTP:200 ok:true kv:true telemetria:true sentry_ok:true painel_fresco:true`); 8 resíduos declarados abaixo, nenhum bloqueante (2 achados pelo agente refutador final, veredito geral PASS) · **Data da Versão:** 2026-09-03 · **Origem do Registro:** evidência visual do operador (02/09 ~23:50 BRT, aba anônima: "Painel atualizado em 02 de setembro de 2026 às 11:09 BRT" com o relógio da própria tela em 23:50) · **Condição de Obsolescência:** cai quando a agenda das rotinas mudar (o SLA de `painel_fresco` é derivado dela) ou quando `FERIADOS_B3_2026/2027` precisarem de renovação (resíduo 8).
...
**Correções (commits `43f99f6` + `8fbfa95`; Worker v4.9.236 DEPLOYADO e confirmado em produção, commit de deploy `20f8a25`, detalhe em `status/ESTADO.md`; nenhum dos 3 empurrado para `origin/main` ainda, decisão do operador).** Política de 429 decidida pelo operador: assinatura primeiro, espera pelo reset real até 120 min, chave paga só como contingência com `ALERTA_AUTH` + e-mail antes, nunca para falha que não seja limite da própria assinatura, e nunca persistida em escopo User. A sonda passou a classificar (`session_limit` / `rate_limit_transitorio` / `websearch_indisponivel` / `erro_desconhecido`) e a extrair o horário de reset do texto real do erro. A linha headless virou `--tools default` + `--permission-mode dontAsk` + `--allowedTools` explícito, com `CLAUDE_CODE_USE_POWERSHELL_TOOL=1` e uma sonda nova (`Test-VixHeadlessTools`) que roda o CLI de verdade e exige shell, leitura, escrita, subagente e busca antes de deixar a rotina começar. Entrega passou a ser julgada pelo ledger `OK|` dentro da janela da rotina, nunca pelo exit code, e o relançamento é reconferido depois de rodar (não entregou, alerta). No Worker, `painel_atualizado_em`/`painel_idade_min`/`painel_fresco`/`painel_regra`/`painel_exigido_desde` no `GET /` e em `checks.painel_fresco`, com SLA derivado da agenda real, fora do `ok` agregado; `canonical-test.yml` reprova o run quando `painel_fresco=false`.

**Prova de duas pontas com o CLI real (03/09):** a linha nova passa a sonda headless (shell, leitura, escrita, subagente e busca confirmados, zero `permission_denials`); a linha antiga reprova por falta de `PowerShell/Bash, Read, Write, Edit, Agent(Task)`. Testes: 241 vitest em 26 arquivos (+8 casos de `painel_fresco`, incluindo o cenário mais próximo do incidente e os dois cenários de feriado B3 do FERIADOB3-PAINEL1 abaixo) e 85 asserts em 3 suítes PowerShell (48 + 14 + 23; número corrigido pelo agente refutador final, que recontou `Assert(` literal nos 3 arquivos — o commit e uma versão anterior deste registro diziam "95 em 4 suítes", contagem errada, nunca existiu um quarto arquivo de teste PowerShell deste incidente).
...
**Gate do agente refutador final (03/09, depois do commit).** Reproduziu o estado do zero, atacou cada achado com medição própria (48/48 + 14/14 PowerShell reproduzidos, 241/241 vitest reproduzido isolado, `node --check` e `git diff --check` reproduzidos, `_okHealth` lido linha a linha para confirmar ausência de `painel_*`). **Veredito: PASS**, nenhum achado anterior foi derrubado. Achou 3 coisas que valem registro: (1) a contagem de testes PowerShell no commit e neste documento estava errada (corrigido acima); (2) o teste rotulado "reproduz o incidente de 02/09" usa como agora `2026-09-03T04:40:00Z` (01:40 BRT do dia seguinte, já depois do prazo de 01:30 da regra noturna), não o instante real em que o operador olhou a tela (02/09 23:50 BRT); simulando esse instante real com o código verbatim, `painel_fresco` teria dado `true`, não `false`, porque antes do prazo de 01:30 o checkpoint que vale ainda é a matinal daquele dia (satisfeita às 11:09). Isso não é um bug novo, é exatamente o item 6 abaixo (lag de detecção do SLA) com um número mais preciso: o campo só passaria a acusar a partir de 01:30, não na hora real da queda; (3) dois resíduos novos, itens 7 e 8 abaixo.

### Resíduos declarados (não fechados nesta sessão)

1. **P2 — O laço de retry da invocação PRINCIPAL não é 429-aware.** A correção cobre o PRÉ-FLIGHT. O laço que roda a skill de verdade (`run_claude_routine.ps1`, `$retryDelays = @(0,30,60)`) só faz backoff curto e desiste. Medido na própria recuperação de 03/09 01:44: 3 tentativas em 2 min, todas com `You've hit your session limit · resets 3:50am`, e `ERRO: claude exit 1`. O reset era 2h05 depois, além do teto de 120 min de qualquer forma, mas o log deveria dizer "429 de limite de sessão, reset HH:MM" em vez de "esgotadas 3 tentativas", e deveria alertar. **FECHADO em 16/09/2026 (COTAESGOTADA1-GENERICO, `scripts/run_claude_routine.ps1`, commit `6d6b9cd`):** o laco passou a classificar a falha com `Get-VixWsProbeClassificacao` (lib de ambiente) e a decidir com a tabela pura `Get-VixSessionLimitAcao` (lib de auth), ambas ja dot-sourced no proprio arquivo - nenhuma regex nova, parser intocado. Cota esgotada com reset fora do teto de parede e sem chave paga para assumir a escalada => UMA chamada ao CLI (antes: 3) e fail-closed no exit code que o runner ja tinha, com motivo operacional honesto em linha propria (`COTA_ESGOTADA: ... reset=HH:MM ... sem retry (a mesma credencial nao muda o desfecho) e sem fallback pago autorizado (ANTHROPIC_API_PAYG=NAO AUTORIZADO)`), mais `ERRO: claude exit N (1 de 3 tentativas: ...)`. Reset que caiba no teto de parede continua sendo esperado (assinatura e mais barata que a chave avulsa) e falha que NAO e cota mantem as 3 tentativas com backoff. Prova medida com harness proprio e stub do CLI (zero token): 3 chamadas na versao anterior / 1 na atual / 3 em falha transiente, sobre o conteudo comitado; mais `lint-encoding` RISCO 0, `preflight-scan` RISCO 0, `test-session-limit-policy` 19/19, `test-retry-janela` 24/24, `check-claude-free` exit 0 e `Get-VixAnthropicApiKey` devolvendo `$null` sob `claude-subscription` (escalada paga inerte). **Nao fechado aqui:** o alerta ao admin neste caminho (`Send-VixRoutineAlert`) nao foi adicionado, para manter o diff minimo; sem ele o operador so ve o aborto no log da rotina. **Correcao de corpus:** desde o MOTORRETRY1 (12/09) os dois wrappers de retry do VIX Radar relancam o MOTOR, nao este runner; o consumidor vivo do runner generico e a rotina semanal `Szuchmacher-AgendaMacro-Claude` (Task Scheduler, `GuardTarget` do `preflight-and-run.ps1`, proxima execucao 18/09/2026 07:07), que roda sem pre-flight de WebSearch e por isso cai exatamente neste laco.
...
7. **P3 — `_lerUpdatedAtEstado` só olha 2 semanas ISO (achado do agente refutador final).** Se o painel ficar mudo por mais de ~8 dias corridos (varia com o dia da semana em que a checagem cai), `painel_atualizado_em` deixa de achar qualquer `updated_at` nas 2 chaves consultadas e `painel_fresco` regride de `false` para `null`. `canonical-test.yml` só reprova em `painel_fresco="false"` literal, então o gate para de acusar depois desse ponto. Severidade baixa: antes de chegar aí o canonical-test já teria reprovado dezenas de vezes seguidas (a cada 6h por 7-8 dias), e o sentido do erro é conservador (vira "sem dado para conferir", nunca finge frescor). Corrigir exigiria ampliar a janela de busca ou tratar ausência prolongada como `false` explícito.
8. **P4 — `_ehFeriadoB3` fica cego a partir de 2028 sem manutenção (achado do agente refutador final).** Só existem `FERIADOS_B3_2026`/`FERIADOS_B3_2027` no arquivo. A partir de 01/01/2028 qualquer feriado B3 (ex.: Tiradentes 21/04/2028, sexta) deixa de ser reconhecido, e o `canonical-test` voltaria a reprovar por falso positivo, o mesmo bug que o FERIADOB3-PAINEL1 corrigiu agora. Lembrete: renovar os sets de feriado B3 antes do fim de 2027.
