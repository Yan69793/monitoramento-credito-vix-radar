...
> **Status:** RESOLVIDO DE FATO para o código. `cvm_novos` estava zerado para os 103 emissores todo dia desde 25/08 (SENTINELA1), dois defeitos independentes, os dois corrigidos e deployados. Horário da rotina mudado no config, mas ainda NÃO ATIVO (depende de restart do Claude Desktop, ação do operador). Fonte intradiária oficial avaliada e bloqueada por credencial/CAPTCHA que não existem no ambiente, documentado como limite externo, não pendência de código.
...
> **Condição de Obsolescência:** cai quando o Worker passar do v4.9.226. O item de horário fica ABERTO separadamente até o restart do Claude Desktop acontecer e for confirmado que `vixradar-noturno` disparou às 08h00 BRT.
