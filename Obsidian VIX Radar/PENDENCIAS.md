...
## 19/09: o `ok` do frescor da CVM parou de acompanhar a idade da fonte em 13/09 (DECISAO A), e o `CLAUDE.md` descreve a regra antiga

> **Status:** ABERTO, decisão do operador, sem correção (o Worker não foi tocado nesta sessão). **Data:** 2026-09-19. **Origem do Registro:** achado ao ressuscitar `scripts/test-frescor-cvm.mjs` (HARNESSMORTO1, fechado no repositório, ver `03 - Estado Atual.md`). Prioridade a definir pelo operador.
> **Medido, na função real extraída do `worker.js`.** Com hoje fixado em 19/08 e meta `{ ok: true, last_modified_iso: "2026-08-05" }` (14 dias, 2 ciclos), `avaliarFrescorCVM` devolve `{ ok: true, motivo: "fonte_sem_publicar_ha_2_ciclos_semanais_14_dias" }`, com `idade_dias: 14` e `ciclos_perdidos: 2`. Meta `{ ok: true }` sem data devolve `{ ok: true, motivo: "sem_data_de_referencia" }`. **Antes/depois medido com o mesmo harness de 46 casos:** contra o `worker.js` de `ced7902` (pai do `2c8acc6`) falham exatamente 3 casos, todos de saída antecipada da função (14 dias, backfill de 14 dias e meta sem data), cada um com `obtido: {"ok":false,...}`. Contra o de hoje, 46/46.
> **Causa no código.** O `2c8acc6` (13/09 21:37, "separar health de escrita e reconciliacao") passou a atribuir `out.ok = meta.ok === true` em `api/src/worker.js:7964`, antes das saídas por idade (`:8009` sem data, `:8018` data inválida, `:8023` ciclos), que ficaram sem zerar o `ok`. Antes disso `out.ok` só virava `true` no fim da função (`out.ok = false` no ramo do gate de reconciliação, `out.ok = true` na última linha, medido no `ced7902`). O `d392335` (13/09 21:51) virou a suíte vitest para a nova semântica com o rótulo "DECISAO A" (`api/test/cvm-frescor.test.mjs:145`, `:163`, `:191`, `:219`, `:293`). Não achei a decisão escrita em `CLAUDE.md`, `status/` nem no vault, só nesses comentários.
> **Consequência lida no código, não medida em produção.** `fonte_externa_ok` é `!!ok` (`:20413`, `:20414`) e `fonte_dentro_cadencia` do AVANCOFEED1 é `!!(_avFrescor && _avFrescor.ok)` (`:19928`). Uma fonte sem publicar há semanas, com o sync gravando sem erro, não derruba nenhum dos dois. O único leitor que alerta por `fonte_externa_ok=false` é o `watch-vixradar-health.ps1` (`:250`, desligado desde 21/08). O `deploy-worker.ps1` (`:414`) só avisa. O `canonical-test.yml` (`:162`) só anota um `::error::` sem reprovar o job quando `cvm_fonte_ok=false`, e reprova apenas se o campo sumir (`:165`). Sobra para essa situação o gate de evento em dias úteis, que o CVMDURA1 já registrou como lento (6 dias para acusar).
> **Documentação que ainda descreve a regra antiga.** O `CLAUDE.md`, seção "Portão de verificação" (`fonte_externa_ok` "só vai a false depois de dois ciclos semanais perdidos"). O comentário de `avaliarFrescorCVM` (`:7885`, "Fail-closed"), que hoje não vale para meta sem data. Os títulos dos testes `api/test/cvm-frescor.test.mjs:134` e `:153` ("derruba fonte_externa_ok"), que contradizem as asserções `toBe(true)` logo abaixo (`:147`, `:164`).
> **Decisão que só o operador toma.** (a) Manter a semântica de escrita. Então corrigir o `CLAUDE.md`, o comentário da função e os dois títulos, e apontar qualquer alerta de fonte parada para `cvm_fonte_ciclos_perdidos` em vez de `fonte_externa_ok`. (b) Reverter. Então o `ok` volta a cair pela idade, e o harness e a suíte vitest voltam junto. O harness pina o contrato de hoje de propósito, então qualquer das duas escolhas o quebra e obriga a decidir, que é o desenho.
> **Condição de Obsolescência:** cai quando o `CLAUDE.md`, o comentário da função e o comportamento do `ok` disserem a mesma coisa.
