...
| P15 | **Histórico estendido na timeline (3 meses).** Ampliar janela da timeline do emissor usando `op=historico_emissor` (reclassificação do P10 original como prioridade). | **DEPLOYADO v201.54** (2026-06-17) — módulo `#p15-timeline-module`; janela 90d; fetch `historico_emissor` ao selecionar emissor. | Médio | P |
